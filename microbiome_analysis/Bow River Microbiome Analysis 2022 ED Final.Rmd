...
# Filtering/pre-processing steps
...
psbl_1<-subset_samples(psbl, Blank_Type!="Buffer tube" & Blank_Type!="DNA extraction reagent")
psbl_1 #19 samples
...
blank_ps<-psbl2
blank_df<-psmelt(blank_ps)
nsamples(blank_ps) #23 blanks total (Water September:3, emergence trap:3, Water July:3, Sheet:4, buffer:2, extraction_neg:6, extraction_neg_4:2)

blank_Bow<-subset_samples(blank_ps, Stream_Type!="ACWA Streams")
nsamples(blank_Bow) #17
blank_ACWA<-subset_samples(blank_ps, Stream_Type!="Bow River")
nsamples(blank_ACWA) #16
```
...
nsamples(spider_Bow) #76

#Total number of reads
print(sum(sample_sums(Bow_ps))) #8,547,820 
#Average # of reads per sample
mean(sample_sums(Bow_ps)) #30,970.36 average reads/sample
...
get_taxa_unique(Bow_ps, "Class") #88 Classes total
...
### Testing significant differences in relative abundance across sites and invert taxa
```{r}
#### Phylum Level ####
...
shapiro.test(bactero_hydro_Bow$Abundance) #p=0.343 normal
leveneTest(data=bactero_hydro_Bow, Abundance ~ Site) #p=0.054, normal
...
shapiro.test(proteo_hydro_Bow$Abundance) #p=0.253 normal
leveneTest(data=proteo_hydro_Bow, Abundance ~ Site) #p=0.2095, normal
summary(aov(data=proteo_hydro_Bow, Abundance ~ Site)) #p = 0.819 no diff.

#Firmicutes
firm_hydro_Bow<-subset(hydro_phy_Bow_df, Phylum=="Firmicutes")
shapiro.test(firm_hydro_Bow$Abundance) #p=0.100 normal
leveneTest(data=firm_hydro_Bow, Abundance ~ Site) #p=0.748, normal
summary(firm_hydro<-aov(data=firm_hydro_Bow, Abundance ~ Site)) #p = 0.013 significant difference
TukeyHSD(firm_hydro) #Cochrane has sig higher proportion of Firmicutes than PMF

#Deferribacterota
def_hydro_Bow<-subset(hydro_phy_Bow_df, Phylum=="Deferribacterota")
shapiro.test(def_hydro_Bow$Abundance) #p=0.0012 not normal
leveneTest(data=def_hydro_Bow, Abundance ~ Site) #p=0.094, normal
summary(aov(data=def_hydro_Bow, Abundance ~ Site)) #p = 0.155 no diff
kruskal.test(data=def_hydro_Bow, Abundance ~ Site) #p=0.207 no diff. 


#Other phyla less than 2 %
...
shapiro.test(proteo_hep_Bow$Abundance) #p=0.047 not normal
leveneTest(data=proteo_hep_Bow, Abundance ~ Site) #p=0.129, normal
summary(proteo_hep<-aov(data=proteo_hep_Bow, Abundance ~ Site)) #p = 0.045 sig diff
TukeyHSD(proteo_hep) #GRVBR has more Proteobacteria than COCH
kruskal.test(data=proteo_hep_Bow, Abundance ~ Site) #p=0.069 no diff

#Firmicutes
firm_hep_Bow<-subset(hep_phy_Bow_df, Phylum=="Firmicutes")
shapiro.test(firm_hep_Bow$Abundance) #p=0.014 not normal
leveneTest(data=firm_hep_Bow, Abundance ~ Site) #p=0.400, normal
summary(aov(data=firm_hep_Bow, Abundance ~ Site)) #p = 0.075 no diff.
kruskal.test(data=firm_hep_Bow, Abundance ~ Site) #p=0.095 no diff

#Bacteroidota
bactero_hep_Bow<-subset(hep_phy_Bow_df, Phylum=="Bacteroidota")
shapiro.test(bactero_hep_Bow$Abundance) #p=0.093 normal
leveneTest(data=bactero_hep_Bow, Abundance ~ Site) #p=0.937, normal
summary(aov(data=bactero_hep_Bow, Abundance ~ Site)) #p = 0.398 no diff.
...
chiro_phy_Bow<-subset_samples(Bow_glom_phy, Family=="Chironomidae")
chiro_phy_Bow<-prune_taxa(taxa_sums(chiro_phy_Bow) > 0, chiro_phy_Bow) 
chiro_phy_Bow_df<-psmelt(chiro_phy_Bow)

shapiro.test(chiro_phy_Bow_df$Abundance) #Not normal
...
#Proteobacteria
proteo_chiro_Bow<-subset(chiro_phy_Bow_df, Phylum=="Proteobacteria")
shapiro.test(proteo_chiro_Bow$Abundance) #p=0.028 not normal
leveneTest(data=proteo_chiro_Bow, Abundance ~ Site) #p=0.57, normal
summary(aov(data=proteo_chiro_Bow, Abundance ~ Site)) #p = 0.157 sig diff
kruskal.test(data=proteo_chiro_Bow, Abundance ~ Site) #p=0.094 no diff

#Bacteroidota
bactero_chiro_Bow<-subset(chiro_phy_Bow_df, Phylum=="Bacteroidota")
shapiro.test(bactero_chiro_Bow$Abundance) #p=0.012 not normal
leveneTest(data=bactero_chiro_Bow, Abundance ~ Site) #p=0.582, normal
summary(aov(data=bactero_chiro_Bow, Abundance ~ Site)) #p = 0.291 no diff
kruskal.test(data=bactero_chiro_Bow, Abundance ~ Site) #p=0.083 no diff

#Firmicutes
firm_chiro_Bow<-subset(chiro_phy_Bow_df, Phylum=="Firmicutes")
shapiro.test(firm_chiro_Bow$Abundance) #p=0.0018 not normal
leveneTest(data=firm_chiro_Bow, Abundance ~ Site) #p=0.0497, not normal
kruskal.test(data=firm_chiro_Bow, Abundance ~ Site) #p=0.43 no diff

#Actinobacteriota
actino_chiro_Bow<-subset(chiro_phy_Bow_df, Phylum=="Actinobacteriota")
shapiro.test(actino_chiro_Bow$Abundance) #p=0.011 not normal
leveneTest(data=actino_chiro_Bow, Abundance ~ Site) #p=0.784, normal
summary(aov(data=actino_chiro_Bow, Abundance ~ Site)) #p = 0.282 no diff.
kruskal.test(data=actino_chiro_Bow, Abundance ~ Site) #p=0.558 no diff
...
perl_phy_Bow<-subset_samples(Bow_glom_phy, Family=="Perlidae")
perl_phy_Bow<-prune_taxa(taxa_sums(perl_phy_Bow) > 0, perl_phy_Bow) 
perl_phy_Bow_df<-psmelt(perl_phy_Bow)

shapiro.test(perl_phy_Bow_df$Abundance) #Not normal
...
#Proteobacteria
proteo_perl_Bow<-subset(perl_phy_Bow_df, Phylum=="Proteobacteria")
shapiro.test(proteo_perl_Bow$Abundance) #p=0.791 normal
leveneTest(data=proteo_perl_Bow, Abundance ~ Site) #p=0.83, normal
summary(aov(data=proteo_perl_Bow, Abundance ~ Site)) #p = 0.153 no diff

#Bacteroidota
bactero_perl_Bow<-subset(perl_phy_Bow_df, Phylum=="Bacteroidota")
shapiro.test(bactero_perl_Bow$Abundance) #p=0.277 normal
leveneTest(data=bactero_perl_Bow, Abundance ~ Site) #p=0.602, normal
summary(aov(data=bactero_perl_Bow, Abundance ~ Site)) #p = 0.212 no diff
...
### Comparing Relative Abundance Ratios - Phyla ###
```{r}
#### Graph top 3 most abundant phyla (Bacteroidota, Proteobacteria, Firmicutes) Bow River ####
...
rel_abun_prune_sample_bow$Site_code <- factor(rel_abun_prune_sample_bow$Site_code, levels=c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF"))
rel_abun_prune_sample_bow$sample_Family<-factor(rel_abun_prune_sample_bow$sample_Family, levels=c("Hydropsychidae", "Heptageniidae", "Baetidae", "Chironomidae", "Perlidae", "Trichoptera", "Ephemeroptera", "Diptera", "Araneidae", "Tetragnathidae"))

rel_abun_bow_subset<-subset(rel_abun_prune_sample_bow, Phylum=="Bacteroidota" | Phylum=="Proteobacteria" | Phylum=="Firmicutes")

ggplot(rel_abun_bow_subset, aes(x=Site_code, y=Abundance, fill=Phylum)) + 
  geom_boxplot() + 
  ...
  theme(axis.title.y=element_text(size=22),
        ...
        axis.text.x=element_text(angle=45, hjust=1, size=16, color="black", face=colorado(rel_abun_bow_subset$Site_code, c("GRVBR", "PMF"))),
                        axis.title.x=element_text(size=22))+
  scale_fill_manual(name="Phylum", values = c("#1F78B4", "#B2DF8A", "#CC9966"))+
  scale_y_continuous(label = scales::percent)+
  rremove("grid")+
  xlab("Site")+
  facet_nested_wrap(~Stage+sample_Family, scales="free_x", nrow=1)

#ggsave(filename="microbiome_analysis/Final Analysis/Relative abundance/All samples/top 3 phyla boxplot Bow.png", height=10, width=22)




#### Graph top 3 most abundant phyla (Bacteroidota, Proteobacteria, Firmicutes) ACWA ####
...
rel_abun_prune_sample_ACWA$Site_code <- factor(rel_abun_prune_sample_ACWA$Site_code, levels=c("BRR2", "PCR1", "PCR3"), labels=c("REF", "EFF5", "EFF15"))
rel_abun_prune_sample_ACWA$sample_Family<-factor(rel_abun_prune_sample_ACWA$sample_Family, levels=c("Hydropsychidae", "Baetidae", "Ephemeroptera"))

rel_abun_ACWA_subset<-subset(rel_abun_prune_sample_ACWA, Phylum=="Bacteroidota" | Phylum=="Proteobacteria" | Phylum=="Firmicutes")

ggplot(rel_abun_ACWA_subset, aes(x=Site_code, y=Abundance, fill=Phylum)) + 
  geom_boxplot() + 
  ylab("Relative Abundance") +
  theme_bw()+
  rremove("grid")+
...
#ggsave(filename="microbiome_analysis/Final Analysis/Relative abundance/All samples/top 3 phyla boxplot ACWA.png", height=10, width=22)

#### Bacteroidota to Proteobacteria Bow ####

#Higher ratio of Bacteroidota to Proteobacteria can signify dysbiosis (more B, less P)
...
rel_abun_bow_subset<-subset(rel_abun_prune_sample_bow, Phylum=="Bacteroidota" | Phylum=="Proteobacteria")
...
ggplot(rel_abun_bow_subset, aes(x=Site_code, y=Abundance, fill=Phylum)) + 
  ...
  theme(axis.title.y=element_text(size=22),
        ...
        axis.text.x=element_text(angle=45, hjust=1, size=16, color="black", face=colorado(rel_abun_bow_subset$Site_code, c("GRVBR", "PMF"))),
                        axis.title.x=element_text(size=22))+
  scale_fill_manual(name="Phylum", values = c("#1F78B4", "#CC9966"))+
  scale_y_continuous(label = scales::percent)+
  rremove("grid")+
  xlab("Site")+
  facet_nested_wrap(~Stage+sample_Family, scales="free_x", nrow=1)

#ggsave(filename="microbiome_analysis/Final Analysis/Relative abundance/All samples/ratio Bact-Prot.png", height=10, width=22)


#### Firmicutes to Bacteroidota Bow ####

#Higher ratio of Firmicutes to Bacteroidota can signify dysbiosis (more F, less B)
...
rel_abun_bow_subset<-subset(rel_abun_prune_sample_bow, Phylum=="Bacteroidota" | Phylum=="Firmicutes")
...
ggplot(rel_abun_bow_subset, aes(x=Site_code, y=Abundance, fill=Phylum)) + 
  ...
  theme(axis.title.y=element_text(size=22),
        ...
        axis.text.x=element_text(angle=45, hjust=1, size=16, color="black", face=colorado(rel_abun_bow_subset$Site_code, c("GRVBR", "PMF"))),
                        axis.title.x=element_text(size=22))+
  scale_fill_manual(name="Phylum", values = c("#1F78B4", "#B2DF8A"))+
  scale_y_continuous(label = scales::percent)+
  rremove("grid")+
  xlab("Site")+
  facet_nested_wrap(~Stage+sample_Family, scales="free_x", nrow=1)

#ggsave(filename="microbiome_analysis/Final Analysis/Relative abundance/All samples/ratio Firm-Bact.png", height=10, width=22)
```
...
#ggsave(filename="microbiome_analysis/Final analysis/Relative abundance/All samples/Mean rel abun top 30 genera Bow.png", height=16, width=30)
...
top_phyla_ACWA <- sample.df.agg.ACWA %>%
    ...
    arrange(-Mean)
top_phyla_ACWA
top5_ACWA <- top_phyla_ACWA$Phylum[1:12]
...
df0_ACWA$Phylum <- factor(df0_ACWA$Phylum, levels=c("Bacteroidota", "Rs-K70 termite group", "Firmicutes", "Deferribacterota", "Other", "Proteobacteria"), labels=c("Bacteroidota", "Rs-K70 termite group", "Firmicutes", "Deferribacterota", "Other", "Proteobacteria"))
...
Papers that do not support rarefying:
...
shan.plot.taxa.ACWA<-ggplot(adiv_ACWA, aes(x=Site, y=Shannon, fill=Site))+
  ...
  labs(x = "Site", y = "Shannon Diversity") +
  theme_bw()+
  theme(legend.position="none", axis.text.x=element_text(angle=45, hjust=1, size=22, color="black"),
...
adiv_Bow_rare$Site_code<-factor(adiv_Bow_rare$Site_code, levels=c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF"))
adiv_Bow_rare$Family<-factor(adiv_Bow_rare$Family, levels=c("Hydropsychidae", "Heptageniidae", "Baetidae", "Chironomidae", "Perlidae", "Trichoptera", "Ephemeroptera", "Diptera", "Araneidae", "Tetragnathidae"))

#Shannon diversity across taxa
shan.plot.taxa.rare<-ggplot(adiv_Bow_rare, aes(x=Site_code, y=Shannon, fill=Site_code))+
  facet_nested_wrap(~ Stage + Family, scales= "free_x", nrow=1)+
  geom_boxplot()+
  labs(x = "Site", y = "Shannon Diversity")+
  theme_bw()+
  theme(legend.position="none", axis.text.x=element_text(angle=45, hjust=1, size=16, color="black", face=colorado(adiv_Bow_rare$Site_code, c("GRVBR", "PMF"))),
        strip.text.x = element_text(size=20), 
        axis.text.y = element_text(size=16, color="black"), axis.title.x = element_text(size=18), axis.title.y = element_text(size=18))+
  rremove("grid")+
  scale_fill_brewer(palette="Accent")
...
shapiro.test(adiv_hydro_Bow_rare$Shannon) #Normal dist. p = 0.073
leveneTest(Shannon~Site, data=adiv_hydro_Bow_rare) #Homogeneity. p = 0.239
#Use ANOVA
...
plot(hydro_aov_rare) #Adheres to assumptions of ANOVA

#Adult Trichoptera
div.df.Bow.Tricho.rare<-subset(adiv_Bow_rare, Family=="Trichoptera")
shapiro.test(div.df.Bow.Tricho.rare$Shannon) #Not normal p < 0.001
leveneTest(Shannon~Site, data=div.df.Bow.Tricho.rare) #Heterogeneity p = 0.095
#Use Kruskal-Wallis
kruskal.test(Shannon~Site, data=div.df.Bow.Tricho.rare) #p= 0.551 no significant diff. across sites.

#Larval Heptageniidae (order Ephemeroptera)
div.df.Bow.Hep.rare<-subset(adiv_Bow_rare, Family=="Heptageniidae")
shapiro.test(div.df.Bow.Hep.rare$Shannon) #Normal dist. p = 0.197
leveneTest(Shannon~Site, data=div.df.Bow.Hep.rare) #Homogeneity p = 0.621
#Use ANOVA
summary(hep_aov_rare<-aov(Shannon~Site, data=div.df.Bow.Hep.rare)) #p=0.129, no differences

plot(hep_aov_rare) #Adheres to assumptions of ANOVA

#Adult Ephemeroptera
div.df.Bow.Ephem.rare<-subset(adiv_Bow_rare, Family=="Ephemeroptera")
shapiro.test(div.df.Bow.Ephem.rare$Shannon) #p=0.0026, not normal
leveneTest(Shannon~Site, data=div.df.Bow.Ephem.rare) #Homogeneity p = 0.387

#Use Kruskal-Wallis
kruskal.test(Shannon~Site, data=div.df.Bow.Ephem.rare) #p=0.038
dunnTest(Shannon~Site, data=div.df.Bow.Ephem.rare) #No sig pairwise comparisons

#Larval Chironomidae (Order Diptera)
div.df.Bow.chiro.rare<-subset(adiv_Bow_rare, Family=="Chironomidae")
shapiro.test(div.df.Bow.chiro.rare$Shannon) #0.0206 not normal
leveneTest(Shannon~Site, data=div.df.Bow.chiro.rare) #Homogeneity. p = 0.845
#Use KW
kruskal.test(Shannon~Site, data=div.df.Bow.chiro.rare) #p=0.387 no differences


#Adult Chironomidae (Order Diptera)
div.df.Bow.dip.rare<-subset(adiv_Bow_rare, Family=="Diptera")
shapiro.test(div.df.Bow.dip.rare$Shannon) #p=0.057 Normal
leveneTest(Shannon~Site, data=div.df.Bow.dip.rare) #0.799. Homogeneity.
#Use ANOVA
summary(dip_aov_rare<-aov(Shannon~Site, data=div.df.Bow.dip.rare)) #p=0.999 no diff

plot(dip_aov_rare) #Adheres to assumptions of ANOVA

#Larval Perlidae (Order Plecoptera)
div.df.Bow.perl.rare<-subset(adiv_Bow_rare, Family=="Perlidae")
shapiro.test(div.df.Bow.perl.rare$Shannon) #0.057 normal
leveneTest(Shannon~Site, data=div.df.Bow.perl.rare) #0.111. Heterogeneity

#ANOVA
summary(aov(Shannon~Site, data=div.df.Bow.perl.rare)) #p=0.0873 no differences

#Araneidae
div.df.Bow.aran.rare<-subset(adiv_Bow_rare, Family=="Araneidae")
shapiro.test(div.df.Bow.aran.rare$Shannon) #0.0365 not normal
leveneTest(Shannon~Site, data=div.df.Bow.aran.rare) #0.828 homogeneity
#Use KW

kruskal.test(Shannon~Site, data=div.df.Bow.aran.rare) #p < 0.001
dunnTest(Shannon~Site, data=div.df.Bow.aran.rare)
#Differences between:
#COCH - PMF
#CUSHBR - PMF
#COCH - SUN
...
#Use KW
kruskal.test(Shannon~Site, data=div.df.Bow.tetra.rare) #p=0.24 No differences across sites 

#Rarefied data resulted in the same interpretations and significant comparisons as non rarefied data.
```
...
hydro_lar_rare<-subset_samples(ps_rare, Family=="Hydropsychidae")

adiv_hydro_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(hydro_lar_rare, measures = "Shannon"),
  "Stream_Type" = phyloseq::sample_data(hydro_lar_rare)$Stream_Type,
  "Site" = phyloseq::sample_data(hydro_lar_rare)$Site)
head(adiv_hydro_rare)
...
hep_lar_rare<-subset_samples(ps_rare, Family=="Heptageniidae")

adiv_hep_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(hep_lar_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(hep_lar_rare)$Site)
...
chiro_lar_rare<-subset_samples(ps_rare, Family=="Chironomidae")

adiv_chiro_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(chiro_lar_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(chiro_lar_rare)$Site)
...
perl_lar_rare<-subset_samples(ps_rare, Family=="Perlidae")

adiv_perl_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(perl_lar_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(perl_lar_rare)$Site)
...
baetid_lar_rare<-subset_samples(ps_rare, Family=="Baetidae")

adiv_baetid_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(baetid_lar_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(baetid_lar_rare)$Site)
...
trichop_ad_rare<-subset_samples(ps_rare, Family=="Trichoptera")

adiv_trichop_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(trichop_ad_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(trichop_ad_rare)$Site)
...
ephem_ad_Bow_rare<-subset_samples(ps_rare, Family=="Ephemeroptera" & Stream_Type=="Bow River")

adiv_ephem_Bow_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(ephem_ad_Bow_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(ephem_ad_Bow_rare)$Site)
...
ephem_ad_ACWA_rare<-subset_samples(ps_rare, Family=="Ephemeroptera" & Stream_Type!="Bow River")

adiv_ephem_ACWA_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(ephem_ad_ACWA_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(ephem_ad_ACWA_rare)$Site)

adiv_ephem_ACWA_rare %>%
  group_by(Site) %>%
  dplyr::summarise(mean = mean(Shannon),
            sd= sd(Shannon))
...
dip_ad_rare<-subset_samples(ps_rare, Family=="Diptera")

adiv_dip_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(dip_ad_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(dip_ad_rare)$Site)
...
adiv_aran_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(aran_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(aran_rare)$Site)
...
adiv_tetra_rare <- data.frame(
  "Shannon" = phyloseq::estimate_richness(tetra_rare, measures = "Shannon"),
  "Site" = phyloseq::sample_data(tetra_rare)$Site)
...
Transforming the data to proportions (relative abundance) is the best method of normalizing the data prior to beta diversity analysis according to Weiss et al., 2017; McKnight et al., 2019. 
...
nsamples(Bow_ps<-subset_samples(sample_ps, Stream_Type=="Bow River")) #276 samples
nsamples(psbl_1) #19
bl_Bow<-subset_samples(psbl_1, Stream_Type!="ACWA Streams")
nsamples(bl_Bow) #13
exp_bl_ps_Bow<-merge_phyloseq(bl_Bow, Bow_ps)
nsamples(exp_bl_ps_Bow) #289 (276 samples, 13 blanks)

#Ordinating blanks with experimental samples to see how closely related they are

#PCoA with Bray Curtis distance matrix
ps_rel_abun_Bow = transform_sample_counts(exp_bl_ps_Bow, function(x) x/sum(x))

dist_all_Bow = phyloseq::distance(ps_rel_abun_Bow, method="bray") #Bray Curtis distance matrix
ordination_all_Bow = ordinate(ps_rel_abun_Bow, method="PCoA", distance=dist_all_Bow) #Ordinate using PCoA

plot_ordination(ps_rel_abun_Bow, ordination_all_Bow, shape="Blank_collection_method", color="Sample_Type") +
  theme_classic() +
...
set.seed(1)
ord_NMDS_blanks_Bow = ordinate(ps_rel_abun_Bow, method="NMDS", distance=dist_all_Bow)
ord_NMDS_blanks_Bow #stress=0.192


blank_nmds_Bow<-plot_ordination(ps_rel_abun_Bow, ord_NMDS_blanks_Bow, color="Sample_Type", shape="Blank_Type") + 
  geom_point()+
  theme_classic() +
  theme(strip.background = element_blank())+
  theme(legend.key.size = unit(1, 'cm'), 
...
blank_nmds_Bow$Blank_Type <- factor(blank_nmds_Bow$Blank_Type, levels = c("Tweezer water rinse","Bedsheet","Buffer tube","DNA extraction reagent","Nested PCR negative"))
print(blank_nmds_Bow)

#ggsave(filename="microbiome_analysis/Final analysis/Beta Diversity Graphs/blank and experimental samples Bow River.png", height=8, width=10)

metadata_blanks_Bow <- data.frame(sample_data(ps_rel_abun_Bow))

adonis.blanks.Bow <- adonis2(dist_all_Bow ~ Sample_Type, data = metadata_blanks_Bow)
adonis.blanks.Bow #p=0.002. Significant dissimilarity between blank samples and invertebrate samples
#F(1, 288) = 2.85, P = 0.002

#Testing variance (beta dispersion) differences between samples and blanks:
bd_blanks_Bow<-betadisper(dist_all_Bow, metadata_blanks_Bow$Sample_Type)
anova(bd_blanks_Bow) #Sig. dispersion F(1,288) = 60.5, p<0.001


## Separating out respective blanks for each sample type ##

#Water July (larvae and blanks from July)
water_July<-subset_samples(ps_rel_abun_Bow, Blank_collection_method=="Water July")
...
#Bedsheet 
bedsheet_ps<-subset_samples(ps_rel_abun_Bow, Blank_collection_method=="Sheet")
...
#### ACWA Streams ####
#Combine filtered experimental samples with blanks
nsamples(ACWA_ps<-subset_samples(sample_ps, Stream_Type=="ACWA Streams")) #55 samples
nsamples(blank_ACWA) #16
exp_bl_ps_ACWA<-merge_phyloseq(blank_ACWA, ACWA_ps)
nsamples(exp_bl_ps_ACWA) #71 (55 samples, 16 blanks)

#Ordinating blanks with experimental samples to see how closely related they are

#PCoA with Bray Curtis distance matrix
ps_rel_abun_ACWA = transform_sample_counts(exp_bl_ps_ACWA, function(x) x/sum(x))

dist_all_ACWA = phyloseq::distance(ps_rel_abun_ACWA, method="bray") #Bray Curtis distance matrix
ordination_all_ACWA = ordinate(ps_rel_abun_ACWA, method="PCoA", distance=dist_all_ACWA) #Ordinate using PCoA

plot_ordination(ps_rel_abun_ACWA, ordination_all_ACWA, shape="Blank_collection_method", color="Sample_Type") +
  theme_classic() +
  theme(strip.background = element_blank())
...
set.seed(1)
ord_NMDS_blanks_ACWA = ordinate(ps_rel_abun_ACWA, method="NMDS", distance=dist_all_ACWA)
ord_NMDS_blanks_ACWA #stress=0.11


blank_nmds_ACWA<-plot_ordination(ps_rel_abun_ACWA, ord_NMDS_blanks_ACWA, color="Sample_Type", shape="Blank_Type") + 
  geom_point()+
  theme_classic() +
  theme(strip.background = element_blank())+
  theme(legend.key.size = unit(1, 'cm'), 
...
blank_nmds_ACWA$Blank_Type <- factor(blank_nmds_ACWA$Blank_Type, levels = c("Tweezer water rinse","Floating emergence trap","Buffer tube","DNA extraction reagent","Nested PCR negative"))
print(blank_nmds_ACWA)

#ggsave(filename="microbiome_analysis/Final analysis/Beta Diversity Graphs/blank and experimental samples ACWA.png", height=10, width=12)

metadata_blanks_ACWA <- data.frame(sample_data(ps_rel_abun_ACWA))

adonis.blanks.ACWA <- adonis2(dist_all_ACWA ~ Sample_Type, data = metadata_blanks_ACWA)
adonis.blanks.ACWA #p<0.001. Significant dissimilarity between blank samples and invertebrate samples
#F(1, 69) = 4.92, P < 0.001

#Testing variance (beta dispersion) differences between samples and blanks:
bd_blanks_ACWA<-betadisper(dist_all_ACWA, metadata_blanks_ACWA$Sample_Type)
anova(bd_blanks_ACWA) #Sig. dispersion F(1,69) = 0.699, p=0.406
```
...
#### Beta Diversity Graphs + PERMANOVA transformed to proportions ####

##                         Normalizing using relative abundance                           ##
...
#ggsave(filename="C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Beta Diversity Graphs\\NMDS Bow River invertebrates.png", height=10, width=20)
...
p1<-plot_ordination(Bow_rel_abun, ord_NMDS_Bow, color = "Family") + 
  ...
  theme(legend.key.size = unit(0.7, 'cm'), 
        ...
        axis.title = element_text(size=18, color="black"),
        strip.text.x = element_text(size=20))+
  stat_ellipse(linetype=1, type="norm", linewidth=0.5)+
  guides(color=guide_legend(nrow=3, ncol=3)) +
  scale_color_manual(values=c("red","#E67E22","#F1C40F","green","#3498DB", "purple", "hotpink", "brown", "black"))+
  labs(color='Invertebrate Taxa')
...
#Family: F = 16.5, R2 = 0.298, P<0.001
...
test.adonis.Bow.trichop #p = 0.022
...
anova(bd_all_fam_Bow_aran) #no beta dispersion p = 0.05
...
test.adonis.Bow.coch #p < 0.001, F(8,56) = 5.32
...
test.adonis.Bow.pmf #p < 0.001, F(7,49)=5.17
...
anova(bd_all_fam_Bow_pmf) #yes beta dispersion p = 0.003.



#### Beta Div. Graphs + PERMANOVA not transformed to proportions (raw reads after quality filtering) ####


Bow_ps<-subset_samples(sample_ps, Stream_Type=="Bow River")

Bow_bray_non_norm = phyloseq::distance(Bow_ps, method="bray") #Bray Curtis distance matrix
ord_Bow_non_norm = ordinate(Bow_ps, method="PCoA", distance=Bow_bray_non_norm) #Ordinate using PCoA

#Scree plot of PCoA
scree <- plot_scree(ord_Bow_non_norm)
scree + theme(axis.text.x=element_text(size=8, color="black"), axis.text.y=element_text(size=18, color="black"), 
        axis.text=element_text(size=18, color="black"), axis.title=element_text(size=18)) 
#axis 1, 2, and 3 account for most variation explained (mostly axis 1)

metadata_Bow_non_norm <- data.frame(sample_data(Bow_ps))

#All together
plot_ordination(Bow_ps, ord_Bow_non_norm, shape="Site", color="Family") + #Plots the ordination
  theme_classic() +
  theme(strip.background = element_blank())

#Axes don't explain a ton of variation so I will try ordinating with an NMDS 


# NMDS 
set.seed(1)
ord_NMDS_Bow_non_norm = ordinate(Bow_ps, method="NMDS", distance=Bow_bray_non_norm)
ord_NMDS_Bow_non_norm #Stress=0.187


#Faceted by invertebrate taxa and life Stage
p<-plot_ordination(Bow_ps, ord_NMDS_Bow_non_norm, color = "Site") + 
  theme_classic() +
  geom_point(size=2)+
  facet_wrap(vars(Stage, Family), scales="free", nrow=1)+
  theme_bw()+
...
#ggsave(filename="C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Beta Diversity Graphs\\NMDS Bow River invertebrates non normalized.png", height=10, width=20)

#Faceted by site
p1<-plot_ordination(Bow_ps, ord_NMDS_Bow_non_norm, color = "Family") + 
  theme_classic() +
  geom_point(size=2)+
  facet_grid(~Site, scales="free")+
  theme_bw()+
...
#ggsave(filename="C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Beta Diversity Graphs\\NMDS across sites Bow non normalized.png", height=10, width=20)


                                ## PERMANOVA by Site and invertebrate taxa ##

test.adonis.Bow.all.non.norm <- adonis2(Bow_bray_non_norm ~ Site*Family, data = metadata_Bow_non_norm)
test.adonis.Bow.all.non.norm
#Site: F = 2.79, R2 = 0.0272, P<0.001
#Family: F = 13.6, R2 = 0.265, P<0.001

bd_all_fam_Bow_non_norm<-betadisper(Bow_bray_non_norm, metadata_Bow_non_norm$Family) 
anova(bd_all_fam_Bow_non_norm) #yes beta disper p < 0.001

bd_all_site_Bow_non_norm<-betadisper(Bow_bray_non_norm, metadata_Bow_non_norm$Site) 
anova(bd_all_site_Bow_non_norm) #no beta disper p = 0.283
...
hydro_Bow<-subset_samples(Bow_ps, Family=="Hydropsychidae")
Bow_bray_hydro_non_norm = phyloseq::distance(hydro_Bow, method="bray") 

metadata_Bow_hydro_non_norm <- data.frame(sample_data(hydro_Bow))

test.adonis.Bow.hydro.non.norm <- adonis2(Bow_bray_hydro_non_norm ~ Site, data = metadata_Bow_hydro_non_norm)
test.adonis.Bow.hydro.non.norm #p<0.001

#Pairwise comparisons
pairwise.adonis2(Bow_bray_hydro_non_norm ~ Site, data = metadata_Bow_hydro_non_norm)
#All sites except CUSHBR - SUN are sig diff

bd_all_fam_Bow_hydro_non_norm<-betadisper(Bow_bray_hydro_non_norm, metadata_Bow_hydro_non_norm$Site) 
anova(bd_all_fam_Bow_hydro_non_norm) #no beta dispersion p = 0.366

#Same conclusion as relative abundance normalized data but the dispersion is a bit better

## Heptageniidae ##

hep_Bow<-subset_samples(Bow_ps, Family=="Heptageniidae")
Bow_bray_hep_non_norm = phyloseq::distance(hep_Bow, method="bray") 

metadata_Bow_hep_non_norm <- data.frame(sample_data(hep_Bow))

test.adonis.Bow.hep.non.norm <- adonis2(Bow_bray_hep_non_norm ~ Site, data = metadata_Bow_hep_non_norm)
test.adonis.Bow.hep.non.norm #p<0.001. 

#Pairwise comparisons
pairwise.adonis2(Bow_bray_hep_non_norm ~ Site, data = metadata_Bow_hep_non_norm)
#All sites except CUSH - SUN diff from each other

bd_all_fam_Bow_hep_non_norm<-betadisper(Bow_bray_hep_non_norm, metadata_Bow_hep_non_norm$Site) 
anova(bd_all_fam_Bow_hep_non_norm) #sig beta dispersion p = 0.0052

#Same conclusion as relative abundance normalized data and dispersion is not any better


## Chironomidae ##

chiro_Bow<-subset_samples(Bow_ps, Family=="Chironomidae")
Bow_bray_chiro_non_norm = phyloseq::distance(chiro_Bow, method="bray") 

metadata_Bow_chiro_non_norm <- data.frame(sample_data(chiro_Bow))

test.adonis.Bow.chiro.non.norm <- adonis2(Bow_bray_chiro_non_norm ~ Site, data = metadata_Bow_chiro_non_norm)
test.adonis.Bow.chiro.non.norm #p=0.112. 

#Pairwise comparisons
pairwise.adonis2(Bow_bray_chiro_non_norm ~ Site, data = metadata_Bow_chiro_non_norm)
#No diff.

bd_all_fam_Bow_chiro_non_norm<-betadisper(Bow_bray_chiro_non_norm, metadata_Bow_chiro_non_norm$Site) 
anova(bd_all_fam_Bow_chiro_non_norm) #no beta dispersion p = 0.566

#Same conclusion as relative abundance normalized data and same amount of dispersion 


## Perlidae ##

perl_Bow<-subset_samples(Bow_ps, Family=="Perlidae")
Bow_bray_perl_non_norm = phyloseq::distance(perl_Bow, method="bray") 

metadata_Bow_perl_non_norm <- data.frame(sample_data(perl_Bow))

test.adonis.Bow.perl.non.norm <- adonis2(Bow_bray_perl_non_norm ~ Site, data = metadata_Bow_perl_non_norm)
test.adonis.Bow.perl.non.norm #p<0.001. 

#Pairwise comparisons
pairwise.adonis2(Bow_bray_perl_non_norm ~ Site, data = metadata_Bow_perl_non_norm)
#All sites were significantly different from each other

bd_all_fam_Bow_perl_non_norm<-betadisper(Bow_bray_perl_non_norm, metadata_Bow_perl_non_norm$Site) 
anova(bd_all_fam_Bow_perl_non_norm) #yes beta dispersion p = 0.046

#Same conclusion as relative abundance normalized data but more dispersion 


## Trichoptera ##

trichop_Bow<-subset_samples(Bow_ps, Family=="Trichoptera")
Bow_bray_trichop_non_norm = phyloseq::distance(trichop_Bow, method="bray") 

metadata_Bow_trichop_non_norm <- data.frame(sample_data(trichop_Bow))

test.adonis.Bow.trichop.non.norm <- adonis2(Bow_bray_trichop_non_norm ~ Site, data = metadata_Bow_trichop_non_norm)
test.adonis.Bow.trichop.non.norm #p = 0.015

#Pairwise comparisons
pairwise.adonis2(Bow_bray_trichop_non_norm ~ Site, data = metadata_Bow_trichop_non_norm)
#CUSHBR - GRVBR, GRVBR - SUN, COCH - GRVBR were sig. different from each other (all upstream sites were diff from GRVBR). 

bd_all_fam_Bow_trichop_non_norm<-betadisper(Bow_bray_trichop_non_norm, metadata_Bow_trichop_non_norm$Site) 
anova(bd_all_fam_Bow_trichop_non_norm) #no beta dispersion p = 0.848
...
ephem_Bow<-subset_samples(Bow_ps, Family=="Ephemeroptera")
Bow_bray_ephem_non_norm = phyloseq::distance(ephem_Bow, method="bray") 

metadata_Bow_ephem_non_norm <- data.frame(sample_data(ephem_Bow))

test.adonis.Bow.ephem.non.norm <- adonis2(Bow_bray_ephem_non_norm ~ Site, data = metadata_Bow_ephem_non_norm)
test.adonis.Bow.ephem.non.norm #p < 0.001 

#Pairwise comparisons
pairwise.adonis2(Bow_bray_ephem_non_norm ~ Site, data = metadata_Bow_ephem_non_norm)
#Sig diff btw sites include:
#COCH - GRVBR
#COCH - CUSHBR
#COCH - PMF
...
bd_all_fam_Bow_ephem_non_norm<-betadisper(Bow_bray_ephem_non_norm, metadata_Bow_ephem_non_norm$Site) 
anova(bd_all_fam_Bow_ephem_non_norm) #yes beta dispersion p = 0.0042
...
## Diptera ##
dip_Bow<-subset_samples(Bow_ps, Family=="Diptera")
Bow_bray_dip_non_norm = phyloseq::distance(dip_Bow, method="bray") 

metadata_Bow_dip_non_norm <- data.frame(sample_data(dip_Bow))

test.adonis.Bow.dip.non.norm <- adonis2(Bow_bray_dip_non_norm ~ Site, data = metadata_Bow_dip_non_norm)
test.adonis.Bow.dip.non.norm #p = 0.102

#Pairwise comparisons
pairwise.adonis2(Bow_bray_dip_non_norm ~ Site, data = metadata_Bow_dip_non_norm)
#sig diff between COCH - CUSHBR and COCH - PMF

bd_all_fam_Bow_dip_non_norm<-betadisper(Bow_bray_dip_non_norm, metadata_Bow_dip_non_norm$Site) 
anova(bd_all_fam_Bow_dip_non_norm) #no beta dispersion p = 0.940
...
aran_Bow<-subset_samples(Bow_ps, Family=="Araneidae")
Bow_bray_aran_non_norm = phyloseq::distance(aran_Bow, method="bray") 

metadata_Bow_aran_non_norm <- data.frame(sample_data(aran_Bow))

test.adonis.Bow.aran.non.norm <- adonis2(Bow_bray_aran_non_norm ~ Site, data = metadata_Bow_aran_non_norm)
test.adonis.Bow.aran.non.norm #p < 0.001 

#Pairwise comparisons
pairwise.adonis2(Bow_bray_aran_non_norm ~ Site, data = metadata_Bow_aran_non_norm)
#Sig diff btw sites include:
#COCH - PMF
#GRVBR - SUN
#GRVBR - PMF
...
bd_all_fam_Bow_aran_non_norm<-betadisper(Bow_bray_aran_non_norm, metadata_Bow_aran_non_norm$Site) 
anova(bd_all_fam_Bow_aran_non_norm) #no beta dispersion p = 0.143

#Same conclusion as relative abundance normalized data with less dispersion 


## Tetragnathidae ##

tetra_Bow<-subset_samples(Bow_ps, Family=="Tetragnathidae")
Bow_bray_tetra_non_norm = phyloseq::distance(tetra_Bow, method="bray") 

metadata_Bow_tetra_non_norm <- data.frame(sample_data(tetra_Bow))

test.adonis.Bow.tetra.non.norm <- adonis2(Bow_bray_tetra_non_norm ~ Site, data = metadata_Bow_tetra_non_norm)
test.adonis.Bow.tetra.non.norm #p = 0.643

#Pairwise comparisons
pairwise.adonis2(Bow_bray_tetra_non_norm ~ Site, data = metadata_Bow_tetra_non_norm)
#No significant differences between any of the sites

bd_all_fam_Bow_tetra_non_norm<-betadisper(Bow_bray_tetra_non_norm, metadata_Bow_tetra_non_norm$Site) 
anova(bd_all_fam_Bow_tetra_non_norm) #no beta dispersion p = 0.665

#Same conclusion as relative abundance normalized data with same amount of dispersion 


## Cochrane ##

coch_Bow<-subset_samples(Bow_ps, Site=="Cochrane")
coch_bray_non_norm = phyloseq::distance(coch_Bow, method="bray") 

metadata_coch_non_norm <- data.frame(sample_data(coch_Bow))

test.adonis.Bow.coch.non.norm <- adonis2(coch_bray_non_norm ~ Family, data = metadata_coch_non_norm)
test.adonis.Bow.coch.non.norm #p < 0.001, F(8,56) = 4.94

#Pairwise comparisons
pairwise.adonis2(coch_bray_non_norm ~ Family, data = metadata_coch_non_norm)
#All diff from each other except
#Dip - Ephem
#Dip - Trichop

bd_all_fam_Bow_coch_non_norm<-betadisper(coch_bray_non_norm, metadata_coch_non_norm$Family) 
anova(bd_all_fam_Bow_coch_non_norm) #yes beta dispersion p < 0.001.

#Same conclusion as relative abundance normalized data with more dispersion 


## Sunalta ##

sun_Bow<-subset_samples(Bow_ps, Site=="Sunalta")
sun_bray_non_norm = phyloseq::distance(sun_Bow, method="bray") 

metadata_sun_non_norm <- data.frame(sample_data(sun_Bow))

test.adonis.Bow.sun.non.norm <- adonis2(sun_bray_non_norm ~ Family, data = metadata_sun_non_norm)
test.adonis.Bow.sun.non.norm #p < 0.001, F(5,40) = 3.90

#Pairwise comparisons
pairwise.adonis2(sun_bray_non_norm ~ Family, data = metadata_sun_non_norm)
#All diff from each other except
#Ephem - Trichop

bd_all_fam_Bow_sun_non_norm<-betadisper(sun_bray_non_norm, metadata_sun_non_norm$Family) 
anova(bd_all_fam_Bow_sun_non_norm) #no beta dispersion p = 0.058.
...
cush_Bow<-subset_samples(Bow_ps, Site=="Cushing Bridge")
cush_bray_non_norm = phyloseq::distance(cush_Bow, method="bray") 

metadata_cush_non_norm <- data.frame(sample_data(cush_Bow))

test.adonis.Bow.cush.non.norm <- adonis2(cush_bray_non_norm ~ Family, data = metadata_cush_non_norm)
test.adonis.Bow.cush.non.norm #p < 0.001, F(8,52) = 4.49

#Pairwise comparisons
pairwise.adonis2(cush_bray_non_norm ~ Family, data = metadata_cush_non_norm)
#All inverts are diff. from each other

bd_all_fam_Bow_cush_non_norm<-betadisper(cush_bray_non_norm, metadata_cush_non_norm$Family) 
anova(bd_all_fam_Bow_cush_non_norm) #yes beta dispersion p = 0.0029.
...
grv_Bow<-subset_samples(Bow_ps, Site=="Graves Bridge")
grv_bray_non_norm = phyloseq::distance(grv_Bow, method="bray") 

metadata_grv_non_norm <- data.frame(sample_data(grv_Bow))

test.adonis.Bow.grv.non.norm <- adonis2(grv_bray_non_norm ~ Family, data = metadata_grv_non_norm)
test.adonis.Bow.grv.non.norm #p < 0.001, F(5,41)=6.83

#Pairwise comparisons
pairwise.adonis2(grv_bray_non_norm ~ Family, data = metadata_grv_non_norm)
#All inverts are diff. from each other

bd_all_fam_Bow_grv_non_norm<-betadisper(grv_bray_non_norm, metadata_grv_non_norm$Family) 
anova(bd_all_fam_Bow_grv_non_norm) #yes beta dispersion p=0.004.
...
pmf_Bow<-subset_samples(Bow_ps, Site=="Policeman Flats")
pmf_bray_non_norm = phyloseq::distance(pmf_Bow, method="bray") 

metadata_pmf_non_norm <- data.frame(sample_data(pmf_Bow))

test.adonis.Bow.pmf.non.norm <- adonis2(pmf_bray_non_norm ~ Family, data = metadata_pmf_non_norm)
test.adonis.Bow.pmf.non.norm #p < 0.001, F(7,50)=4.67

#Pairwise comparisons
pairwise.adonis2(pmf_bray_non_norm ~ Family, data = metadata_pmf_non_norm)
#Most inverts are diff. from each other except
#Dip - Ephem

bd_all_fam_Bow_pmf_non_norm<-betadisper(pmf_bray_non_norm, metadata_pmf_non_norm$Family) 
anova(bd_all_fam_Bow_pmf_non_norm) #yes beta dispersion p < 0.001.


#Same conclusion as relative abundance normalized data with same amount of dispersion


## Overall, we get the same results no matter if we transformed to relative abundance or not. There was no difference in the dispersion estimates overall either. ##
...
abund_TOC # Mantel r=0.0365, p=0.053
...
#Pairwise comparisons
# Comparing invertebrate types - all streams combined
...
# Comparing sites - all inverts combined
...
Compares the mean absolute abundance of bacteria between two groups. The DESeq2 model internally corrects for library size, but if your library size is extremely variable ( > 10x) , normalization such as rareying should be used prior to analysis. Since this dataset's library size ranges from ~2000 reads per sample to 200,000 reads per sample (100x), I will rarefy prior to running DESeq2. It is absolutely critical that the columns of the count matrix and the rows of the column data (information about samples) are in the same order. Rarefied data should be used 

Ignoring excess zeros in zero inflated data can lead to overestimation of dispersion and subsequent reduction in statistical power to detect differentially abundant features. To correct for this I used 'poscounts' which adds a constant value so that a log transformation can be applied. A modified geometric mean is also applied by taking the nth root of the product of the non-zero counts. The prevalence threshold was increased to 3% to reduce the chance of false positives. Adjusted p-values using Benjamini-Hochberg correction were used to determine significance with an alpha of 0.001.
...
ANCOM and ALDEx2 don't work well with small sample size (< 20 samples/group) and can be too conservative. Therefore, DESeq2 is used with rarefied data (even sequencing depth of 2010).

DAtest package can test a bunch of different DA methods and see which one is the best but takes a while to run


## DESeq2 all sites compared to reference - Cochrane/Cushing Bridge/REF - inverts separated
```{r}
#### Hydropsychidae Bow ####

#Exposed (Graves Bridge + Policeman Flats) vs non wastewater exposed sites (Cochrane + Sunalta + Cushing Bridge)

hydro_ps_Bow_rare<-subset_samples(ps_rare, Family=="Hydropsychidae" & Stream_Type=="Bow River")

deseq_exposed_hydro_Bow_rare <- phyloseq_to_deseq2(hydro_ps_Bow_rare, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_hydro_Bow_rare$Exposure <-relevel(deseq_exposed_hydro_Bow_rare$Exposure, "Unexposed")

deseq_exposed_hydro_Bow_rare <- DESeq(deseq_exposed_hydro_Bow_rare, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_hydro_Bow_rare <- results(deseq_exposed_hydro_Bow_rare, cooksCutoff = FALSE)
alpha = 0.001
sigtab_exposed_hydro_Bow_rare = res_deseq_exposed_hydro_Bow_rare[which(res_deseq_exposed_hydro_Bow_rare$padj < alpha), ]
sigtab_exposed_hydro_Bow_rare = cbind(as(sigtab_exposed_hydro_Bow_rare, "data.frame"), as(tax_table(hydro_ps_Bow_rare)[rownames(sigtab_exposed_hydro_Bow_rare), ], "matrix"))

x_exposed_hydro_Bow_rare = tapply(sigtab_exposed_hydro_Bow_rare$log2FoldChange, sigtab_exposed_hydro_Bow_rare$Phylum, function(x) max(x))
x_exposed_hydro_Bow_rare = sort(x_exposed_hydro_Bow_rare, TRUE)
sigtab_exposed_hydro_Bow_rare$Phylum = factor(as.character(sigtab_exposed_hydro_Bow_rare$Phylum), levels=names(x_exposed_hydro_Bow_rare))
# Genus
x_exposed_hydro_Bow_rare = tapply(sigtab_exposed_hydro_Bow_rare$log2FoldChange, sigtab_exposed_hydro_Bow_rare$Genus, function(x) max(x))
x_exposed_hydro_Bow_rare = sort(x_exposed_hydro_Bow_rare, TRUE)
sigtab_exposed_hydro_Bow_rare$Genus = factor(as.character(sigtab_exposed_hydro_Bow_rare$Genus), levels=sort(names(x_exposed_hydro_Bow_rare)))
#43 differentially abundant taxa

diff_abund_hydro_Bow_plot_rare<-ggplot(sigtab_exposed_hydro_Bow_rare, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") + 
  theme_bw()+
...
diff_abund_hydro_Bow_plot_rare

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Exposed vs Unexposed sites hydropsychidae larvae Bow rarefied.png", height=24, width=24)


#Cushing Bridge compared to Graves Bridge and Policeman Flats

hydro_ps_Bow_rare<-subset_samples(ps_rare, Family=="Hydropsychidae" & Stream_Type=="Bow River")

hydro_ps_Bow_subset_rare<-subset_samples(hydro_ps_Bow_rare, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_hydro_Bow_subset_rare <- phyloseq_to_deseq2(hydro_ps_Bow_subset_rare, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_hydro_Bow_subset_rare$Exposure <-relevel(deseq_exposed_hydro_Bow_subset_rare$Exposure, "Unexposed")

deseq_exposed_hydro_Bow_subset_rare <- DESeq(deseq_exposed_hydro_Bow_subset_rare, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_hydro_Bow_subset_rare <- results(deseq_exposed_hydro_Bow_subset_rare, cooksCutoff = FALSE)
alpha = 0.001
sigtab_exposed_hydro_Bow_subset_rare = res_deseq_exposed_hydro_Bow_subset_rare[which(res_deseq_exposed_hydro_Bow_subset_rare$padj < alpha), ]

sigtab_exposed_hydro_Bow_subset_rare = cbind(as(sigtab_exposed_hydro_Bow_subset_rare, "data.frame"), as(tax_table(hydro_ps_Bow_subset_rare)[rownames(sigtab_exposed_hydro_Bow_subset_rare), ], "matrix"))

x_exposed_hydro_Bow_subset_rare = tapply(sigtab_exposed_hydro_Bow_subset_rare$log2FoldChange, sigtab_exposed_hydro_Bow_subset_rare$Phylum, function(x) max(x))
x_exposed_hydro_Bow_subset_rare = sort(x_exposed_hydro_Bow_subset_rare, TRUE)
sigtab_exposed_hydro_Bow_subset_rare$Phylum = factor(as.character(sigtab_exposed_hydro_Bow_subset_rare$Phylum), levels=names(x_exposed_hydro_Bow_subset_rare))
# Genus
x_exposed_hydro_Bow_subset_rare = tapply(sigtab_exposed_hydro_Bow_subset_rare$log2FoldChange, sigtab_exposed_hydro_Bow_subset_rare$Genus, function(x) max(x))
x_exposed_hydro_Bow_subset_rare = sort(x_exposed_hydro_Bow_subset_rare, TRUE)
sigtab_exposed_hydro_Bow_subset_rare$Genus = factor(as.character(sigtab_exposed_hydro_Bow_subset_rare$Genus), levels=sort(names(x_exposed_hydro_Bow_subset_rare)))
#7 differentially abundant taxa

sigtab_exposed_hydro_Bow_subset_rare$Genus <- factor(sigtab_exposed_hydro_Bow_subset_rare$Genus, levels=c("Anaerocella", "Mucinivorans", "Rhodobacter", "NA"), labels=c("Anaerocella", "Mucinivorans", "Rhodobacter", "Unidentified"))

sigtab_exposed_hydro_Bow_subset_rare$Genus[is.na(sigtab_exposed_hydro_Bow_subset_rare$Genus)] <- "Unidentified"

diff_abund_hydro_Bow_plot_subset_rare<-ggplot(sigtab_exposed_hydro_Bow_subset_rare, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
          theme(legend.title = element_text(size=34, color="black"), legend.position="bottom")+
...
diff_abund_hydro_Bow_plot_subset_rare

## Using Cushing Bridge as a reference because it captures stormwater inputs ##

#CUSHBR vs COCH

hydro_cushbr_coch_rare<-subset_samples(hydro_ps_Bow_rare, Site=="Cochrane" | Site=="Cushing Bridge")

deseq_cushbr_coch_hydro_rare <- phyloseq_to_deseq2(hydro_cushbr_coch_rare, ~ Site)

deseq_cushbr_coch_hydro_rare <- DESeq(deseq_cushbr_coch_hydro_rare, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_coch_hydro_rare <- results(deseq_cushbr_coch_hydro_rare, cooksCutoff = FALSE)
alpha = 0.001
sigtab_cushbr_coch_hydro_rare = res_deseq_cushbr_coch_hydro_rare[which(res_deseq_cushbr_coch_hydro_rare$padj < alpha), ]
sigtab_cushbr_coch_hydro_rare = cbind(as(sigtab_cushbr_coch_hydro_rare, "data.frame"), as(tax_table(hydro_cushbr_coch_rare)[rownames(sigtab_cushbr_coch_hydro_rare), ], "matrix"))

x_cushbr_coch_hydro_rare = tapply(sigtab_cushbr_coch_hydro_rare$log2FoldChange, sigtab_cushbr_coch_hydro_rare$Phylum, function(x) max(x))
x_cushbr_coch_hydro_rare = sort(x_cushbr_coch_hydro_rare, TRUE)
sigtab_cushbr_coch_hydro_rare$Phylum = factor(as.character(sigtab_cushbr_coch_hydro_rare$Phylum), levels=names(x_cushbr_coch_hydro_rare))
# Genus
x_cushbr_coch_hydro_rare = tapply(sigtab_cushbr_coch_hydro_rare$log2FoldChange, sigtab_cushbr_coch_hydro_rare$Genus, function(x) max(x))
x_cushbr_coch_hydro_rare = sort(x_cushbr_coch_hydro_rare, TRUE)
sigtab_cushbr_coch_hydro_rare$Genus = factor(as.character(sigtab_cushbr_coch_hydro_rare$Genus), levels=sort(names(x_cushbr_coch_hydro_rare)))
#1 observations

sigtab_cushbr_coch_hydro_rare$Genus <- factor(sigtab_cushbr_coch_hydro_rare$Genus, levels=c("Mucinivorans", "NA"), labels=c("Mucinivorans", "Unidentified"))

sigtab_cushbr_coch_hydro_rare$Genus[is.na(sigtab_cushbr_coch_hydro_rare$Genus)] <- "Unidentified"

diff_abund_hydro_cushbr_coch_plot_rare<-ggplot(sigtab_cushbr_coch_hydro_rare, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
...
diff_abund_hydro_cushbr_coch_plot_rare

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane hydropsychidae larvae rarefied.png", height=20, width=22)


#CUSHBR vs SUN

hydro_cushbr_sun_rare<-subset_samples(hydro_ps_Bow_rare, Site=="Sunalta" | Site=="Cushing Bridge")

deseq_cushbr_sun_hydro_rare <- phyloseq_to_deseq2(hydro_cushbr_sun_rare, ~ Site)

deseq_cushbr_sun_hydro_rare <- DESeq(deseq_cushbr_sun_hydro_rare, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_sun_hydro_rare <- results(deseq_cushbr_sun_hydro_rare, cooksCutoff = FALSE)
res_deseq_cushbr_sun_hydro_rare
alpha = 0.001
sigtab_cushbr_sun_hydro_rare = res_deseq_cushbr_sun_hydro_rare[which(res_deseq_cushbr_sun_hydro_rare$padj < alpha), ]
sigtab_cushbr_sun_hydro_rare = cbind(as(sigtab_cushbr_sun_hydro_rare, "data.frame"), as(tax_table(hydro_cushbr_sun_rare)[rownames(sigtab_cushbr_sun_hydro_rare), ], "matrix"))
#0


#CUSHBR vs GRVBR

hydro_cushbr_grvbr_rare<-subset_samples(hydro_ps_Bow_rare, Site=="Graves Bridge" | Site=="Cushing Bridge")

deseq_cushbr_grvbr_hydro_rare <- phyloseq_to_deseq2(hydro_cushbr_grvbr_rare, ~ Site)

deseq_cushbr_grvbr_hydro_rare$Site <-relevel(deseq_cushbr_grvbr_hydro_rare$Site, "Graves Bridge") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_grvbr_hydro_rare <- DESeq(deseq_cushbr_grvbr_hydro_rare, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_grvbr_hydro_rare <- results(deseq_cushbr_grvbr_hydro_rare, cooksCutoff = FALSE)

alpha = 0.001
sigtab_cushbr_grvbr_hydro_rare = res_deseq_cushbr_grvbr_hydro_rare[which(res_deseq_cushbr_grvbr_hydro_rare$padj < alpha), ]
sigtab_cushbr_grvbr_hydro_rare = cbind(as(sigtab_cushbr_grvbr_hydro_rare, "data.frame"), as(tax_table(hydro_cushbr_grvbr_rare)[rownames(sigtab_cushbr_grvbr_hydro_rare), ], "matrix"))
#0


#CUSHBR - PMF

hydro_cushbr_pmf_rare<-subset_samples(hydro_ps_Bow_rare, Site=="Policeman Flats" | Site=="Cushing Bridge")

deseq_cushbr_pmf_hydro_rare <- phyloseq_to_deseq2(hydro_cushbr_pmf_rare, ~ Site)

deseq_cushbr_pmf_hydro_rare$Site <-relevel(deseq_cushbr_pmf_hydro_rare$Site, "Policeman Flats") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_pmf_hydro_rare <- DESeq(deseq_cushbr_pmf_hydro_rare, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_pmf_hydro_rare <- results(deseq_cushbr_pmf_hydro_rare, cooksCutoff = FALSE)

alpha = 0.001
sigtab_cushbr_pmf_hydro_rare = res_deseq_cushbr_pmf_hydro_rare[which(res_deseq_cushbr_pmf_hydro_rare$padj < alpha), ]
sigtab_cushbr_pmf_hydro_rare = cbind(as(sigtab_cushbr_pmf_hydro_rare, "data.frame"), as(tax_table(hydro_cushbr_pmf_rare)[rownames(sigtab_cushbr_pmf_hydro_rare), ], "matrix"))

x_cushbr_pmf_hydro_rare = tapply(sigtab_cushbr_pmf_hydro_rare$log2FoldChange, sigtab_cushbr_pmf_hydro_rare$Phylum, function(x) max(x))
x_cushbr_pmf_hydro_rare = sort(x_cushbr_pmf_hydro_rare, TRUE)
sigtab_cushbr_pmf_hydro_rare$Phylum = factor(as.character(sigtab_cushbr_pmf_hydro_rare$Phylum), levels=names(x_cushbr_pmf_hydro_rare))
# Genus
x_cushbr_pmf_hydro_rare = tapply(sigtab_cushbr_pmf_hydro_rare$log2FoldChange, sigtab_cushbr_pmf_hydro_rare$Genus, function(x) max(x))
x_cushbr_pmf_hydro_rare = sort(x_cushbr_pmf_hydro_rare, TRUE)
sigtab_cushbr_pmf_hydro_rare$Genus = factor(as.character(sigtab_cushbr_pmf_hydro_rare$Genus), levels=sort(names(x_cushbr_pmf_hydro_rare)))
#32 observations


sigtab_cushbr_pmf_hydro_rare$Genus <- factor(sigtab_cushbr_pmf_hydro_rare$Genus, levels=c("AAP99", "Anaerocella", "Defluviimonas", "Ellin6067", "M2PB4-61 termite group", "Mucinivorans", "Mucispirillum", "Nitrosomonas", "Rhizorhapis", "Rhodobacter", "Rhodoferax", "Rickettsia", "Sphingopyxis", "Sulfurospirillum", "NA"), labels=c("AAP99", "Anaerocella", "Defluviimonas", "Ellin6067", "M2PB4-61 termite group", "Mucinivorans", "Mucispirillum", "Nitrosomonas", "Rhizorhapis", "Rhodobacter", "Rhodoferax", "Rickettsia", "Sphingopyxis", "Sulfurospirillum", "Unidentified"))

sigtab_cushbr_pmf_hydro_rare$Genus[is.na(sigtab_cushbr_pmf_hydro_rare$Genus)] <- "Unidentified"


diff_abund_hydro_cushbr_pmf_plot_rare<-ggplot(sigtab_cushbr_pmf_hydro_rare, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_hydro_cushbr_pmf_plot_rare

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats hydropsychidae larvae rarefied.png", height=20, width=22)
...
diff_abund_trichop_plot<-ggplot(sigtab_exposed_trichop, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=28, face="italic", color="black")) + theme(axis.text=element_text(size=24, color="black"), axis.title.x=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
          theme(legend.title = element_text(size=24, color="black"), legend.position="bottom")+
  guides(nrow=8)+
  rremove("grid")+
...
#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Exposed vs Unexposed sites Trichoptera adults Bow.png", height=18, width=18)

#No color legend

ggplot(sigtab_exposed_trichop, aes(x=log2FoldChange, y=Genus))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Exposed vs Unexposed sites Trichoptera adults Bow no legend.png", height=18, width=18)


#Exposed vs non wastewater exposed sites (Cushing Bridge compared to both downstream sites)

trichop_ps_Bow_subset<-subset_samples(trichop_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_trichop_Bow_subset <- phyloseq_to_deseq2(trichop_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_trichop_Bow_subset$Exposure <-relevel(deseq_exposed_trichop_Bow_subset$Exposure, "Unexposed")

deseq_exposed_trichop_Bow_subset <- DESeq(deseq_exposed_trichop_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_trichop_Bow_subset <- results(deseq_exposed_trichop_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_trichop_Bow_subset 
alpha = 0.001
sigtab_exposed_trichop_Bow_subset = res_deseq_exposed_trichop_Bow_subset[which(res_deseq_exposed_trichop_Bow_subset$padj < alpha), ]
sigtab_exposed_trichop_Bow_subset = cbind(as(sigtab_exposed_trichop_Bow_subset, "data.frame"), as(tax_table(trichop_ps_Bow_subset)[rownames(sigtab_exposed_trichop_Bow_subset), ], "matrix"))

x_exposed_trichop_Bow_subset = tapply(sigtab_exposed_trichop_Bow_subset$log2FoldChange, sigtab_exposed_trichop_Bow_subset$Phylum, function(x) max(x))
x_exposed_trichop_Bow_subset = sort(x_exposed_trichop_Bow_subset, TRUE)
sigtab_exposed_trichop_Bow_subset$Phylum = factor(as.character(sigtab_exposed_trichop_Bow_subset$Phylum), levels=names(x_exposed_trichop_Bow_subset))
# Genus
x_exposed_trichop_Bow_subset = tapply(sigtab_exposed_trichop_Bow_subset$log2FoldChange, sigtab_exposed_trichop_Bow_subset$Genus, function(x) max(x))
x_exposed_trichop_Bow_subset = sort(x_exposed_trichop_Bow_subset, TRUE)
sigtab_exposed_trichop_Bow_subset$Genus = factor(as.character(sigtab_exposed_trichop_Bow_subset$Genus), levels=sort(names(x_exposed_trichop_Bow_subset)))
#44 differentially abundant taxa

sigtab_exposed_trichop_Bow_subset$Genus <- factor(sigtab_exposed_trichop_Bow_subset$Genus, levels=c("Agathobacter", "Arthrobacter", "Aureimonas", "Bacillus", "Clavibacter", "Clostridium sensu stricto 1", "Devosia", "Erythrobacter", "Flavobacterium", "Hydrogenophaga", "Hymenobacter", "Ideonella", "Jannaschia", "Limnobacter", "Macrococcus", "Massilia", "Methylobacterium-Methylorubrum", "Microvirga", "Ottawia", "Pedobacter", "Phreatobacter", "Ramlibacter", "Rhodobacter", "Rhodoferax", "Rhodoplanes", "Rubellimicrobium", "Serratia", "Skermanella", "Sphaerotilus", "Sphingorhabdus", "Staphylococcus", "Tabrizicola", "Tychonema CCAP 1459-11B", "NA"), labels=c("Agathobacter", "Arthrobacter", "Aureimonas", "Bacillus", "Clavibacter", "Clostridium sensu stricto 1", "Devosia", "Erythrobacter", "Flavobacterium", "Hydrogenophaga", "Hymenobacter", "Ideonella", "Jannaschia", "Limnobacter", "Macrococcus", "Massilia", "Methylobacterium-Methylorubrum", "Microvirga", "Ottawia", "Pedobacter", "Phreatobacter", "Ramlibacter", "Rhodobacter", "Rhodoferax", "Rhodoplanes", "Rubellimicrobium", "Serratia", "Skermanella", "Sphaerotilus", "Sphingorhabdus", "Staphylococcus", "Tabrizicola", "Tychonema CCAP 1459-11B","Unidentified"))

sigtab_exposed_trichop_Bow_subset$Genus[is.na(sigtab_exposed_trichop_Bow_subset$Genus)] <- "Unidentified"

diff_abund_trichop_Bow_plot_subset<-ggplot(sigtab_exposed_trichop_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_trichop_Bow_plot_subset


#COCH - SUN

hydro_sun_coch<-subset_samples(hydro_ps_Bow, Site=="Cochrane" | Site=="Sunalta")
...
## Using Cushing Bridge as a reference instead of Cochrane because it captures stormwater inputs ##


#CUSHBR - COCH

trichop_cushbr_coch<-subset_samples(trichop_ps, Site=="Cochrane" | Site=="Cushing Bridge")

deseq_cushbr_coch_trichop <- phyloseq_to_deseq2(trichop_cushbr_coch, ~ Site)

deseq_cushbr_coch_trichop <- DESeq(deseq_cushbr_coch_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_coch_trichop <- results(deseq_cushbr_coch_trichop, cooksCutoff = FALSE)
res_deseq_cushbr_coch_trichop
alpha = 0.001
sigtab_cushbr_coch_trichop = res_deseq_cushbr_coch_trichop[which(res_deseq_cushbr_coch_trichop$padj < alpha), ]
sigtab_cushbr_coch_trichop = cbind(as(sigtab_cushbr_coch_trichop, "data.frame"), as(tax_table(trichop_cushbr_coch)[rownames(sigtab_cushbr_coch_trichop), ], "matrix"))

x_cushbr_coch_trichop = tapply(sigtab_cushbr_coch_trichop$log2FoldChange, sigtab_cushbr_coch_trichop$Phylum, function(x) max(x))
x_cushbr_coch_trichop = sort(x_cushbr_coch_trichop, TRUE)
sigtab_cushbr_coch_trichop$Phylum = factor(as.character(sigtab_cushbr_coch_trichop$Phylum), levels=names(x_cushbr_coch_trichop))
# Genus
x_cushbr_coch_trichop = tapply(sigtab_cushbr_coch_trichop$log2FoldChange, sigtab_cushbr_coch_trichop$Genus, function(x) max(x))
x_cushbr_coch_trichop = sort(x_cushbr_coch_trichop, TRUE)
sigtab_cushbr_coch_trichop$Genus = factor(as.character(sigtab_cushbr_coch_trichop$Genus), levels=sort(names(x_cushbr_coch_trichop)))
#9 observations

diff_abund_trichop_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_trichop, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_trichop_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane adult Trichoptera larvae.png", height=20, width=22)


#CUSHBR - SUN

trichop_cushbr_sun<-subset_samples(trichop_ps, Site=="Sunalta" | Site=="Cushing Bridge")

deseq_cushbr_sun_trichop <- phyloseq_to_deseq2(trichop_cushbr_sun, ~ Site)

deseq_cushbr_sun_trichop <- DESeq(deseq_cushbr_sun_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_sun_trichop <- results(deseq_cushbr_sun_trichop, cooksCutoff = FALSE)
res_deseq_cushbr_sun_trichop
alpha = 0.001
sigtab_cushbr_sun_trichop = res_deseq_cushbr_sun_trichop[which(res_deseq_cushbr_sun_trichop$padj < alpha), ]
sigtab_cushbr_sun_trichop = cbind(as(sigtab_cushbr_sun_trichop, "data.frame"), as(tax_table(trichop_cushbr_sun)[rownames(sigtab_cushbr_sun_trichop), ], "matrix"))

x_cushbr_sun_trichop = tapply(sigtab_cushbr_sun_trichop$log2FoldChange, sigtab_cushbr_sun_trichop$Phylum, function(x) max(x))
x_cushbr_sun_trichop = sort(x_cushbr_sun_trichop, TRUE)
sigtab_cushbr_sun_trichop$Phylum = factor(as.character(sigtab_cushbr_sun_trichop$Phylum), levels=names(x_cushbr_sun_trichop))
# Genus
x_cushbr_sun_trichop = tapply(sigtab_cushbr_sun_trichop$log2FoldChange, sigtab_cushbr_sun_trichop$Genus, function(x) max(x))
x_cushbr_sun_trichop = sort(x_cushbr_sun_trichop, TRUE)
sigtab_cushbr_sun_trichop$Genus = factor(as.character(sigtab_cushbr_sun_trichop$Genus), levels=sort(names(x_cushbr_sun_trichop)))
#5 observations

diff_abund_trichop_cushbr_sun_plot<-ggplot(sigtab_cushbr_sun_trichop, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_trichop_cushbr_sun_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Sunalta adult Trichoptera.png", height=20, width=22)


#CUSHBR - GRVBR

trichop_cushbr_grvbr<-subset_samples(trichop_ps, Site=="Graves Bridge" | Site=="Cushing Bridge")

deseq_cushbr_grvbr_trichop <- phyloseq_to_deseq2(trichop_cushbr_grvbr, ~ Site)

deseq_cushbr_grvbr_trichop$Site <-relevel(deseq_cushbr_grvbr_trichop$Site, "Graves Bridge") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_grvbr_trichop <- DESeq(deseq_cushbr_grvbr_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_grvbr_trichop <- results(deseq_cushbr_grvbr_trichop, cooksCutoff = FALSE)
res_deseq_cushbr_grvbr_trichop
alpha = 0.001
sigtab_cushbr_grvbr_trichop = res_deseq_cushbr_grvbr_trichop[which(res_deseq_cushbr_grvbr_trichop$padj < alpha), ]
sigtab_cushbr_grvbr_trichop = cbind(as(sigtab_cushbr_grvbr_trichop, "data.frame"), as(tax_table(trichop_cushbr_grvbr)[rownames(sigtab_cushbr_grvbr_trichop), ], "matrix"))

x_cushbr_grvbr_trichop = tapply(sigtab_cushbr_grvbr_trichop$log2FoldChange, sigtab_cushbr_grvbr_trichop$Phylum, function(x) max(x))
x_cushbr_grvbr_trichop = sort(x_cushbr_grvbr_trichop, TRUE)
sigtab_cushbr_grvbr_trichop$Phylum = factor(as.character(sigtab_cushbr_grvbr_trichop$Phylum), levels=names(x_cushbr_grvbr_trichop))
# Genus
x_cushbr_grvbr_trichop = tapply(sigtab_cushbr_grvbr_trichop$log2FoldChange, sigtab_cushbr_grvbr_trichop$Genus, function(x) max(x))
x_cushbr_grvbr_trichop = sort(x_cushbr_grvbr_trichop, TRUE)
sigtab_cushbr_grvbr_trichop$Genus = factor(as.character(sigtab_cushbr_grvbr_trichop$Genus), levels=sort(names(x_cushbr_grvbr_trichop)))
#44 observations

diff_abund_trichop_cushbr_grvbr_plot<-ggplot(sigtab_cushbr_grvbr_trichop, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_trichop_cushbr_grvbr_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Graves Bridge adult Trichoptera.png", height=20, width=22)
...
diff_abund_hep_plot<-ggplot(sigtab_exposed_hep, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=28, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
          theme(legend.title = element_text(size=24, color="black"), legend.position="bottom")+
  guides(nrow=8)+
  rremove("grid")+
...
hep_ps_Bow_subset<-subset_samples(hep_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_hep_Bow_subset <- phyloseq_to_deseq2(hep_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_hep_Bow_subset$Exposure <-relevel(deseq_exposed_hep_Bow_subset$Exposure, "Unexposed")

deseq_exposed_hep_Bow_subset <- DESeq(deseq_exposed_hep_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_hep_Bow_subset <- results(deseq_exposed_hep_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_hep_Bow_subset 
alpha = 0.001
sigtab_exposed_hep_Bow_subset = res_deseq_exposed_hep_Bow_subset[which(res_deseq_exposed_hep_Bow_subset$padj < alpha), ]
sigtab_exposed_hep_Bow_subset = cbind(as(sigtab_exposed_hep_Bow_subset, "data.frame"), as(tax_table(hep_ps_Bow_subset)[rownames(sigtab_exposed_hep_Bow_subset), ], "matrix"))

x_exposed_hep_Bow_subset = tapply(sigtab_exposed_hep_Bow_subset$log2FoldChange, sigtab_exposed_hep_Bow_subset$Phylum, function(x) max(x))
x_exposed_hep_Bow_subset = sort(x_exposed_hep_Bow_subset, TRUE)
sigtab_exposed_hep_Bow_subset$Phylum = factor(as.character(sigtab_exposed_hep_Bow_subset$Phylum), levels=names(x_exposed_hep_Bow_subset))
# Genus
x_exposed_hep_Bow_subset = tapply(sigtab_exposed_hep_Bow_subset$log2FoldChange, sigtab_exposed_hep_Bow_subset$Genus, function(x) max(x))
x_exposed_hep_Bow_subset = sort(x_exposed_hep_Bow_subset, TRUE)
sigtab_exposed_hep_Bow_subset$Genus = factor(as.character(sigtab_exposed_hep_Bow_subset$Genus), levels=sort(names(x_exposed_hep_Bow_subset)))
#38 differentially abundant taxa

sigtab_exposed_hep_Bow_subset$Genus <- factor(sigtab_exposed_hep_Bow_subset$Genus, levels=c("Arcicella", "Calothrix KVSF5", "Candidatus Bacilloplasma", "Candidatus Symbiobacter", "Cereibacter", "Emticicia", "Flavobacterium", "Fluviicola", "Lacihabitans", "Limnohabitans", "Porphyrobacter", "Pseudorhodobacter", "Rhizorhapis", "Sphaerotilus", "Sphingorhabdus", "NA"), labels=c("Arcicella", "Calothrix KVSF5", "Candidatus Bacilloplasma", "Candidatus Symbiobacter", "Cereibacter", "Emticicia", "Flavobacterium", "Fluviicola", "Lacihabitans", "Limnohabitans", "Porphyrobacter", "Pseudorhodobacter", "Rhizorhapis", "Sphaerotilus", "Sphingorhabdus","Unidentified"))

sigtab_exposed_hep_Bow_subset$Genus[is.na(sigtab_exposed_hep_Bow_subset$Genus)] <- "Unidentified"

diff_abund_hep_Bow_plot_subset<-ggplot(sigtab_exposed_hep_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_hep_Bow_plot_subset
...
hep_cushbr_coch<-subset_samples(hep_ps, Site=="Cochrane" | Site=="Cushing Bridge")

deseq_cushbr_coch_hep <- phyloseq_to_deseq2(hep_cushbr_coch, ~ Site)

deseq_cushbr_coch_hep <- DESeq(deseq_cushbr_coch_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_coch_hep <- results(deseq_cushbr_coch_hep, cooksCutoff = FALSE)
res_deseq_cushbr_coch_hep
alpha = 0.001
sigtab_cushbr_coch_hep = res_deseq_cushbr_coch_hep[which(res_deseq_cushbr_coch_hep$padj < alpha), ]
sigtab_cushbr_coch_hep = cbind(as(sigtab_cushbr_coch_hep, "data.frame"), as(tax_table(hep_cushbr_coch)[rownames(sigtab_cushbr_coch_hep), ], "matrix"))

x_cushbr_coch_hep = tapply(sigtab_cushbr_coch_hep$log2FoldChange, sigtab_cushbr_coch_hep$Phylum, function(x) max(x))
x_cushbr_coch_hep = sort(x_cushbr_coch_hep, TRUE)
sigtab_cushbr_coch_hep$Phylum = factor(as.character(sigtab_cushbr_coch_hep$Phylum), levels=names(x_cushbr_coch_hep))
# Genus
x_cushbr_coch_hep = tapply(sigtab_cushbr_coch_hep$log2FoldChange, sigtab_cushbr_coch_hep$Genus, function(x) max(x))
x_cushbr_coch_hep = sort(x_cushbr_coch_hep, TRUE)
sigtab_cushbr_coch_hep$Genus = factor(as.character(sigtab_cushbr_coch_hep$Genus), levels=sort(names(x_cushbr_coch_hep)))
#2 observations

diff_abund_hep_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_hep, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_hep_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane heptageniidae larvae.png", height=20, width=22)


#CUSHBR - SUN

hep_cushbr_sun<-subset_samples(hep_ps, Site=="Sunalta" | Site=="Cushing Bridge")

deseq_cushbr_sun_hep <- phyloseq_to_deseq2(hep_cushbr_sun, ~ Site)

deseq_cushbr_sun_hep <- DESeq(deseq_cushbr_sun_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_sun_hep <- results(deseq_cushbr_sun_hep, cooksCutoff = FALSE)
res_deseq_cushbr_sun_hep
alpha = 0.001
sigtab_cushbr_sun_hep = res_deseq_cushbr_sun_hep[which(res_deseq_cushbr_sun_hep$padj < alpha), ]
sigtab_cushbr_sun_hep = cbind(as(sigtab_cushbr_sun_hep, "data.frame"), as(tax_table(hep_cushbr_sun)[rownames(sigtab_cushbr_sun_hep), ], "matrix"))

x_cushbr_sun_hep = tapply(sigtab_cushbr_sun_hep$log2FoldChange, sigtab_cushbr_sun_hep$Phylum, function(x) max(x))
x_cushbr_sun_hep = sort(x_cushbr_sun_hep, TRUE)
sigtab_cushbr_sun_hep$Phylum = factor(as.character(sigtab_cushbr_sun_hep$Phylum), levels=names(x_cushbr_sun_hep))
# Genus
x_cushbr_sun_hep = tapply(sigtab_cushbr_sun_hep$log2FoldChange, sigtab_cushbr_sun_hep$Genus, function(x) max(x))
x_cushbr_sun_hep = sort(x_cushbr_sun_hep, TRUE)
sigtab_cushbr_sun_hep$Genus = factor(as.character(sigtab_cushbr_sun_hep$Genus), levels=sort(names(x_cushbr_sun_hep)))
#7 observations

diff_abund_hep_cushbr_sun_plot<-ggplot(sigtab_cushbr_sun_hep, aes(x=log2FoldChange, y=Genus, fill=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_hep_cushbr_sun_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Sunalta heptagenidae larvae.png", height=20, width=22)


#CUSHBR - GRVBR

hep_cushbr_grvbr<-subset_samples(hep_ps, Site=="Graves Bridge" | Site=="Cushing Bridge")

deseq_cushbr_grvbr_hep <- phyloseq_to_deseq2(hep_cushbr_grvbr, ~ Site)

deseq_cushbr_grvbr_hep$Site <-relevel(deseq_cushbr_grvbr_hep$Site, "Graves Bridge") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_grvbr_hep <- DESeq(deseq_cushbr_grvbr_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_grvbr_hep <- results(deseq_cushbr_grvbr_hep, cooksCutoff = FALSE)
res_deseq_cushbr_grvbr_hep
alpha = 0.001
sigtab_cushbr_grvbr_hep = res_deseq_cushbr_grvbr_hep[which(res_deseq_cushbr_grvbr_hep$padj < alpha), ]
sigtab_cushbr_grvbr_hep = cbind(as(sigtab_cushbr_grvbr_hep, "data.frame"), as(tax_table(hep_cushbr_grvbr)[rownames(sigtab_cushbr_grvbr_hep), ], "matrix"))

x_cushbr_grvbr_hep = tapply(sigtab_cushbr_grvbr_hep$log2FoldChange, sigtab_cushbr_grvbr_hep$Phylum, function(x) max(x))
x_cushbr_grvbr_hep = sort(x_cushbr_grvbr_hep, TRUE)
sigtab_cushbr_grvbr_hep$Phylum = factor(as.character(sigtab_cushbr_grvbr_hep$Phylum), levels=names(x_cushbr_grvbr_hep))
# Genus
x_cushbr_grvbr_hep = tapply(sigtab_cushbr_grvbr_hep$log2FoldChange, sigtab_cushbr_grvbr_hep$Genus, function(x) max(x))
x_cushbr_grvbr_hep = sort(x_cushbr_grvbr_hep, TRUE)
sigtab_cushbr_grvbr_hep$Genus = factor(as.character(sigtab_cushbr_grvbr_hep$Genus), levels=sort(names(x_cushbr_grvbr_hep)))
#4 observations

diff_abund_hep_cushbr_grvbr_plot<-ggplot(sigtab_cushbr_grvbr_hep, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_hep_cushbr_grvbr_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Graves Bridge heptageniidae larvae.png", height=20, width=22)


#CUSHBR - PMF

hep_cushbr_pmf<-subset_samples(hep_ps, Site=="Policeman Flats" | Site=="Cushing Bridge")

deseq_cushbr_pmf_hep <- phyloseq_to_deseq2(hep_cushbr_pmf, ~ Site)

deseq_cushbr_pmf_hep$Site <-relevel(deseq_cushbr_pmf_hep$Site, "Policeman Flats") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_pmf_hep <- DESeq(deseq_cushbr_pmf_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_pmf_hep <- results(deseq_cushbr_pmf_hep, cooksCutoff = FALSE)
res_deseq_cushbr_pmf_hep
alpha = 0.001
sigtab_cushbr_pmf_hep = res_deseq_cushbr_pmf_hep[which(res_deseq_cushbr_pmf_hep$padj < alpha), ]
sigtab_cushbr_pmf_hep = cbind(as(sigtab_cushbr_pmf_hep, "data.frame"), as(tax_table(hep_cushbr_pmf)[rownames(sigtab_cushbr_pmf_hep), ], "matrix"))

x_cushbr_pmf_hep = tapply(sigtab_cushbr_pmf_hep$log2FoldChange, sigtab_cushbr_pmf_hep$Phylum, function(x) max(x))
x_cushbr_pmf_hep = sort(x_cushbr_pmf_hep, TRUE)
sigtab_cushbr_pmf_hep$Phylum = factor(as.character(sigtab_cushbr_pmf_hep$Phylum), levels=names(x_cushbr_pmf_hep))
# Genus
x_cushbr_pmf_hep = tapply(sigtab_cushbr_pmf_hep$log2FoldChange, sigtab_cushbr_pmf_hep$Genus, function(x) max(x))
x_cushbr_pmf_hep = sort(x_cushbr_pmf_hep, TRUE)
sigtab_cushbr_pmf_hep$Genus = factor(as.character(sigtab_cushbr_pmf_hep$Genus), levels=sort(names(x_cushbr_pmf_hep)))
#40 observations

diff_abund_hep_cushbr_pmf_plot<-ggplot(sigtab_cushbr_pmf_hep, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_hep_cushbr_pmf_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats heptageniidae larvae.png", height=20, width=22)
...
ephem_ps_Bow_subset<-subset_samples(ephem_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_ephem_Bow_subset <- phyloseq_to_deseq2(ephem_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_ephem_Bow_subset$Exposure <-relevel(deseq_exposed_ephem_Bow_subset$Exposure, "Unexposed")

deseq_exposed_ephem_Bow_subset <- DESeq(deseq_exposed_ephem_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_ephem_Bow_subset <- results(deseq_exposed_ephem_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_ephem_Bow_subset 
alpha = 0.001
sigtab_exposed_ephem_Bow_subset = res_deseq_exposed_ephem_Bow_subset[which(res_deseq_exposed_ephem_Bow_subset$padj < alpha), ]
sigtab_exposed_ephem_Bow_subset = cbind(as(sigtab_exposed_ephem_Bow_subset, "data.frame"), as(tax_table(ephem_ps_Bow_subset)[rownames(sigtab_exposed_ephem_Bow_subset), ], "matrix"))

x_exposed_ephem_Bow_subset = tapply(sigtab_exposed_ephem_Bow_subset$log2FoldChange, sigtab_exposed_ephem_Bow_subset$Phylum, function(x) max(x))
x_exposed_ephem_Bow_subset = sort(x_exposed_ephem_Bow_subset, TRUE)
sigtab_exposed_ephem_Bow_subset$Phylum = factor(as.character(sigtab_exposed_ephem_Bow_subset$Phylum), levels=names(x_exposed_ephem_Bow_subset))
# Genus
x_exposed_ephem_Bow_subset = tapply(sigtab_exposed_ephem_Bow_subset$log2FoldChange, sigtab_exposed_ephem_Bow_subset$Genus, function(x) max(x))
x_exposed_ephem_Bow_subset = sort(x_exposed_ephem_Bow_subset, TRUE)
sigtab_exposed_ephem_Bow_subset$Genus = factor(as.character(sigtab_exposed_ephem_Bow_subset$Genus), levels=sort(names(x_exposed_ephem_Bow_subset)))
#7 differentially abundant taxa

sigtab_exposed_ephem_Bow_subset$Genus <- factor(sigtab_exposed_ephem_Bow_subset$Genus, levels=c("Alcaligenes", "Bacillus", "Clostridium sensu stricto 1", "Moraxella", "Staphylococcus", "NA"), labels=c("Alcaligenes", "Bacillus", "Clostridium sensu stricto 1", "Moraxella", "Staphylococcus", "Unidentified"))

sigtab_exposed_ephem_Bow_subset$Genus[is.na(sigtab_exposed_ephem_Bow_subset$Genus)] <- "Unidentified"

diff_abund_ephem_Bow_plot_subset<-ggplot(sigtab_exposed_ephem_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_ephem_Bow_plot_subset
...
ephem_cushbr_coch<-subset_samples(ephem_ps, Site=="Cochrane" | Site=="Cushing Bridge")

deseq_cushbr_coch_ephem <- phyloseq_to_deseq2(ephem_cushbr_coch, ~ Site)

deseq_cushbr_coch_ephem <- DESeq(deseq_cushbr_coch_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_coch_ephem <- results(deseq_cushbr_coch_ephem, cooksCutoff = FALSE)
res_deseq_cushbr_coch_ephem
alpha = 0.001
sigtab_cushbr_coch_ephem = res_deseq_cushbr_coch_ephem[which(res_deseq_cushbr_coch_ephem$padj < alpha), ]
sigtab_cushbr_coch_ephem = cbind(as(sigtab_cushbr_coch_ephem, "data.frame"), as(tax_table(ephem_cushbr_coch)[rownames(sigtab_cushbr_coch_ephem), ], "matrix"))

x_cushbr_coch_ephem = tapply(sigtab_cushbr_coch_ephem$log2FoldChange, sigtab_cushbr_coch_ephem$Phylum, function(x) max(x))
x_cushbr_coch_ephem = sort(x_cushbr_coch_ephem, TRUE)
sigtab_cushbr_coch_ephem$Phylum = factor(as.character(sigtab_cushbr_coch_ephem$Phylum), levels=names(x_cushbr_coch_ephem))
# Genus
x_cushbr_coch_ephem = tapply(sigtab_cushbr_coch_ephem$log2FoldChange, sigtab_cushbr_coch_ephem$Genus, function(x) max(x))
x_cushbr_coch_ephem = sort(x_cushbr_coch_ephem, TRUE)
sigtab_cushbr_coch_ephem$Genus = factor(as.character(sigtab_cushbr_coch_ephem$Genus), levels=sort(names(x_cushbr_coch_ephem)))
#15 observations

diff_abund_ephem_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_ephem, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_ephem_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane ephemeroptera adult.png", height=20, width=22)


#CUSHBR - SUN

ephem_cushbr_sun<-subset_samples(ephem_ps, Site=="Sunalta" | Site=="Cushing Bridge")

deseq_cushbr_sun_ephem <- phyloseq_to_deseq2(ephem_cushbr_sun, ~ Site)

deseq_cushbr_sun_ephem <- DESeq(deseq_cushbr_sun_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_sun_ephem <- results(deseq_cushbr_sun_ephem, cooksCutoff = FALSE)
res_deseq_cushbr_sun_ephem
alpha = 0.001
sigtab_cushbr_sun_ephem = res_deseq_cushbr_sun_ephem[which(res_deseq_cushbr_sun_ephem$padj < alpha), ]
sigtab_cushbr_sun_ephem = cbind(as(sigtab_cushbr_sun_ephem, "data.frame"), as(tax_table(ephem_cushbr_sun)[rownames(sigtab_cushbr_sun_ephem), ], "matrix"))

x_cushbr_sun_ephem = tapply(sigtab_cushbr_sun_ephem$log2FoldChange, sigtab_cushbr_sun_ephem$Phylum, function(x) max(x))
x_cushbr_sun_ephem = sort(x_cushbr_sun_ephem, TRUE)
sigtab_cushbr_sun_ephem$Phylum = factor(as.character(sigtab_cushbr_sun_ephem$Phylum), levels=names(x_cushbr_sun_ephem))
# Genus
x_cushbr_sun_ephem = tapply(sigtab_cushbr_sun_ephem$log2FoldChange, sigtab_cushbr_sun_ephem$Genus, function(x) max(x))
x_cushbr_sun_ephem = sort(x_cushbr_sun_ephem, TRUE)
sigtab_cushbr_sun_ephem$Genus = factor(as.character(sigtab_cushbr_sun_ephem$Genus), levels=sort(names(x_cushbr_sun_ephem)))
#10 observations

diff_abund_ephem_cushbr_sun_plot<-ggplot(sigtab_cushbr_sun_ephem, aes(x=log2FoldChange, y=Genus, fill=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_ephem_cushbr_sun_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Sunalta ephemeroptera adult.png", height=20, width=22)


#CUSHBR - GRVBR

ephem_cushbr_grvbr<-subset_samples(ephem_ps, Site=="Graves Bridge" | Site=="Cushing Bridge")

deseq_cushbr_grvbr_ephem <- phyloseq_to_deseq2(ephem_cushbr_grvbr, ~ Site)

deseq_cushbr_grvbr_ephem$Site <-relevel(deseq_cushbr_grvbr_ephem$Site, "Graves Bridge") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_grvbr_ephem <- DESeq(deseq_cushbr_grvbr_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_grvbr_ephem <- results(deseq_cushbr_grvbr_ephem, cooksCutoff = FALSE)
res_deseq_cushbr_grvbr_ephem
alpha = 0.001
sigtab_cushbr_grvbr_ephem = res_deseq_cushbr_grvbr_ephem[which(res_deseq_cushbr_grvbr_ephem$padj < alpha), ]
sigtab_cushbr_grvbr_ephem = cbind(as(sigtab_cushbr_grvbr_ephem, "data.frame"), as(tax_table(ephem_cushbr_grvbr)[rownames(sigtab_cushbr_grvbr_ephem), ], "matrix"))

x_cushbr_grvbr_ephem = tapply(sigtab_cushbr_grvbr_ephem$log2FoldChange, sigtab_cushbr_grvbr_ephem$Phylum, function(x) max(x))
x_cushbr_grvbr_ephem = sort(x_cushbr_grvbr_ephem, TRUE)
sigtab_cushbr_grvbr_ephem$Phylum = factor(as.character(sigtab_cushbr_grvbr_ephem$Phylum), levels=names(x_cushbr_grvbr_ephem))
# Genus
x_cushbr_grvbr_ephem = tapply(sigtab_cushbr_grvbr_ephem$log2FoldChange, sigtab_cushbr_grvbr_ephem$Genus, function(x) max(x))
x_cushbr_grvbr_ephem = sort(x_cushbr_grvbr_ephem, TRUE)
sigtab_cushbr_grvbr_ephem$Genus = factor(as.character(sigtab_cushbr_grvbr_ephem$Genus), levels=sort(names(x_cushbr_grvbr_ephem)))
#13 observations

diff_abund_ephem_cushbr_grvbr_plot<-ggplot(sigtab_cushbr_grvbr_ephem, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_ephem_cushbr_grvbr_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Graves Bridge ephemeroptera adult.png", height=20, width=22)


#CUSHBR - PMF

ephem_cushbr_pmf<-subset_samples(ephem_ps, Site=="Policeman Flats" | Site=="Cushing Bridge")

deseq_cushbr_pmf_ephem <- phyloseq_to_deseq2(ephem_cushbr_pmf, ~ Site)

deseq_cushbr_pmf_ephem$Site <-relevel(deseq_cushbr_pmf_ephem$Site, "Policeman Flats") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_pmf_ephem <- DESeq(deseq_cushbr_pmf_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_pmf_ephem <- results(deseq_cushbr_pmf_ephem, cooksCutoff = FALSE)
res_deseq_cushbr_pmf_ephem
alpha = 0.001
sigtab_cushbr_pmf_ephem = res_deseq_cushbr_pmf_ephem[which(res_deseq_cushbr_pmf_ephem$padj < alpha), ]
sigtab_cushbr_pmf_ephem = cbind(as(sigtab_cushbr_pmf_ephem, "data.frame"), as(tax_table(ephem_cushbr_pmf)[rownames(sigtab_cushbr_pmf_ephem), ], "matrix"))

x_cushbr_pmf_ephem = tapply(sigtab_cushbr_pmf_ephem$log2FoldChange, sigtab_cushbr_pmf_ephem$Phylum, function(x) max(x))
x_cushbr_pmf_ephem = sort(x_cushbr_pmf_ephem, TRUE)
sigtab_cushbr_pmf_ephem$Phylum = factor(as.character(sigtab_cushbr_pmf_ephem$Phylum), levels=names(x_cushbr_pmf_ephem))
# Genus
x_cushbr_pmf_ephem = tapply(sigtab_cushbr_pmf_ephem$log2FoldChange, sigtab_cushbr_pmf_ephem$Genus, function(x) max(x))
x_cushbr_pmf_ephem = sort(x_cushbr_pmf_ephem, TRUE)
sigtab_cushbr_pmf_ephem$Genus = factor(as.character(sigtab_cushbr_pmf_ephem$Genus), levels=sort(names(x_cushbr_pmf_ephem)))
#4 observations

diff_abund_ephem_cushbr_pmf_plot<-ggplot(sigtab_cushbr_pmf_ephem, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_ephem_cushbr_pmf_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats ephemeroptera adult.png", height=20, width=22)
...
ggplot(sigtab_exposed_chiro, aes(x=log2FoldChange, y=Genus))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_blank()) +
          theme(legend.text=element_text(size=34, color="black")) + 
          theme(legend.title = element_text(size=34, color="black"), legend.position="bottom")+
  scale_y_discrete(limits=rev)+
  rremove("grid")+
...
#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Exposed vs Unexposed sites Chironomidae larvae Bow no legend.png", height=20, width=22)


#Exposed vs non wastewater exposed sites (Cushing Bridge compared to both downstream sites)

chiro_ps_Bow_subset<-subset_samples(chiro_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_chiro_Bow_subset <- phyloseq_to_deseq2(chiro_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_chiro_Bow_subset$Exposure <-relevel(deseq_exposed_chiro_Bow_subset$Exposure, "Unexposed")

deseq_exposed_chiro_Bow_subset <- DESeq(deseq_exposed_chiro_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_chiro_Bow_subset <- results(deseq_exposed_chiro_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_chiro_Bow_subset 
alpha = 0.001
sigtab_exposed_chiro_Bow_subset = res_deseq_exposed_chiro_Bow_subset[which(res_deseq_exposed_chiro_Bow_subset$padj < alpha), ]
sigtab_exposed_chiro_Bow_subset = cbind(as(sigtab_exposed_chiro_Bow_subset, "data.frame"), as(tax_table(chiro_ps_Bow_subset)[rownames(sigtab_exposed_chiro_Bow_subset), ], "matrix"))

x_exposed_chiro_Bow_subset = tapply(sigtab_exposed_chiro_Bow_subset$log2FoldChange, sigtab_exposed_chiro_Bow_subset$Phylum, function(x) max(x))
x_exposed_chiro_Bow_subset = sort(x_exposed_chiro_Bow_subset, TRUE)
sigtab_exposed_chiro_Bow_subset$Phylum = factor(as.character(sigtab_exposed_chiro_Bow_subset$Phylum), levels=names(x_exposed_chiro_Bow_subset))
# Genus
x_exposed_chiro_Bow_subset = tapply(sigtab_exposed_chiro_Bow_subset$log2FoldChange, sigtab_exposed_chiro_Bow_subset$Genus, function(x) max(x))
x_exposed_chiro_Bow_subset = sort(x_exposed_chiro_Bow_subset, TRUE)
sigtab_exposed_chiro_Bow_subset$Genus = factor(as.character(sigtab_exposed_chiro_Bow_subset$Genus), levels=sort(names(x_exposed_chiro_Bow_subset)))
#26 differentially abundant taxa

sigtab_exposed_chiro_Bow_subset$Genus <- factor(sigtab_exposed_chiro_Bow_subset$Genus, levels=c("Bacteroides", "CM1G08", "Exiguobacterium", "Ferribacterium", "Ideonella", "Leptothrix", "Limnohabitans", "Paracoccus", "Pseudorhodobacter", "Rhodobacter", "Sphingorhabdus", "Streptococcus","NA"), labels=c("Bacteroides", "CM1G08", "Exiguobacterium", "Ferribacterium", "Ideonella", "Leptothrix", "Limnohabitans", "Paracoccus", "Pseudorhodobacter", "Rhodobacter", "Sphingorhabdus", "Streptococcus", "Unidentified"))

sigtab_exposed_chiro_Bow_subset$Genus[is.na(sigtab_exposed_chiro_Bow_subset$Genus)] <- "Unidentified"

diff_abund_chiro_Bow_plot_subset<-ggplot(sigtab_exposed_chiro_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_chiro_Bow_plot_subset
...
chiro_cushbr_coch<-subset_samples(chiro_ps, Site=="Cochrane" | Site=="Cushing Bridge")

deseq_cushbr_coch_chiro <- phyloseq_to_deseq2(chiro_cushbr_coch, ~ Site)

deseq_cushbr_coch_chiro <- DESeq(deseq_cushbr_coch_chiro, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_coch_chiro <- results(deseq_cushbr_coch_chiro, cooksCutoff = FALSE)
res_deseq_cushbr_coch_chiro
alpha = 0.001
sigtab_cushbr_coch_chiro = res_deseq_cushbr_coch_chiro[which(res_deseq_cushbr_coch_chiro$padj < alpha), ]
sigtab_cushbr_coch_chiro = cbind(as(sigtab_cushbr_coch_chiro, "data.frame"), as(tax_table(chiro_cushbr_coch)[rownames(sigtab_cushbr_coch_chiro), ], "matrix"))

x_cushbr_coch_chiro = tapply(sigtab_cushbr_coch_chiro$log2FoldChange, sigtab_cushbr_coch_chiro$Phylum, function(x) max(x))
x_cushbr_coch_chiro = sort(x_cushbr_coch_chiro, TRUE)
sigtab_cushbr_coch_chiro$Phylum = factor(as.character(sigtab_cushbr_coch_chiro$Phylum), levels=names(x_cushbr_coch_chiro))
# Genus
x_cushbr_coch_chiro = tapply(sigtab_cushbr_coch_chiro$log2FoldChange, sigtab_cushbr_coch_chiro$Genus, function(x) max(x))
x_cushbr_coch_chiro = sort(x_cushbr_coch_chiro, TRUE)
sigtab_cushbr_coch_chiro$Genus = factor(as.character(sigtab_cushbr_coch_chiro$Genus), levels=sort(names(x_cushbr_coch_chiro)))
#22 observations

diff_abund_chiro_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_chiro, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_chiro_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane chironomidae larvae.png", height=20, width=22)


#CUSHBR - PMF

chiro_cushbr_pmf<-subset_samples(chiro_ps, Site=="Policeman Flats" | Site=="Cushing Bridge")

deseq_cushbr_pmf_chiro <- phyloseq_to_deseq2(chiro_cushbr_pmf, ~ Site)

deseq_cushbr_pmf_chiro$Site <-relevel(deseq_cushbr_pmf_chiro$Site, "Policeman Flats") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_pmf_chiro <- DESeq(deseq_cushbr_pmf_chiro, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_pmf_chiro <- results(deseq_cushbr_pmf_chiro, cooksCutoff = FALSE)
res_deseq_cushbr_pmf_chiro
alpha = 0.001
sigtab_cushbr_pmf_chiro = res_deseq_cushbr_pmf_chiro[which(res_deseq_cushbr_pmf_chiro$padj < alpha), ]
sigtab_cushbr_pmf_chiro = cbind(as(sigtab_cushbr_pmf_chiro, "data.frame"), as(tax_table(chiro_cushbr_pmf)[rownames(sigtab_cushbr_pmf_chiro), ], "matrix"))

x_cushbr_pmf_chiro = tapply(sigtab_cushbr_pmf_chiro$log2FoldChange, sigtab_cushbr_pmf_chiro$Phylum, function(x) max(x))
x_cushbr_pmf_chiro = sort(x_cushbr_pmf_chiro, TRUE)
sigtab_cushbr_pmf_chiro$Phylum = factor(as.character(sigtab_cushbr_pmf_chiro$Phylum), levels=names(x_cushbr_pmf_chiro))
# Genus
x_cushbr_pmf_chiro = tapply(sigtab_cushbr_pmf_chiro$log2FoldChange, sigtab_cushbr_pmf_chiro$Genus, function(x) max(x))
x_cushbr_pmf_chiro = sort(x_cushbr_pmf_chiro, TRUE)
sigtab_cushbr_pmf_chiro$Genus = factor(as.character(sigtab_cushbr_pmf_chiro$Genus), levels=sort(names(x_cushbr_pmf_chiro)))
#26 observations

diff_abund_chiro_cushbr_pmf_plot<-ggplot(sigtab_cushbr_pmf_chiro, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_chiro_cushbr_pmf_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats chironomidae larvae.png", height=20, width=22)
...
dip_ps_Bow_subset<-subset_samples(dip_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_dip_Bow_subset <- phyloseq_to_deseq2(dip_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_dip_Bow_subset$Exposure <-relevel(deseq_exposed_dip_Bow_subset$Exposure, "Unexposed")

deseq_exposed_dip_Bow_subset <- DESeq(deseq_exposed_dip_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_dip_Bow_subset <- results(deseq_exposed_dip_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_dip_Bow_subset 
alpha = 0.001
sigtab_exposed_dip_Bow_subset = res_deseq_exposed_dip_Bow_subset[which(res_deseq_exposed_dip_Bow_subset$padj < alpha), ]
sigtab_exposed_dip_Bow_subset = cbind(as(sigtab_exposed_dip_Bow_subset, "data.frame"), as(tax_table(dip_ps_Bow_subset)[rownames(sigtab_exposed_dip_Bow_subset), ], "matrix"))

x_exposed_dip_Bow_subset = tapply(sigtab_exposed_dip_Bow_subset$log2FoldChange, sigtab_exposed_dip_Bow_subset$Phylum, function(x) max(x))
x_exposed_dip_Bow_subset = sort(x_exposed_dip_Bow_subset, TRUE)
sigtab_exposed_dip_Bow_subset$Phylum = factor(as.character(sigtab_exposed_dip_Bow_subset$Phylum), levels=names(x_exposed_dip_Bow_subset))
# Genus
x_exposed_dip_Bow_subset = tapply(sigtab_exposed_dip_Bow_subset$log2FoldChange, sigtab_exposed_dip_Bow_subset$Genus, function(x) max(x))
x_exposed_dip_Bow_subset = sort(x_exposed_dip_Bow_subset, TRUE)
sigtab_exposed_dip_Bow_subset$Genus = factor(as.character(sigtab_exposed_dip_Bow_subset$Genus), levels=sort(names(x_exposed_dip_Bow_subset)))
#88 differentially abundant taxa

sigtab_exposed_dip_Bow_subset$Genus <- factor(sigtab_exposed_dip_Bow_subset$Genus, levels=c("Aeromonas", "Alcaligenes", "Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium", "Anaerostipes", "Bacillus", "Blautia", "Bradyrhizobium", "Brevibacillus", "Brevundimonas", "Caenimonas", "Cereibacter", "CL500-29 marine group", "Clavibacter", "Coprococcus", "Corynebacterium", "Cytophaga", "Faecalibacterium", "Ferruginibacter", "Flavisolibacter", "Flavobacterium", "Fluviicola", "Frigoribacterium", "Haemophilus", "Hirschia", "Hymenobacter", "Ideonella", "Klebsiella", "Lachnoclostridium", "Lachnospiraceae NK4A136 group", "Lactobacillus", "Limnohabitans", "Lysinibacillus", "Marmoricola", "Methylobacterium-Methylorubrum", "Methylophilus", "Methylotenera", "Micrococcus", "Microterricola", "Neisseria", "Nocardioides", "Paenisporosarcina", "Pedobacter", "Rathayibacter", "Rhizobacter", "Rhizorhapis", "Rhodoferax", "Rubellimicrobium", "[Ruminococcus] gauvreauii group", "Serratia", "Solitalea", "Sphaerotilus", "Sphingorhabdus", "Spiroplasma", "Staphylococcus", "Streptococcus", "Wolbachia", "NA"), labels=c("Aeromonas", "Alcaligenes", "Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium", "Anaerostipes", "Bacillus", "Blautia", "Bradyrhizobium", "Brevibacillus", "Brevundimonas", "Caenimonas", "Cereibacter", "CL500-29 marine group", "Clavibacter", "Coprococcus", "Corynebacterium", "Cytophaga", "Faecalibacterium", "Ferruginibacter", "Flavisolibacter", "Flavobacterium", "Fluviicola", "Frigoribacterium", "Haemophilus", "Hirschia", "Hymenobacter", "Ideonella", "Klebsiella", "Lachnoclostridium", "Lachnospiraceae NK4A136 group", "Lactobacillus", "Limnohabitans", "Lysinibacillus", "Marmoricola", "Methylobacterium-Methylorubrum", "Methylophilus", "Methylotenera", "Micrococcus", "Microterricola", "Neisseria", "Nocardioides", "Paenisporosarcina", "Pedobacter", "Rathayibacter", "Rhizobacter", "Rhizorhapis", "Rhodoferax", "Rubellimicrobium","[Ruminococcus] gauvreauii group","Serratia", "Solitalea", "Sphaerotilus", "Sphingorhabdus", "Spiroplasma", "Staphylococcus", "Streptococcus", "Wolbachia", "Unidentified"))

sigtab_exposed_dip_Bow_subset$Genus[is.na(sigtab_exposed_dip_Bow_subset$Genus)] <- "Unidentified"

diff_abund_dip_Bow_plot_subset<-ggplot(sigtab_exposed_dip_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_dip_Bow_plot_subset
...
dip_cushbr_coch<-subset_samples(dip_ps, Site=="Cochrane" | Site=="Cushing Bridge")

deseq_cushbr_coch_dip <- phyloseq_to_deseq2(dip_cushbr_coch, ~ Site)

deseq_cushbr_coch_dip <- DESeq(deseq_cushbr_coch_dip, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_coch_dip <- results(deseq_cushbr_coch_dip, cooksCutoff = FALSE)
res_deseq_cushbr_coch_dip
alpha = 0.001
...
diff_abund_dip_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_dip, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_dip_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane diptera adult.png", height=20, width=22)
...
diff_abund_dip_cushbr_pmf_plot<-ggplot(sigtab_cushbr_pmf_dip, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_dip_cushbr_pmf_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats diptera adult.png", height=20, width=22)
...
perl_ps_Bow_subset<-subset_samples(perl_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_perl_Bow_subset <- phyloseq_to_deseq2(perl_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_perl_Bow_subset$Exposure <-relevel(deseq_exposed_perl_Bow_subset$Exposure, "Unexposed")

deseq_exposed_perl_Bow_subset <- DESeq(deseq_exposed_perl_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_perl_Bow_subset <- results(deseq_exposed_perl_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_perl_Bow_subset 
alpha = 0.001
sigtab_exposed_perl_Bow_subset = res_deseq_exposed_perl_Bow_subset[which(res_deseq_exposed_perl_Bow_subset$padj < alpha), ]
sigtab_exposed_perl_Bow_subset = cbind(as(sigtab_exposed_perl_Bow_subset, "data.frame"), as(tax_table(perl_ps_Bow_subset)[rownames(sigtab_exposed_perl_Bow_subset), ], "matrix"))

x_exposed_perl_Bow_subset = tapply(sigtab_exposed_perl_Bow_subset$log2FoldChange, sigtab_exposed_perl_Bow_subset$Phylum, function(x) max(x))
x_exposed_perl_Bow_subset = sort(x_exposed_perl_Bow_subset, TRUE)
sigtab_exposed_perl_Bow_subset$Phylum = factor(as.character(sigtab_exposed_perl_Bow_subset$Phylum), levels=names(x_exposed_perl_Bow_subset))
# Genus
x_exposed_perl_Bow_subset = tapply(sigtab_exposed_perl_Bow_subset$log2FoldChange, sigtab_exposed_perl_Bow_subset$Genus, function(x) max(x))
x_exposed_perl_Bow_subset = sort(x_exposed_perl_Bow_subset, TRUE)
sigtab_exposed_perl_Bow_subset$Genus = factor(as.character(sigtab_exposed_perl_Bow_subset$Genus), levels=sort(names(x_exposed_perl_Bow_subset)))
#36 differentially abundant taxa

sigtab_exposed_perl_Bow_subset$Genus <- factor(sigtab_exposed_perl_Bow_subset$Genus, levels=c("Aquabacterium", "Azovibrio", "Clostridium sensu stricto 1", "Emticicia", "Flavobacterium", "Hydrogenophaga", "Ideonella", "Iodobacter", "Leptothrix", "Limnohabitans", "Methylotenera", "Pseudorhodobacter", "Rhizobacter", "Rhodococcus", "Rhodoferax", "Rubrivivax", "Serratia", "Spingorhabdus", "Tabrizicola", "NA"), labels=c("Aquabacterium", "Azovibrio", "Clostridium sensu stricto 1", "Emticicia", "Flavobacterium", "Hydrogenophaga", "Ideonella", "Iodobacter", "Leptothrix", "Limnohabitans", "Methylotenera", "Pseudorhodobacter", "Rhizobacter", "Rhodococcus", "Rhodoferax", "Rubrivivax", "Serratia", "Spingorhabdus", "Tabrizicola", "Unidentified"))

sigtab_exposed_perl_Bow_subset$Genus[is.na(sigtab_exposed_perl_Bow_subset$Genus)] <- "Unidentified"

diff_abund_perl_Bow_plot_subset<-ggplot(sigtab_exposed_perl_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_perl_Bow_plot_subset
...
diff_abund_perl_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_perl, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_perl_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane perlidae larvae.png", height=20, width=22)
...
diff_abund_perl_cushbr_pmf_plot<-ggplot(sigtab_cushbr_pmf_perl, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_perl_cushbr_pmf_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats perlidae larvae.png", height=20, width=22)
...
ggplot(sigtab_exposed_aran, aes(x=log2FoldChange, y=Genus))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_blank()) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Exposed vs Unexposed sites Araneidae spiders Bow no legend.png", height=20, width=22)


#Exposed vs non wastewater exposed sites (Cushing Bridge compared to both downstream sites)

aran_ps_Bow_subset<-subset_samples(aran_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_aran_Bow_subset <- phyloseq_to_deseq2(aran_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_aran_Bow_subset$Exposure <-relevel(deseq_exposed_aran_Bow_subset$Exposure, "Unexposed")

deseq_exposed_aran_Bow_subset <- DESeq(deseq_exposed_aran_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_aran_Bow_subset <- results(deseq_exposed_aran_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_aran_Bow_subset 
alpha = 0.001
sigtab_exposed_aran_Bow_subset = res_deseq_exposed_aran_Bow_subset[which(res_deseq_exposed_aran_Bow_subset$padj < alpha), ]
sigtab_exposed_aran_Bow_subset = cbind(as(sigtab_exposed_aran_Bow_subset, "data.frame"), as(tax_table(aran_ps_Bow_subset)[rownames(sigtab_exposed_aran_Bow_subset), ], "matrix"))

x_exposed_aran_Bow_subset = tapply(sigtab_exposed_aran_Bow_subset$log2FoldChange, sigtab_exposed_aran_Bow_subset$Phylum, function(x) max(x))
x_exposed_aran_Bow_subset = sort(x_exposed_aran_Bow_subset, TRUE)
sigtab_exposed_aran_Bow_subset$Phylum = factor(as.character(sigtab_exposed_aran_Bow_subset$Phylum), levels=names(x_exposed_aran_Bow_subset))
# Genus
x_exposed_aran_Bow_subset = tapply(sigtab_exposed_aran_Bow_subset$log2FoldChange, sigtab_exposed_aran_Bow_subset$Genus, function(x) max(x))
x_exposed_aran_Bow_subset = sort(x_exposed_aran_Bow_subset, TRUE)
sigtab_exposed_aran_Bow_subset$Genus = factor(as.character(sigtab_exposed_aran_Bow_subset$Genus), levels=sort(names(x_exposed_aran_Bow_subset)))
#3 differentially abundant taxa

sigtab_exposed_aran_Bow_subset$Genus <- factor(sigtab_exposed_aran_Bow_subset$Genus, levels=c("Agathobacter", "Massilia"))

diff_abund_aran_Bow_plot_subset<-ggplot(sigtab_exposed_aran_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_aran_Bow_plot_subset
...
diff_abund_aran_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_aran, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_aran_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane araneidae.png", height=20, width=22)
...
diff_abund_aran_cushbr_sun_plot<-ggplot(sigtab_cushbr_sun_aran, aes(x=log2FoldChange, y=Genus, fill=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_aran_cushbr_sun_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Sunalta araneidae.png", height=20, width=22)
...
diff_abund_aran_cushbr_grvbr_plot<-ggplot(sigtab_cushbr_grvbr_aran, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_aran_cushbr_grvbr_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Graves Bridge aran.png", height=20, width=22)
...
diff_abund_aran_cushbr_pmf_plot<-ggplot(sigtab_cushbr_pmf_aran, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_aran_cushbr_pmf_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats araneidae.png", height=20, width=22)
...
ggplot(sigtab_exposed_tetra, aes(x=log2FoldChange, y=Genus))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_blank()) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Exposed vs Unexposed sites Tetragnathidae spiders Bow no legend.png", height=20, width=22)


#Exposed vs non wastewater exposed sites (Cushing Bridge compared to both downstream sites)

tetra_ps_Bow_subset<-subset_samples(tetra_ps, Site!="Cochrane" & Site!="Sunalta")

deseq_exposed_tetra_Bow_subset <- phyloseq_to_deseq2(tetra_ps_Bow_subset, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
deseq_exposed_tetra_Bow_subset$Exposure <-relevel(deseq_exposed_tetra_Bow_subset$Exposure, "Unexposed")

deseq_exposed_tetra_Bow_subset <- DESeq(deseq_exposed_tetra_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_exposed_tetra_Bow_subset <- results(deseq_exposed_tetra_Bow_subset, cooksCutoff = FALSE)
res_deseq_exposed_tetra_Bow_subset 
alpha = 0.001
sigtab_exposed_tetra_Bow_subset = res_deseq_exposed_tetra_Bow_subset[which(res_deseq_exposed_tetra_Bow_subset$padj < alpha), ]
sigtab_exposed_tetra_Bow_subset = cbind(as(sigtab_exposed_tetra_Bow_subset, "data.frame"), as(tax_table(tetra_ps_Bow_subset)[rownames(sigtab_exposed_tetra_Bow_subset), ], "matrix"))

x_exposed_tetra_Bow_subset = tapply(sigtab_exposed_tetra_Bow_subset$log2FoldChange, sigtab_exposed_tetra_Bow_subset$Phylum, function(x) max(x))
x_exposed_tetra_Bow_subset = sort(x_exposed_tetra_Bow_subset, TRUE)
sigtab_exposed_tetra_Bow_subset$Phylum = factor(as.character(sigtab_exposed_tetra_Bow_subset$Phylum), levels=names(x_exposed_tetra_Bow_subset))
# Genus
x_exposed_tetra_Bow_subset = tapply(sigtab_exposed_tetra_Bow_subset$log2FoldChange, sigtab_exposed_tetra_Bow_subset$Genus, function(x) max(x))
x_exposed_tetra_Bow_subset = sort(x_exposed_tetra_Bow_subset, TRUE)
sigtab_exposed_tetra_Bow_subset$Genus = factor(as.character(sigtab_exposed_tetra_Bow_subset$Genus), levels=sort(names(x_exposed_tetra_Bow_subset)))
#3 differentially abundant taxa

sigtab_exposed_tetra_Bow_subset$Genus <- factor(sigtab_exposed_tetra_Bow_subset$Genus, levels=c("Spiroplasma", "Wolbachia"))

diff_abund_tetra_Bow_plot_subset<-ggplot(sigtab_exposed_tetra_Bow_subset, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
diff_abund_tetra_Bow_plot_subset
...
diff_abund_tetra_cushbr_coch_plot<-ggplot(sigtab_cushbr_coch_tetra, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_tetra_cushbr_coch_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Cochrane tetragnathidae.png", height=20, width=22)


#CUSHBR - SUN

tetra_cushbr_sun<-subset_samples(tetra_ps, Site=="Sunalta" | Site=="Cushing Bridge")

deseq_cushbr_sun_tetra <- phyloseq_to_deseq2(tetra_cushbr_sun, ~ Site)

deseq_cushbr_sun_tetra <- DESeq(deseq_cushbr_sun_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_sun_tetra <- results(deseq_cushbr_sun_tetra, cooksCutoff = FALSE)
res_deseq_cushbr_sun_tetra
alpha = 0.001
sigtab_cushbr_sun_tetra = res_deseq_cushbr_sun_tetra[which(res_deseq_cushbr_sun_tetra$padj < alpha), ]
sigtab_cushbr_sun_tetra = cbind(as(sigtab_cushbr_sun_tetra, "data.frame"), as(tax_table(tetra_cushbr_sun)[rownames(sigtab_cushbr_sun_tetra), ], "matrix"))

x_cushbr_sun_tetra = tapply(sigtab_cushbr_sun_tetra$log2FoldChange, sigtab_cushbr_sun_tetra$Phylum, function(x) max(x))
x_cushbr_sun_tetra = sort(x_cushbr_sun_tetra, TRUE)
sigtab_cushbr_sun_tetra$Phylum = factor(as.character(sigtab_cushbr_sun_tetra$Phylum), levels=names(x_cushbr_sun_tetra))
# Genus
x_cushbr_sun_tetra = tapply(sigtab_cushbr_sun_tetra$log2FoldChange, sigtab_cushbr_sun_tetra$Genus, function(x) max(x))
x_cushbr_sun_tetra = sort(x_cushbr_sun_tetra, TRUE)
sigtab_cushbr_sun_tetra$Genus = factor(as.character(sigtab_cushbr_sun_tetra$Genus), levels=sort(names(x_cushbr_sun_tetra)))
#2 observations

diff_abund_tetra_cushbr_sun_plot<-ggplot(sigtab_cushbr_sun_tetra, aes(x=log2FoldChange, y=Genus, fill=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_tetra_cushbr_sun_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Sunalta tetragnathidae.png", height=20, width=22)


#CUSHBR - GRVBR

tetra_cushbr_grvbr<-subset_samples(tetra_ps, Site=="Graves Bridge" | Site=="Cushing Bridge")

deseq_cushbr_grvbr_tetra <- phyloseq_to_deseq2(tetra_cushbr_grvbr, ~ Site)

deseq_cushbr_grvbr_tetra$Site <-relevel(deseq_cushbr_grvbr_tetra$Site, "Graves Bridge") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_grvbr_tetra <- DESeq(deseq_cushbr_grvbr_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_grvbr_tetra <- results(deseq_cushbr_grvbr_tetra, cooksCutoff = FALSE)
res_deseq_cushbr_grvbr_tetra
alpha = 0.001
sigtab_cushbr_grvbr_tetra = res_deseq_cushbr_grvbr_tetra[which(res_deseq_cushbr_grvbr_tetra$padj < alpha), ]
sigtab_cushbr_grvbr_tetra = cbind(as(sigtab_cushbr_grvbr_tetra, "data.frame"), as(tax_table(tetra_cushbr_grvbr)[rownames(sigtab_cushbr_grvbr_tetra), ], "matrix"))

x_cushbr_grvbr_tetra = tapply(sigtab_cushbr_grvbr_tetra$log2FoldChange, sigtab_cushbr_grvbr_tetra$Phylum, function(x) max(x))
x_cushbr_grvbr_tetra = sort(x_cushbr_grvbr_tetra, TRUE)
sigtab_cushbr_grvbr_tetra$Phylum = factor(as.character(sigtab_cushbr_grvbr_tetra$Phylum), levels=names(x_cushbr_grvbr_tetra))
# Genus
x_cushbr_grvbr_tetra = tapply(sigtab_cushbr_grvbr_tetra$log2FoldChange, sigtab_cushbr_grvbr_tetra$Genus, function(x) max(x))
x_cushbr_grvbr_tetra = sort(x_cushbr_grvbr_tetra, TRUE)
sigtab_cushbr_grvbr_tetra$Genus = factor(as.character(sigtab_cushbr_grvbr_tetra$Genus), levels=sort(names(x_cushbr_grvbr_tetra)))
#1 observations

diff_abund_tetra_cushbr_grvbr_plot<-ggplot(sigtab_cushbr_grvbr_tetra, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_tetra_cushbr_grvbr_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Graves Bridge tetragnathidae.png", height=20, width=22)


#CUSHBR - PMF

tetra_cushbr_pmf<-subset_samples(tetra_ps, Site=="Policeman Flats" | Site=="Cushing Bridge")

deseq_cushbr_pmf_tetra <- phyloseq_to_deseq2(tetra_cushbr_pmf, ~ Site)

deseq_cushbr_pmf_tetra$Site <-relevel(deseq_cushbr_pmf_tetra$Site, "Policeman Flats") #Releveling to make Cushing Bridge be compared to Graves Bridge

deseq_cushbr_pmf_tetra <- DESeq(deseq_cushbr_pmf_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

res_deseq_cushbr_pmf_tetra <- results(deseq_cushbr_pmf_tetra, cooksCutoff = FALSE)
res_deseq_cushbr_pmf_tetra
alpha = 0.001
sigtab_cushbr_pmf_tetra = res_deseq_cushbr_pmf_tetra[which(res_deseq_cushbr_pmf_tetra$padj < alpha), ]
sigtab_cushbr_pmf_tetra = cbind(as(sigtab_cushbr_pmf_tetra, "data.frame"), as(tax_table(tetra_cushbr_pmf)[rownames(sigtab_cushbr_pmf_tetra), ], "matrix"))

x_cushbr_pmf_tetra = tapply(sigtab_cushbr_pmf_tetra$log2FoldChange, sigtab_cushbr_pmf_tetra$Phylum, function(x) max(x))
x_cushbr_pmf_tetra = sort(x_cushbr_pmf_tetra, TRUE)
sigtab_cushbr_pmf_tetra$Phylum = factor(as.character(sigtab_cushbr_pmf_tetra$Phylum), levels=names(x_cushbr_pmf_tetra))
# Genus
x_cushbr_pmf_tetra = tapply(sigtab_cushbr_pmf_tetra$log2FoldChange, sigtab_cushbr_pmf_tetra$Genus, function(x) max(x))
x_cushbr_pmf_tetra = sort(x_cushbr_pmf_tetra, TRUE)
sigtab_cushbr_pmf_tetra$Genus = factor(as.character(sigtab_cushbr_pmf_tetra$Genus), levels=sort(names(x_cushbr_pmf_tetra)))
#3 observations

diff_abund_tetra_cushbr_pmf_plot<-ggplot(sigtab_cushbr_pmf_tetra, aes(x=log2FoldChange, y=Genus, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=6, color="black") +
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_tetra_cushbr_pmf_plot

#ggsave(filename="microbiome_analysis/Final analysis/Differential Abundance Results/Cushing Bridge vs Policeman Flats tetragnathidae.png", height=20, width=22)
...
diff_abund_hydro_plot<-ggplot(sigtab_exposed_hydro_Bow, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=42, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=28)) +
...
sigtab_exposed_hydro_Bow_subset$Phylum
#Bacteroidota, Campilobacterota, Firmicutes, Fusobacteriota, Proteobacteria, Rs-K70 termite group
sigtab_exposed_hydro_Bow_subset$Phylum<-factor(sigtab_exposed_hydro_Bow_subset$Phylum, levels=c("Bacteroidota","Campilobacterota", "Firmicutes", "Fusobacteriota", "Proteobacteria", "Rs-K70 termite group"))

diff_abund_hydro_plot_subset<-ggplot(sigtab_exposed_hydro_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=42, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=10, color="black")) + 
 ...
 scale_x_continuous(limits=c(-30, 30), breaks=c(-30, -20, -10, 0, 10, 20, 30))+
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#D72323", "#F98B39", "#FCFF7D", "#CCA60A", "#235b54", "#3998f5", "#2f2aa0"))+
  scale_y_discrete(limits=rev)
diff_abund_hydro_plot_subset
...
sigtab_exposed_hep_Bow_subset$Phylum
#Actinobacteriota, Bacteroidota, Cyanobacteria, Firmicutes, Proteobacteria
sigtab_exposed_hep_Bow_subset$Phylum<-factor(sigtab_exposed_hep_Bow_subset$Phylum, levels=c("Actinobacteriota", "Bacteroidota", "Cyanobacteria", "Firmicutes", "Proteobacteria"))

diff_abund_hep_plot_subset<-ggplot(sigtab_exposed_hep_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=42, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=10, color="black")) + 
 ...
 scale_x_continuous(limits=c(-30, 30), breaks=c(-30, -20, -10, 0, 10, 20, 30))+
  rremove("grid")+
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#ff3333", "#991919", "#ff9933", "#fcff5d", "#235b54"))+
  scale_y_discrete(limits=rev)
diff_abund_hep_plot_subset
...
diff_abund_chiro_plot<-ggplot(sigtab_exposed_chiro, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=34, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=10, color="black")) + 
          theme(legend.title = element_text(size=24, color="black"), legend.position="bottom")+
  guides(nrow=8)+
  rremove("grid")+
...
sigtab_exposed_chiro_Bow_subset$Phylum
#Bacteroidota, Firmicutes, Proteobacteria

sigtab_exposed_chiro_Bow_subset$Phylum<-factor(sigtab_exposed_chiro_Bow_subset$Phylum, levels=c("Bacteroidota", "Firmicutes", "Proteobacteria"))

diff_abund_chiro_plot_subset<-ggplot(sigtab_exposed_chiro_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=34, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=10, color="black")) + 
 ...
 scale_x_continuous(limits=c(-30, 30), breaks=c(-30, -20, -10, 0, 10, 20, 30))+
  rremove("grid")+
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#991919", "#fcff5d", "#235b54"))+
   scale_y_discrete(limits=rev)
diff_abund_chiro_plot_subset
...
sigtab_exposed_perl_Bow_subset$Phylum
#Actinobacteriota, Bacteroidota, Firmicutes, Proteobacteria, Verrucomicrobiota
sigtab_exposed_perl_Bow_subset$Phylum<-factor(sigtab_exposed_perl_Bow_subset$Phylum, levels=c("Actinobacteriota", "Bacteroidota", "Firmicutes", "Proteobacteria", "Verrucomicrobiota"))

diff_abund_perl_plot_subset<-ggplot(sigtab_exposed_perl_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
scale_x_continuous(limits=c(-30, 30), breaks=c(-30, -20, -10, 0, 10, 20, 30))+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
  ...
  rremove("grid")+
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#ff3333", "#991919", "#fcff5d", "#235b54" ,"#2f2aa0"))+
   scale_y_discrete(limits=rev)
diff_abund_perl_plot_subset
...
sigtab_exposed_trichop_Bow_subset$Phylum
#Actinobacteriota, Bacteroidota, Cyanobacteria, Firmicutes, Proteobacteria
sigtab_exposed_trichop_Bow_subset$Phylum<-factor(sigtab_exposed_trichop_Bow_subset$Phylum, levels=c("Actinobacteriota", "Bacteroidota", "Cyanobacteria", "Firmicutes", "Proteobacteria"))

diff_abund_trichop_plot_subset<-ggplot(sigtab_exposed_trichop_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=34, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=10, color="black")) + 
 ...
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#ff3333", "#991919", "#ff9933", "#fcff5d", "#235b54"))+
   scale_y_discrete(limits=rev)
diff_abund_trichop_plot_subset
...
sigtab_exposed_ephem_Bow_subset$Phylum
#Firmicutes, Proteobacteria
sigtab_exposed_ephem_Bow_subset$Phylum<-factor(sigtab_exposed_ephem_Bow_subset$Phylum, levels=c("Firmicutes", "Proteobacteria"))

diff_abund_ephem_plot_subset<-ggplot(sigtab_exposed_ephem_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=34, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=10, color="black")) + 
 ...
 scale_x_continuous(limits=c(-30, 30), breaks=c(-30, -20, -10, 0, 10, 20, 30))+
  rremove("grid")+
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#fcff5d", "#235b54"))+
   scale_y_discrete(limits=rev)
diff_abund_ephem_plot_subset
...
diff_abund_dip_plot<-ggplot(sigtab_exposed_dip, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=40, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=10, color="black")) + 
          theme(legend.title = element_text(size=24, color="black"), legend.position="bottom")+
  guides(nrow=8)+
  rremove("grid")+
...
#cushbr as the reference

sigtab_exposed_dip_Bow_subset$Phylum
#Actinobacteriota, Bacteroidota, Firmicutes, Patescibacteria, Proteobacteria
sigtab_exposed_dip_Bow_subset$Phylum<-factor(sigtab_exposed_dip_Bow_subset$Phylum, levels=c("Actinobacteriota", "Bacteroidota", "Cyanobacteria", "Firmicutes","Patescibacteria","Proteobacteria"))

diff_abund_dip_plot_subset<-ggplot(sigtab_exposed_dip_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  scale_x_continuous(limits=c(-30, 30), breaks=c(-30, -20, -10, 0, 10, 20, 30))+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=40, color="black"), axis.title=element_text(size=28)) +
  ...
  rremove("grid")+
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#ff3333", "#991919", "#ff9933", "#fcff5d", "#228c68", "#235b54"))+
   scale_y_discrete(limits=rev)
diff_abund_dip_plot_subset
...
sigtab_exposed_aran_Bow_subset$Phylum
#Firmicutes, Proteobacteria

diff_abund_aran_plot_subset<-ggplot(sigtab_exposed_aran_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
diff_abund_tetra_plot<-ggplot(sigtab_exposed_tetra, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
          theme(legend.title = element_text(size=24, color="black"), legend.position="bottom")+
  guides(nrow=8)+
  rremove("grid")+
...
sigtab_exposed_tetra_Bow_subset$Phylum
#Firmicutes, Proteobacteria

diff_abund_tetra_plot_subset<-ggplot(sigtab_exposed_tetra_Bow_subset, aes(y=Genus, x=log2FoldChange, color=Phylum))+
  geom_point(aes(fill=Phylum), pch=21, size=8, color="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
 ...
 labs(y = expression(Log[2]~ "Ratio of Absolute Abundance"))+
  scale_fill_manual(values=c("#fcff5d", "#235b54"))+
   scale_y_discrete(limits=rev)
diff_abund_tetra_plot_subset

# Hydropsychidae #
diff_abund_hydro_plot_1<- diff_abund_hydro_plot_subset + rremove("axis.title") + rremove("legend")
diff_abund_hydro_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_hydro_exposed_cushbr.png", height=18, width=18, bg="white")

# Heptageniidae #
diff_abund_hep_plot_1<- diff_abund_hep_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_hep_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_hep_exposed_cushbr.png", height=18, width=18, bg="white")

# Chironomidae #
diff_abund_chiro_plot_1<- diff_abund_chiro_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_chiro_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_chiro_exposed_cushbr.png", height=14, width=14, bg="white")

# Perlidae #
diff_abund_perl_plot_1<- diff_abund_perl_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_perl_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_perl_exposed_cushbr.png", height=10, width=10, bg="white")

# Trichoptera #
diff_abund_trichop_plot_1<- diff_abund_trichop_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_trichop_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_trichop_exposed_cushbr.png", height=16, width=16, bg="white")

# Ephemeroptera #
diff_abund_ephem_plot_1<- diff_abund_ephem_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_ephem_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_ephem_exposed_cushbr.png", height=14, width=14, bg="white")

# Diptera #
diff_abund_dip_plot_1<- diff_abund_dip_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_dip_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_dip_exposed_cushbr.png", height=28, width=24, bg="white")

# Araneidae #
diff_abund_aran_plot_1<- diff_abund_aran_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_aran_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_aran_exposed_cushbr.png", height=6, width=8, bg="white")

# Tetragnathidae #
diff_abund_tetra_plot_1<- diff_abund_tetra_plot_subset + rremove("axis.title")+ rremove("legend")
diff_abund_tetra_plot_1
#ggsave("C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\microbiome_analysis\\Final analysis\\Differential Abundance Results\\diff_abund_tetra_exposed_cushbr.png", height=6, width=8, bg="white")
...
DA_test_hydro_Bow_exposed <- testDA(hydro_Bow_ps, predictor = "Exposure", relative=FALSE)
summary(DA_test_hydro_Bow_exposed)
vennDA(DA_test_hydro_Bow_exposed, tests = c("adx.t","wil"), split = TRUE)


# Cochrane - Sunalta #
hydro_coch_sun<-subset_samples(hydro_Bow_ps, Site=="Cochrane" | Site=="Sunalta")

aldex_hydro_coch_sun <- DA.adx(data = hydro_Coch_Sun, predictor = "Site")
sig_aldex_hydro_coch_sun<-subset(aldex_hydro_coch_sun, wi.eBH < 0.05) #filtering for a Benjamini-Hochberg corrected p-value of Wilcoxon test less than 0.05

# Cochrane - Cushing bridge #
hydro_coch_cush<-subset_samples(hydro_Bow_ps, Site=="Cochrane" | Site=="Cushing Bridge")

aldex_hydro_coch_cush <- DA.adx(data = hydro_coch_cush, predictor = "Site")
sig_aldex_hydro_coch_cush<-subset(aldex_hydro_coch_cush, wi.eBH < 0.05)

# Cochrane - Graves Bridge #
hydro_coch_grvbr<-subset_samples(hydro_Bow_ps, Site=="Cochrane" | Site=="Graves Bridge")

aldex_hydro_coch_grvbr <- DA.adx(data = hydro_coch_grvbr, predictor = "Site")
sig_aldex_hydro_coch_grvbr<-subset(aldex_hydro_coch_grvbr, wi.eBH < 0.05)
...
hydro_REF_EFF5<-subset_samples(hydro_ACWA, Site!="PCR3")

aldex_hydro_REF_EFF5 <- DA.adx(data = hydro_REF_EFF5, predictor = "Site")
sig_aldex_hydro_REF_EFF5<-subset(aldex_hydro_REF_EFF5, wi.eBH < 0.05) 
#filtering for a Benjamini-Hochberg corrected p-value of Wilcoxon test less than 0.05


# REF - EFF15 #

hydro_REF_EFF15<-subset_samples(hydro_ACWA, Site!="PCR1")

aldex_hydro_REF_EFF15 <- DA.adx(data = hydro_REF_EFF15, predictor = "Site")
sig_aldex_hydro_REF_EFF15<-subset(aldex_hydro_REF_EFF15, wi.eBH < 0.05) 


# EFF5 - EFF15 #

hydro_EFF5_EFF15<-subset_samples(hydro_ACWA, Site!="BRR2")

aldex_hydro_EFF5_EFF15 <- DA.adx(data = hydro_EFF5_EFF15, predictor = "Site")
sig_aldex_hydro_EFF5_EFF15<-subset(aldex_hydro_EFF5_EFF15, wi.eBH < 0.05) 
```

# Endosymbionts
## Relative abundance of endosymbionts to total abundance/site and taxa
```{r}
endo<-subset_taxa(sample_ps, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")
...
coch_endo_hydro<- hydro_coch_genus %>%
  mutate(Genus = fct_other(Genus, keep=c("Rickettsia", "Spiroplasma", "Buchnera", "Wolbachia", "Candidatus Cardinium", "Candidatus Hamiltonella", "Candidatus Megaira", "Rickettsiella", "Diplorickettsia")))%>%
  ungroup()
...
endo<-subset_taxa(sample_ps, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus =="Wolbachia" | Genus=="Spiroplasma")
endo<-prune_taxa(taxa_sums(endo) > 0, endo) 
endo #48 taxa, 331 samples 
...
endosymbionts_Bow <- sort_genus_Bow$Genus[c(481:485, 591:595, 616:620, 626:630, 2836:2840, 3146:3150, 3546:3550)]
endosymbionts_Bow
...
endo_agg_df_Bow$Genus<-factor(endo_agg_df_Bow$Genus, levels=c("Other", "Buchnera","Candidatus Cardinium", "Candidatus Hamiltonella", "Candidatus Megaira", "Rickettsia", "Spiroplasma", "Wolbachia"), labels=c( "Non-endosymbionts", "Buchnera","Candidatus Cardinium", "Candidatus Hamiltonella", "Candidatus Megaira", "Rickettsia", "Spiroplasma", "Wolbachia"))
...
endo_plot_Bow <- ggplot(endo_agg_df_Bow, aes(x=Site_code, y=Abundance, fill=Genus)) +
  ...
  theme(axis.title.y=element_text(size=30),
        ...
        axis.text.x=element_text(angle=45, hjust=1, size=24, color="black", face=colorado(endo_agg_df_Bow$Site_code, c("GRVBR", "PMF"))),
                        axis.title.x=element_text(size=30))+
  scale_fill_manual(name="Genus", values=c("#808080", "tan1", "#3DED97", "#9867C5", "#FC4C4E", "skyblue1", "lightgoldenrod1","lightpink1"))+
  scale_y_continuous(label = scales::percent)+
  xlab("Site")+
  rremove("grid")
endo_plot_Bow
...
endosymbionts_ACWA <- sort_genus_ACWA$Genus[c(376:378, 1702:1704, 2128:2130)]
...
endo_agg_df_ACWA$Genus<-factor(endo_agg_df_ACWA$Genus, levels=c("Other", "Candidatus Megaira",  "Rickettsia", "Wolbachia"), labels=c("Non-endosymbionts", "Candidatus Megaira", "Rickettsia", "Wolbachia"))
...
endo_plot_ACWA <- ggplot(endo_agg_df_ACWA, aes(x=Site_code, y=Abundance, fill=Genus)) +
  ...
  theme(axis.title.y=element_text(size=30),
        ...
        axis.text.x=element_text(angle=45, hjust=1, size=24, color="black"),
                        axis.title.x=element_text(size=30))+
  scale_fill_manual(name="Genus", values=c("honeydew4", "#FC4C4E","skyblue1","lightpink1"))+
  scale_y_continuous(label = scales::percent)+
  xlab("Site")+
  rremove("grid")
endo_plot_ACWA
...
## Statistically testing endosymbiont bacteria relative abundance across sites
```{r}
Bow_ps<-subset_samples(sample_ps, Stream_Type=="Bow River")
...
hydro_glom_Bow = tax_glom(hydro_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

hydro_Bow_df<-psmelt(hydro_glom_Bow)

endo_hydro<-subset(hydro_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")
endo_hydro<-prune_taxa(taxa_sums(endo_hydro) > 0, endo_hydro) 
endo_hydro #48 taxa, 276 samples

shapiro.test(endo_hydro$Abundance) #p<0.001 not normal
...
kruskal.test(data=endo_hydro, Abundance ~ Site) #p = 0.0011 sig. diff
dunnTest(data=endo_hydro, Abundance ~ Site)
#GRVBR - COCH Z = 3.22, P. adj = 0.012
#PMF - COCH Z = 3.60, P. adj = 0.0032


#### Heptageniidae Bow ####

hep_Bow<-subset_samples(Bow_ps, Family=="Heptageniidae")
hep_Bow<-prune_taxa(taxa_sums(hep_Bow) > 0, hep_Bow) 

hep_glom_Bow = tax_glom(hep_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

hep_Bow_df<-psmelt(hep_glom_Bow)

endo_hep<-subset(hep_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")
endo_hep<-prune_taxa(taxa_sums(endo_hep) > 0, endo_hep) 
endo_hep 

shapiro.test(endo_hep$Abundance) #p<0.001 not normal

#Listing the top abundant phyla
endo_hep %>%
    group_by(Genus, Site) %>%
  dplyr::select(c("Genus", "Abundance", "Site")) %>%
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>%
  arrange(desc(Mean)) %>%
    ungroup 
...
rick_endo_hep<-subset(endo_hep, Genus=="Rickettsia")
shapiro.test(rick_endo_hep$Abundance) #P<0.001
kruskal.test(data=rick_endo_hep, Abundance ~ Site) #p = 0.149 no sig. diff
dunnTest(data=rick_endo_hep, Abundance ~ Site)

#Wolbachia

wol_endo_hep<-subset(endo_hep, Genus=="Wolbachia")
shapiro.test(wol_endo_hep$Abundance) #P<0.001
kruskal.test(data=wol_endo_hep, Abundance ~ Site) #p = 0.0023 sig. diff
dunnTest(data=wol_endo_hep, Abundance ~ Site) #No significant comparisons


#### Chironomidae ####

chiro_Bow<-subset_samples(Bow_ps, Family=="Chironomidae")
chiro_Bow<-prune_taxa(taxa_sums(chiro_Bow) > 0, chiro_Bow) 

chiro_glom_Bow = tax_glom(chiro_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

chiro_Bow_df<-psmelt(chiro_glom_Bow)

endo_chiro<-subset(chiro_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")
endo_chiro<-prune_taxa(taxa_sums(endo_chiro) > 0, endo_chiro) 

shapiro.test(endo_chiro$Abundance) #p<0.001 not normal
...
perl_Bow<-subset_samples(Bow_ps, Family=="Perlidae")
perl_Bow<-prune_taxa(taxa_sums(perl_Bow) > 0, perl_Bow) 

perl_glom_Bow = tax_glom(perl_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

perl_Bow_df<-psmelt(perl_glom_Bow)

endo_perl<-subset(perl_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")
endo_perl<-prune_taxa(taxa_sums(endo_perl) > 0, endo_perl) 

shapiro.test(endo_perl$Abundance) #p<0.001 not normal
...
trichop_Bow<-subset_samples(Bow_ps, Family=="Trichoptera")
trichop_Bow<-prune_taxa(taxa_sums(trichop_Bow) > 0, trichop_Bow) 

trichop_glom_Bow = tax_glom(trichop_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

trichop_Bow_df<-psmelt(trichop_glom_Bow)

endo_trichop<-subset(trichop_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")
endo_trichop<-prune_taxa(taxa_sums(endo_trichop) > 0, endo_trichop) 

shapiro.test(endo_trichop$Abundance) #p<0.001 not normal
...
rick_endo_trichop<-subset(endo_trichop, Genus=="Rickettsia")
shapiro.test(rick_endo_trichop$Abundance) #P<0.001
kruskal.test(data=rick_endo_trichop, Abundance ~ Site) #p = 0.544 no sig. diff
dunnTest(data=rick_endo_trichop, Abundance ~ Site)
...
ephem_glom_Bow = tax_glom(ephem_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

ephem_Bow_df<-psmelt(ephem_glom_Bow)

endo_ephem<-subset(ephem_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")

shapiro.test(endo_ephem$Abundance) #p<0.001 not normal
...
rick_endo_ephem<-subset(endo_ephem, Genus=="Rickettsia")
shapiro.test(rick_endo_ephem$Abundance) #P<0.001
kruskal.test(data=rick_endo_ephem, Abundance ~ Site) #p = 0.865 no sig. diff
dunnTest(data=rick_endo_ephem, Abundance ~ Site)


#### Diptera ####

dip_Bow<-subset_samples(Bow_ps, Family=="Diptera")
dip_Bow<-prune_taxa(taxa_sums(dip_Bow) > 0, dip_Bow) 

dip_glom_Bow = tax_glom(dip_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

dip_Bow_df<-psmelt(dip_glom_Bow)

endo_dip<-subset(dip_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Diplorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")

shapiro.test(endo_dip$Abundance) #p<0.001 not normal
...
rick_endo_dip<-subset(endo_dip, Genus=="Rickettsia")
shapiro.test(rick_endo_dip$Abundance) #P<0.001
kruskal.test(data=rick_endo_dip, Abundance ~ Site) #p = 0.053 no sig. diff
dunnTest(data=rick_endo_dip, Abundance ~ Site) #COCH - CUSHBR

#Wolbachia

wol_endo_dip<-subset(endo_dip, Genus=="Wolbachia")
shapiro.test(wol_endo_dip$Abundance) #P<0.001
kruskal.test(data=wol_endo_dip, Abundance ~ Site) #p = 0.061 no sig. diff
dunnTest(data=wol_endo_dip, Abundance ~ Site) 

#Spiroplasma

spiro_endo_dip<-subset(endo_dip, Genus=="Spiroplasma")
shapiro.test(spiro_endo_dip$Abundance) #P<0.001
kruskal.test(data=spiro_endo_dip, Abundance ~ Site) #p = 0.175 no sig. diff
dunnTest(data=spiro_endo_dip, Abundance ~ Site) 


#### Araneidae ####

aran_Bow<-subset_samples(Bow_ps, Family=="Araneidae")
aran_Bow<-prune_taxa(taxa_sums(aran_Bow) > 0, aran_Bow) 

aran_glom_Bow = tax_glom(aran_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

aran_Bow_df<-psmelt(aran_glom_Bow)

endo_aran<-subset(aran_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="aranlorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")

shapiro.test(endo_aran$Abundance) #p<0.001 not normal
...
rick_endo_aran<-subset(endo_aran, Genus=="Rickettsia")
shapiro.test(rick_endo_aran$Abundance) #P<0.001
kruskal.test(data=rick_endo_aran, Abundance ~ Site) #p = 0.043 sig. diff
dunnTest(data=rick_endo_aran, Abundance ~ Site) #no sig comparisons

#Wolbachia

wol_endo_aran<-subset(endo_aran, Genus=="Wolbachia")
shapiro.test(wol_endo_aran$Abundance) #P<0.001
kruskal.test(data=wol_endo_aran, Abundance ~ Site) #p = 0.611
dunnTest(data=wol_endo_aran, Abundance ~ Site) 

#Spiroplasma

spiro_endo_aran<-subset(endo_aran, Genus=="Spiroplasma")
shapiro.test(spiro_endo_aran$Abundance) #P<0.001
kruskal.test(data=spiro_endo_aran, Abundance ~ Site) #p = 0.0227
dunnTest(data=spiro_endo_aran, Abundance ~ Site) #no sig comparisons

#Candidatus Hamiltonella

cand_H_endo_aran<-subset(endo_aran, Genus=="Candidatus Hamiltonella")
shapiro.test(cand_H_endo_aran$Abundance) #P<0.001
kruskal.test(data=cand_H_endo_aran, Abundance ~ Site) #p = 0.495
dunnTest(data=cand_H_endo_aran, Abundance ~ Site) #no sig comparisons


#### Tetragnathidae ####

tetra_Bow<-subset_samples(Bow_ps, Family=="Tetragnathidae")
tetra_Bow<-prune_taxa(taxa_sums(tetra_Bow) > 0, tetra_Bow) 

tetra_glom_Bow = tax_glom(tetra_Bow, "Genus") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

tetra_Bow_df<-psmelt(tetra_glom_Bow)

endo_tetra<-subset(tetra_Bow_df, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="tetralorickettsia" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira"| Genus=="Rickettsiella")

shapiro.test(endo_tetra$Abundance) #p<0.001 not normal
...
rick_endo_tetra<-subset(endo_tetra, Genus=="Rickettsia")
shapiro.test(rick_endo_tetra$Abundance) #P<0.001
kruskal.test(data=rick_endo_tetra, Abundance ~ Site) #p = 0.081 sig. diff
dunnTest(data=rick_endo_tetra, Abundance ~ Site) #no sig comparisons

#Wolbachia

wol_endo_tetra<-subset(endo_tetra, Genus=="Wolbachia")
shapiro.test(wol_endo_tetra$Abundance) #P<0.001
kruskal.test(data=wol_endo_tetra, Abundance ~ Site) #p = 0.061
dunnTest(data=wol_endo_tetra, Abundance ~ Site) #COCH - SUN

#Spiroplasma

spiro_endo_tetra<-subset(endo_tetra, Genus=="Spiroplasma")
shapiro.test(spiro_endo_tetra$Abundance) #P<0.001
kruskal.test(data=spiro_endo_tetra, Abundance ~ Site) #p = 0.0029
dunnTest(data=spiro_endo_tetra, Abundance ~ Site) #CUSHBR - PMF

#Candidatus Cardinium

cand_C_endo_tetra<-subset(endo_tetra, Genus=="Candidatus Cardinium")
shapiro.test(cand_C_endo_tetra$Abundance) #P<0.001
kruskal.test(data=cand_C_endo_tetra, Abundance ~ Site) #p = 0.048
dunnTest(data=cand_C_endo_tetra, Abundance ~ Site) #no sig comparisons
```
...
tax_select(sample_ps, tax_list="Diplorickettsia") #9 taxa

endo<-subset_taxa(sample_ps, Genus=="Buchnera" | Genus=="Candidatus Cardinium" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Megaira" | Genus=="Diplorickettsia")
endo<-prune_taxa(taxa_sums(endo) > 0, endo) 
endo #48 taxa 
...
endo_Bow<-subset_taxa(Bow_ps, Genus=="Buchnera" | Genus=="Rickettsia" | Genus =="Wolbachia" | Genus=="Spiroplasma" | Genus=="Candidatus Hamiltonella" | Genus=="Candidatus Cardinium" | Genus=="Candidatus Megaira" | Genus=="Diplorickettsia")
endo_Bow<-prune_taxa(taxa_sums(endo_Bow) > 0, endo_Bow) 
endo_Bow #48 taxa, 276 samples
...
# Percent of total reads (Bow River and ACWA)

effluent_bac<-subset_taxa(sample_ps, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" |  Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_bac<-prune_samples(sample_sums(effluent_bac) > 0, effluent_bac) 
effluent_bac #1258 taxa 316 samples

((sum(sample_sums(effluent_bac)))/(sum(sample_sums(sample_ps)))*100) #8.10%

# Percent of total Bow River reads #

effluent_Bow<-subset_taxa(Bow_ps, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" | Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_Bow<-prune_samples(sample_sums(effluent_Bow) > 0, effluent_Bow) 
effluent_Bow

((sum(sample_sums(effluent_Bow)))/(sum(sample_sums(Bow_ps)))*100) #8.20%


# Percent of total Bow larvae reads #

Bow_lar<-subset_samples(Bow_ps, Stage=="Larvae")

effluent_Bow_lar<-subset_taxa(Bow_lar, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" | Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_Bow_lar<-prune_samples(sample_sums(effluent_Bow_lar) > 0, effluent_Bow_lar) 
effluent_Bow_lar

((sum(sample_sums(effluent_Bow_lar)))/(sum(sample_sums(Bow_lar)))*100) #10.1%%


# Percent of total Bow adults reads #

Bow_ad<-subset_samples(Bow_ps, Stage=="Adults")

effluent_Bow_ad<-subset_taxa(Bow_ad, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" | Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_Bow_ad<-prune_samples(sample_sums(effluent_Bow_ad) > 0, effluent_Bow_ad) 
effluent_Bow_ad

((sum(sample_sums(effluent_Bow_ad)))/(sum(sample_sums(Bow_ad)))*100) #16.6%%



# Percent of total Bow spiders reads #

Bow_spi<-subset_samples(Bow_ps, Stage=="Spiders")

effluent_Bow_spi<-subset_taxa(Bow_spi, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" | Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_Bow_spi<-prune_samples(sample_sums(effluent_Bow_spi) > 0, effluent_Bow_spi) 
effluent_Bow_spi

((sum(sample_sums(effluent_Bow_spi)))/(sum(sample_sums(Bow_spi)))*100) #2.17%%


# Percent of total ACWA reads #

effluent_ACWA<-subset_taxa(ACWA_ps, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" | Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_ACWA<-prune_samples(sample_sums(effluent_ACWA) > 0, effluent_ACWA) 
effluent_ACWA

((sum(sample_sums(effluent_ACWA)))/(sum(sample_sums(ACWA_ps)))*100) #7.69%

# Percent of total ACWA larvae reads #

ACWA_lar<-subset_samples(ACWA_ps, Stage=="Larvae")

effluent_ACWA_lar<-subset_taxa(ACWA_lar, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" |  Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_ACWA_lar<-prune_samples(sample_sums(effluent_ACWA_lar) > 0, effluent_ACWA_lar) 
effluent_ACWA_lar

((sum(sample_sums(effluent_ACWA_lar)))/(sum(sample_sums(ACWA_lar)))*100) #9.32%%


# Percent of total ACWA adult reads #

nsamples(ACWA_ad<-subset_samples(ACWA_ps, Stage=="Adults"))

effluent_ACWA_ad<-subset_taxa(ACWA_ad, Genus == "AAP99" | Genus=="Achromobacter" | Genus=="Acidovorax" | Genus=="Acinetobacter"| Genus=="Aeromonas" | Genus=="Alcaligenes" | Genus=="Amaricoccus" | Genus=="Anaerovorax" |Genus=="Arcticibacter"|Genus=="Aridibacter"| Genus=="Arthrobacter" | Genus=="Asticcacaulis"| Genus=="Azoarcus" | Genus=="Azospira"| Genus=="Bacillus" |Genus=="Bacteroides"| Genus=="Bauldia" | Genus=="Bdellovibrio"| Genus=="Blastocatella" | Genus=="Blastomonas"| Genus=="Blautia" | Genus=="Blvii28 wastewater-sludge group"| Genus=="Bordetella" |Genus== "Brachybacterium"| Genus=="Bryobacter"| Genus=="Candidatus Accumulibacter" | Genus== "Candidatus Amoebophilus" | Genus=="Candidatus Anammoximicrobium"| Genus=="Candidatus Competibacter" | Genus=="Candidatus Microthrix" | Genus=="Candidatus Nitrotoga" | Genus=="Cetobacterium" | Genus=="Chryseobacterium" | Genus=="Citrobacter" | Genus=="Cloacibacterium" | Genus=="Clostridium sensu stricto 1" | Genus=="Clostridium sensu stricto 2" | Genus=="Clostridium sensu stricto 3"| Genus=="Clostridium sensu stricto 5" | Genus=="Clostridium sensu stricto 8"| Genus=="Clostridium sensu stricto 9"| Genus=="Clostridium sensu stricto 10"| Genus=="Clostridium sensu stricto 13"| Genus=="Clostridium sensu stricto 14" | Genus=="Comamonas" | Genus=="Corynebacterium" | Genus=="Cytophaga" | Genus=="Dechloromonas" | Genus=="Defluviicoccus" | Genus=="Desulfatiglans" | Genus=="Desulfovibrio" | Genus=="Devosia"| Genus=="Ensifer"| Genus=="Enterococcus" | Genus=="Escherichia-Shigella" | Genus=="Faecalibacterium" | Genus=="Faecalitalea" | Genus=="Ferruginibacter" | Genus=="Flexibacter"| Genus=="Fluviicola"| Genus=="Gemmata" |Genus=="Gordonia"| Genus=="Haliscomenobacter" | Genus=="Helicobacter" | Genus=="Hydrogenophaga" | Genus=="Hyphomicrobium" | Genus=="Hyphomonas"| Genus=="Ignavibacterium"| Genus=="Isosphaera" | Genus=="Klebsiella"| Genus=="Lachnospira" | Genus=="Lachnobacillus" | Genus=="Legionella" | Genus=="Methylocystis" | Genus=="Methylotenera"| Genus=="Micrococcus" | Genus=="Moraxella" | Genus=="Mycobacterium" | Genus=="Neisseria" | Genus=="Nitrobacter"| Genus=="Nitrosomonas" | Genus=="Nitrospira" | Genus=="Nocardioides" | Genus=="Novosphingobium" | Genus=="Ottawia" | Genus=="Paludibacter" | Genus=="Pantoea"| Genus=="Parabacteroides"| Genus=="Paracoccus" | Genus=="Pseudomonas" |  Genus=="Ralstonia" | Genus=="Rhodobacter" | Genus=="Rhodococcus"| Genus=="Romboutsia" | Genus=="Roseburia"| Genus=="Ruminococcus" | Genus=="Runella" | Genus=="Serratia" | Genus=="Shinella" | Genus=="Smithella"| Genus=="Staphylococcus" | Genus=="Stenotrophobacter" | Genus=="Stenotrophomonas"| Genus=="Streptococcus"| Genus=="Sulfuritalea" | Genus=="Syntrophobacter" | Genus=="Syntrophomonas"| Genus=="Syntrophus"| Genus=="Terrimonas" | Genus=="Thermomonas" | Genus=="Trichococcus" | Genus=="Xanthomonas"| Genus=="Zoogloea")
effluent_ACWA_ad<-prune_samples(sample_sums(effluent_ACWA_ad) > 0, effluent_ACWA_ad) 
effluent_ACWA_ad

((sum(sample_sums(effluent_ACWA_ad)))/(sum(sample_sums(ACWA_ad)))*100) #1.87%%
...
nsamples(cochrane_lar<-subset_samples(sample_ps, Site=="Cochrane" & Stage=="Larvae")) #28

coch_genus_lar = tax_glom(cochrane_lar, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
coch_genus_lar<-prune_taxa(taxa_sums(coch_genus_lar) > 0, coch_genus_lar)

coch_genus_df_lar <- psmelt(coch_genus_lar) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

coch_eff_lar<- coch_genus_df_lar %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas" ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(coch_eff_lar)

coch_eff_lar$SD_squared ='^'(coch_eff_lar$SD,2)
coch_eff_lar$SD_squared_divided =(coch_eff_lar$SD_squared/28) 
  
top_means_eff_coch_lar<-aggregate(Mean ~ Genus, data = coch_eff_lar, FUN = sum)
top_SD_eff_coch_lar<-aggregate(SD_squared_divided ~ Genus, data = coch_eff_lar, FUN = sum)

top_eff_coch_lar<-merge(top_means_eff_coch_lar, top_SD_eff_coch_lar, by=c("Genus"))
top_eff_coch_lar$SD_Final<-sqrt(top_eff_coch_lar$SD_squared_divided)
View(top_eff_coch_lar)

# Finding prevalence of effluent bacteria #

coch_genus_df_lar <- psmelt(coch_genus_lar) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise((Prevalence = count(Abundance > 0)/n())*100)
  
  coch_eff_lar<- coch_genus_df_lar %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas" ,"Pseudorhodobacter" ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(coch_eff_lar)
...
nsamples(cochrane_ad<-subset_samples(sample_ps, Site=="Cochrane" & Stage=="Adults")) #21

coch_genus_ad = tax_glom(cochrane_ad, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
coch_genus_ad<-prune_taxa(taxa_sums(coch_genus_ad) > 0, coch_genus_ad)

coch_genus_df_ad <- psmelt(coch_genus_ad) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

coch_eff_ad<- coch_genus_df_ad %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas" ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(coch_eff_ad)

coch_eff_ad$SD_squared ='^'(coch_eff_ad$SD,2)
coch_eff_ad$SD_squared_divided =(coch_eff_ad$SD_squared/21) 
  
top_means_eff_coch_ad<-aggregate(Mean ~ Genus, data = coch_eff_ad, FUN = sum)
top_SD_eff_coch_ad<-aggregate(SD_squared_divided ~ Genus, data = coch_eff_ad, FUN = sum)

top_eff_coch_ad<-merge(top_means_eff_coch_ad, top_SD_eff_coch_ad, by=c("Genus"))
top_eff_coch_ad$SD_Final<-sqrt(top_eff_coch_ad$SD_squared_divided)
View(top_eff_coch_ad)
...
nsamples(cochrane_spi<-subset_samples(sample_ps, Site=="Cochrane" & Stage=="Spiders")) #16

coch_genus_spi = tax_glom(cochrane_spi, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
coch_genus_spi<-prune_taxa(taxa_sums(coch_genus_spi) > 0, coch_genus_spi)

coch_genus_df_spi <- psmelt(coch_genus_spi) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

coch_eff_spi<- coch_genus_df_spi %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas" ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(coch_eff_spi)

coch_eff_spi$SD_squared ='^'(coch_eff_spi$SD,2)
coch_eff_spi$SD_squared_divided =(coch_eff_spi$SD_squared/16) 
  
top_means_eff_coch_spi<-aggregate(Mean ~ Genus, data = coch_eff_spi, FUN = sum)
top_SD_eff_coch_spi<-aggregate(SD_squared_divided ~ Genus, data = coch_eff_spi, FUN = sum)

top_eff_coch_spi<-merge(top_means_eff_coch_spi, top_SD_eff_coch_spi, by=c("Genus"))
top_eff_coch_spi$SD_Final<-sqrt(top_eff_coch_spi$SD_squared_divided)
View(top_eff_coch_spi)
...
# Larvae #
nsamples(sunalta_lar<-subset_samples(sample_ps, Site=="Sunalta" & Stage=="Larvae")) #15

sun_genus_lar = tax_glom(sunalta_lar, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
sun_genus_lar<-prune_taxa(taxa_sums(sun_genus_lar) > 0, sun_genus_lar)

sun_genus_df_lar <- psmelt(sun_genus_lar) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

sun_eff_lar<- sun_genus_df_lar %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(sun_eff_lar)

sun_eff_lar$SD_squared ='^'(sun_eff_lar$SD,2)
sun_eff_lar$SD_squared_divided =(sun_eff_lar$SD_squared/15) 
  
top_means_eff_sun_lar<-aggregate(Mean ~ Genus, data = sun_eff_lar, FUN = sum)
top_SD_eff_sun_lar<-aggregate(SD_squared_divided ~ Genus, data = sun_eff_lar, FUN = sum)

top_eff_sun_lar<-merge(top_means_eff_sun_lar, top_SD_eff_sun_lar, by=c("Genus"))
top_eff_sun_lar$SD_Final<-sqrt(top_eff_sun_lar$SD_squared_divided)
View(top_eff_sun_lar)
...
nsamples(sunalta_ad<-subset_samples(sample_ps, Site=="Sunalta" & Stage=="Adults")) #16

sun_genus_ad = tax_glom(sunalta_ad, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
sun_genus_ad<-prune_taxa(taxa_sums(sun_genus_ad) > 0, sun_genus_ad)

sun_genus_df_ad <- psmelt(sun_genus_ad) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

sun_eff_ad<- sun_genus_df_ad %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas" ,"Pseudorhodobacter" ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(sun_eff_ad)

sun_eff_ad$SD_squared ='^'(sun_eff_ad$SD,2)
sun_eff_ad$SD_squared_divided =(sun_eff_ad$SD_squared/16) 
  
top_means_eff_sun_ad<-aggregate(Mean ~ Genus, data = sun_eff_ad, FUN = sum)
top_SD_eff_sun_ad<-aggregate(SD_squared_divided ~ Genus, data = sun_eff_ad, FUN = sum)

top_eff_sun_ad<-merge(top_means_eff_sun_ad, top_SD_eff_sun_ad, by=c("Genus"))
top_eff_sun_ad$SD_Final<-sqrt(top_eff_sun_ad$SD_squared_divided)
View(top_eff_sun_ad)
...
nsamples(sunalta_spi<-subset_samples(sample_ps, Site=="Sunalta" & Stage=="Spiders")) #15

sun_genus_spi = tax_glom(sunalta_spi, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
sun_genus_spi<-prune_taxa(taxa_sums(sun_genus_spi) > 0, sun_genus_spi)

sun_genus_df_spi <- psmelt(sun_genus_spi) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

sun_eff_spi<- sun_genus_df_spi %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas" ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(sun_eff_spi)

sun_eff_spi$SD_squared ='^'(sun_eff_spi$SD,2)
sun_eff_spi$SD_squared_divided =(sun_eff_spi$SD_squared/15) 
  
top_means_eff_sun_spi<-aggregate(Mean ~ Genus, data = sun_eff_spi, FUN = sum)
top_SD_eff_sun_spi<-aggregate(SD_squared_divided ~ Genus, data = sun_eff_spi, FUN = sum)

top_eff_sun_spi<-merge(top_means_eff_sun_spi, top_SD_eff_sun_spi, by=c("Genus"))
top_eff_sun_spi$SD_Final<-sqrt(top_eff_sun_spi$SD_squared_divided)
View(top_eff_sun_spi)
...
nsamples(cushbr_lar<-subset_samples(sample_ps, Site=="Cushing Bridge" & Stage=="Larvae")) #22

cushbr_genus_lar = tax_glom(cushbr_lar, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
cushbr_genus_lar<-prune_taxa(taxa_sums(cushbr_genus_lar) > 0, cushbr_genus_lar)

cushbr_genus_df_lar <- psmelt(cushbr_genus_lar) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

cushbr_eff_lar<- cushbr_genus_df_lar %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(cushbr_eff_lar)

cushbr_eff_lar$SD_squared ='^'(cushbr_eff_lar$SD,2)
cushbr_eff_lar$SD_squared_divided =(cushbr_eff_lar$SD_squared/22) 
  
top_means_eff_cushbr_lar<-aggregate(Mean ~ Genus, data = cushbr_eff_lar, FUN = sum)
top_SD_eff_cushbr_lar<-aggregate(SD_squared_divided ~ Genus, data = cushbr_eff_lar, FUN = sum)

top_eff_cushbr_lar<-merge(top_means_eff_cushbr_lar, top_SD_eff_cushbr_lar, by=c("Genus"))
top_eff_cushbr_lar$SD_Final<-sqrt(top_eff_cushbr_lar$SD_squared_divided)
View(top_eff_cushbr_lar)
...
nsamples(cushbr_ad<-subset_samples(sample_ps, Site=="Cushing Bridge" & Stage=="Adults")) #24

cushbr_genus_ad = tax_glom(cushbr_ad, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
cushbr_genus_ad<-prune_taxa(taxa_sums(cushbr_genus_ad) > 0, cushbr_genus_ad)

cushbr_genus_df_ad <- psmelt(cushbr_genus_ad) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

cushbr_eff_ad<- cushbr_genus_df_ad %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(cushbr_eff_ad)

cushbr_eff_ad$SD_squared ='^'(cushbr_eff_ad$SD,2)
cushbr_eff_ad$SD_squared_divided =(cushbr_eff_ad$SD_squared/24) 
  
top_means_eff_cushbr_ad<-aggregate(Mean ~ Genus, data = cushbr_eff_ad, FUN = sum)
top_SD_eff_cushbr_ad<-aggregate(SD_squared_divided ~ Genus, data = cushbr_eff_ad, FUN = sum)

top_eff_cushbr_ad<-merge(top_means_eff_cushbr_ad, top_SD_eff_cushbr_ad, by=c("Genus"))
top_eff_cushbr_ad$SD_Final<-sqrt(top_eff_cushbr_ad$SD_squared_divided)
View(top_eff_cushbr_ad)
...
nsamples(cush_spi<-subset_samples(sample_ps, Site=="Cushing Bridge" & Stage=="Spiders")) #15

cush_genus_spi = tax_glom(cush_spi, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
cush_genus_spi<-prune_taxa(taxa_sums(cush_genus_spi) > 0, cush_genus_spi)

cush_genus_df_spi <- psmelt(cush_genus_spi) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

cush_eff_spi<- cush_genus_df_spi %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(cush_eff_spi)

cush_eff_spi$SD_squared ='^'(cush_eff_spi$SD,2)
cush_eff_spi$SD_squared_divided =(cush_eff_spi$SD_squared/15) 
  
top_means_eff_cush_spi<-aggregate(Mean ~ Genus, data = cush_eff_spi, FUN = sum)
top_SD_eff_cush_spi<-aggregate(SD_squared_divided ~ Genus, data = cush_eff_spi, FUN = sum)

top_eff_cush_spi<-merge(top_means_eff_cush_spi, top_SD_eff_cush_spi, by=c("Genus"))
top_eff_cush_spi$SD_Final<-sqrt(top_eff_cush_spi$SD_squared_divided)
View(top_eff_cush_spi)
...
nsamples(grvbr_lar<-subset_samples(sample_ps, Site=="Graves Bridge" & Stage=="Larvae")) #16

grvbr_genus_lar = tax_glom(grvbr_lar, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
grvbr_genus_lar<-prune_taxa(taxa_sums(grvbr_genus_lar) > 0, grvbr_genus_lar)

grvbr_genus_df_lar <- psmelt(grvbr_genus_lar) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

grvbr_eff_lar<- grvbr_genus_df_lar %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(grvbr_eff_lar)

grvbr_eff_lar$SD_squared ='^'(grvbr_eff_lar$SD,2)
grvbr_eff_lar$SD_squared_divided =(grvbr_eff_lar$SD_squared/16) 
  
top_means_eff_grvbr_lar<-aggregate(Mean ~ Genus, data = grvbr_eff_lar, FUN = sum)
top_SD_eff_grvbr_lar<-aggregate(SD_squared_divided ~ Genus, data = grvbr_eff_lar, FUN = sum)

top_eff_grvbr_lar<-merge(top_means_eff_grvbr_lar, top_SD_eff_grvbr_lar, by=c("Genus"))
top_eff_grvbr_lar$SD_Final<-sqrt(top_eff_grvbr_lar$SD_squared_divided)
View(top_eff_grvbr_lar)
...
nsamples(grvbr_ad<-subset_samples(sample_ps, Site=="Graves Bridge" & Stage=="Adults")) #15

grvbr_genus_ad = tax_glom(grvbr_ad, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
grvbr_genus_ad<-prune_taxa(taxa_sums(grvbr_genus_ad) > 0, grvbr_genus_ad)

grvbr_genus_df_ad <- psmelt(grvbr_genus_ad) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

grvbr_eff_ad<- grvbr_genus_df_ad %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(grvbr_eff_ad)

grvbr_eff_ad$SD_squared ='^'(grvbr_eff_ad$SD,2)
grvbr_eff_ad$SD_squared_divided =(grvbr_eff_ad$SD_squared/15) 
  
top_means_eff_grvbr_ad<-aggregate(Mean ~ Genus, data = grvbr_eff_ad, FUN = sum)
top_SD_eff_grvbr_ad<-aggregate(SD_squared_divided ~ Genus, data = grvbr_eff_ad, FUN = sum)

top_eff_grvbr_ad<-merge(top_means_eff_grvbr_ad, top_SD_eff_grvbr_ad, by=c("Genus"))
top_eff_grvbr_ad$SD_Final<-sqrt(top_eff_grvbr_ad$SD_squared_divided)
View(top_eff_grvbr_ad)
...
nsamples(grvbr_spi<-subset_samples(sample_ps, Site=="Graves Bridge" & Stage=="Spiders")) #16

grv_genus_spi = tax_glom(grvbr_spi, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
grv_genus_spi<-prune_taxa(taxa_sums(grv_genus_spi) > 0, grv_genus_spi)

grv_genus_df_spi <- psmelt(grv_genus_spi) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

grv_eff_spi<- grv_genus_df_spi %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(grv_eff_spi)

grv_eff_spi$SD_squared ='^'(grv_eff_spi$SD,2)
grv_eff_spi$SD_squared_divided =(grv_eff_spi$SD_squared/16) 
  
top_means_eff_grv_spi<-aggregate(Mean ~ Genus, data = grv_eff_spi, FUN = sum)
top_SD_eff_grv_spi<-aggregate(SD_squared_divided ~ Genus, data = grv_eff_spi, FUN = sum)

top_eff_grv_spi<-merge(top_means_eff_grv_spi, top_SD_eff_grv_spi, by=c("Genus"))
top_eff_grv_spi$SD_Final<-sqrt(top_eff_grv_spi$SD_squared_divided)
View(top_eff_grv_spi)
...
nsamples(pmf_lar<-subset_samples(sample_ps, Site=="Policeman Flats" & Stage=="Larvae")) #29

pmf_genus_lar = tax_glom(pmf_lar, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
pmf_genus_lar<-prune_taxa(taxa_sums(pmf_genus_lar) > 0, pmf_genus_lar)

pmf_genus_df_lar <- psmelt(pmf_genus_lar) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

pmf_eff_lar<- pmf_genus_df_lar %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(pmf_eff_lar)

pmf_eff_lar$SD_squared ='^'(pmf_eff_lar$SD,2)
pmf_eff_lar$SD_squared_divided =(pmf_eff_lar$SD_squared/29) 
  
top_means_eff_pmf_lar<-aggregate(Mean ~ Genus, data = pmf_eff_lar, FUN = sum)
top_SD_eff_pmf_lar<-aggregate(SD_squared_divided ~ Genus, data = pmf_eff_lar, FUN = sum)

top_eff_pmf_lar<-merge(top_means_eff_pmf_lar, top_SD_eff_pmf_lar, by=c("Genus"))
top_eff_pmf_lar$SD_Final<-sqrt(top_eff_pmf_lar$SD_squared_divided)
View(top_eff_pmf_lar)
...
nsamples(pmf_ad<-subset_samples(sample_ps, Site=="Policeman Flats" & Stage=="Adults")) #14

pmf_genus_ad = tax_glom(pmf_ad, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
pmf_genus_ad<-prune_taxa(taxa_sums(pmf_genus_ad) > 0, pmf_genus_ad)

pmf_genus_df_ad <- psmelt(pmf_genus_ad) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

pmf_eff_ad<- pmf_genus_df_ad %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(pmf_eff_ad)

pmf_eff_ad$SD_squared ='^'(pmf_eff_ad$SD,2)
pmf_eff_ad$SD_squared_divided =(pmf_eff_ad$SD_squared/14) 
  
top_means_eff_pmf_ad<-aggregate(Mean ~ Genus, data = pmf_eff_ad, FUN = sum)
top_SD_eff_pmf_ad<-aggregate(SD_squared_divided ~ Genus, data = pmf_eff_ad, FUN = sum)

top_eff_pmf_ad<-merge(top_means_eff_pmf_ad, top_SD_eff_pmf_ad, by=c("Genus"))
top_eff_pmf_ad$SD_Final<-sqrt(top_eff_pmf_ad$SD_squared_divided)
View(top_eff_pmf_ad)
...
nsamples(pmf_spi<-subset_samples(sample_ps, Site=="Policeman Flats" & Stage=="Spiders")) #14

pmf_genus_spi = tax_glom(pmf_spi, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
pmf_genus_spi<-prune_taxa(taxa_sums(pmf_genus_spi) > 0, pmf_genus_spi)

pmf_genus_df_spi <- psmelt(pmf_genus_spi) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

pmf_eff_spi<- pmf_genus_df_spi %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(pmf_eff_spi)

pmf_eff_spi$SD_squared ='^'(pmf_eff_spi$SD,2)
pmf_eff_spi$SD_squared_divided =(pmf_eff_spi$SD_squared/14) 
  
top_means_eff_pmf_spi<-aggregate(Mean ~ Genus, data = pmf_eff_spi, FUN = sum)
top_SD_eff_pmf_spi<-aggregate(SD_squared_divided ~ Genus, data = pmf_eff_spi, FUN = sum)

top_eff_pmf_spi<-merge(top_means_eff_pmf_spi, top_SD_eff_pmf_spi, by=c("Genus"))
top_eff_pmf_spi$SD_Final<-sqrt(top_eff_pmf_spi$SD_squared_divided)
View(top_eff_pmf_spi)
...
nsamples(REF<-subset_samples(sample_ps, Site=="BRR2")) #8

REF_genus = tax_glom(REF, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
REF_genus<-prune_taxa(taxa_sums(REF_genus) > 0, REF_genus)

REF_genus_df <- psmelt(REF_genus) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

REF_eff<- REF_genus_df %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(REF_eff)

REF_eff$SD_squared ='^'(REF_eff$SD,2)
REF_eff$SD_squared_divided =(REF_eff$SD_squared/8) 
  
top_means_eff_REF<-aggregate(Mean ~ Genus, data = REF_eff, FUN = sum)
top_SD_eff_REF<-aggregate(SD_squared_divided ~ Genus, data = REF_eff, FUN = sum)

top_eff_REF<-merge(top_means_eff_REF, top_SD_eff_REF, by=c("Genus"))
top_eff_REF$SD_Final<-sqrt(top_eff_REF$SD_squared_divided)
View(top_eff_REF)
...
nsamples(EFF5<-subset_samples(sample_ps, Site=="PCR1")) #24

# Larval Hydropsychidae #

nsamples(EFF5_hydro<-subset_samples(EFF5, Family=="Hydropsychidae")) #8

EFF5_hydro_genus = tax_glom(EFF5_hydro, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
EFF5_hydro_genus<-prune_taxa(taxa_sums(EFF5_hydro_genus) > 0, EFF5_hydro_genus)

EFF5_hydro_genus_df <- psmelt(EFF5_hydro_genus) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

EFF5_hydro_eff<- EFF5_hydro_genus_df %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(EFF5_hydro_eff)

EFF5_hydro_eff$SD_squared ='^'(EFF5_hydro_eff$SD,2)
EFF5_hydro_eff$SD_squared_divided =(EFF5_hydro_eff$SD_squared/8) 
  
top_means_eff_EFF5_hydro<-aggregate(Mean ~ Genus, data = EFF5_hydro_eff, FUN = sum)
top_SD_eff_EFF5_hydro<-aggregate(SD_squared_divided ~ Genus, data = EFF5_hydro_eff, FUN = sum)

top_eff_EFF5_hydro<-merge(top_means_eff_EFF5_hydro, top_SD_eff_EFF5_hydro, by=c("Genus"))
top_eff_EFF5_hydro$SD_Final<-sqrt(top_eff_EFF5_hydro$SD_squared_divided)
View(top_eff_EFF5_hydro)
...
nsamples(EFF5_baetid<-subset_samples(EFF5, Family=="Baetidae")) #8

EFF5_baetid_genus = tax_glom(EFF5_baetid, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
EFF5_baetid_genus<-prune_taxa(taxa_sums(EFF5_baetid_genus) > 0, EFF5_baetid_genus)

EFF5_baetid_genus_df <- psmelt(EFF5_baetid_genus) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

EFF5_baetid_eff<- EFF5_baetid_genus_df %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas" ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(EFF5_baetid_eff)

EFF5_baetid_eff$SD_squared ='^'(EFF5_baetid_eff$SD,2)
EFF5_baetid_eff$SD_squared_divided =(EFF5_baetid_eff$SD_squared/8) 
  
top_means_eff_EFF5_baetid<-aggregate(Mean ~ Genus, data = EFF5_baetid_eff, FUN = sum)
top_SD_eff_EFF5_baetid<-aggregate(SD_squared_divided ~ Genus, data = EFF5_baetid_eff, FUN = sum)

top_eff_EFF5_baetid<-merge(top_means_eff_EFF5_baetid, top_SD_eff_EFF5_baetid, by=c("Genus"))
top_eff_EFF5_baetid$SD_Final<-sqrt(top_eff_EFF5_baetid$SD_squared_divided)
View(top_eff_EFF5_baetid)
...
nsamples(EFF5_ephem<-subset_samples(EFF5, Family=="Ephemeroptera")) #8

EFF5_ephem_genus = tax_glom(EFF5_ephem, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
EFF5_ephem_genus<-prune_taxa(taxa_sums(EFF5_ephem_genus) > 0, EFF5_ephem_genus)

EFF5_ephem_genus_df <- psmelt(EFF5_ephem_genus) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

EFF5_ephem_eff<- EFF5_ephem_genus_df %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(EFF5_ephem_eff)

EFF5_ephem_eff$SD_squared ='^'(EFF5_ephem_eff$SD,2)
EFF5_ephem_eff$SD_squared_divided =(EFF5_ephem_eff$SD_squared/8) 
  
top_means_eff_EFF5_ephem<-aggregate(Mean ~ Genus, data = EFF5_ephem_eff, FUN = sum)
top_SD_eff_EFF5_ephem<-aggregate(SD_squared_divided ~ Genus, data = EFF5_ephem_eff, FUN = sum)

top_eff_EFF5_ephem<-merge(top_means_eff_EFF5_ephem, top_SD_eff_EFF5_ephem, by=c("Genus"))
top_eff_EFF5_ephem$SD_Final<-sqrt(top_eff_EFF5_ephem$SD_squared_divided)
View(top_eff_EFF5_ephem)
...
nsamples(EFF15<-subset_samples(sample_ps, Site=="PCR3")) #23

# Larval Hydropsychidae #

nsamples(EFF15_hydro<-subset_samples(EFF15, Family=="Hydropsychidae")) #8

EFF15_hydro_genus = tax_glom(EFF15_hydro, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
EFF15_hydro_genus<-prune_taxa(taxa_sums(EFF15_hydro_genus) > 0, EFF15_hydro_genus)

EFF15_hydro_genus_df <- psmelt(EFF15_hydro_genus) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

EFF15_hydro_eff<- EFF15_hydro_genus_df %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(EFF15_hydro_eff)

EFF15_hydro_eff$SD_squared ='^'(EFF15_hydro_eff$SD,2)
EFF15_hydro_eff$SD_squared_divided =(EFF15_hydro_eff$SD_squared/8) 
  
top_means_eff_EFF15_hydro<-aggregate(Mean ~ Genus, data = EFF15_hydro_eff, FUN = sum)
top_SD_eff_EFF15_hydro<-aggregate(SD_squared_divided ~ Genus, data = EFF15_hydro_eff, FUN = sum)

top_eff_EFF15_hydro<-merge(top_means_eff_EFF15_hydro, top_SD_eff_EFF15_hydro, by=c("Genus"))
top_eff_EFF15_hydro$SD_Final<-sqrt(top_eff_EFF15_hydro$SD_squared_divided)
View(top_eff_EFF15_hydro)
...
nsamples(EFF15_baetid<-subset_samples(EFF15, Family=="Baetidae")) #7

EFF15_baetid_genus = tax_glom(EFF15_baetid, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
EFF15_baetid_genus<-prune_taxa(taxa_sums(EFF15_baetid_genus) > 0, EFF15_baetid_genus)

EFF15_baetid_genus_df <- psmelt(EFF15_baetid_genus) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

EFF15_baetid_eff<- EFF15_baetid_genus_df %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(EFF15_baetid_eff)

EFF15_baetid_eff$SD_squared ='^'(EFF15_baetid_eff$SD,2)
EFF15_baetid_eff$SD_squared_divided =(EFF15_baetid_eff$SD_squared/7) 
  
top_means_eff_EFF15_baetid<-aggregate(Mean ~ Genus, data = EFF15_baetid_eff, FUN = sum)
top_SD_eff_EFF15_baetid<-aggregate(SD_squared_divided ~ Genus, data = EFF15_baetid_eff, FUN = sum)

top_eff_EFF15_baetid<-merge(top_means_eff_EFF15_baetid, top_SD_eff_EFF15_baetid, by=c("Genus"))
top_eff_EFF15_baetid$SD_Final<-sqrt(top_eff_EFF15_baetid$SD_squared_divided)
View(top_eff_EFF15_baetid)
...
nsamples(EFF15_ephem<-subset_samples(EFF15, Family=="Ephemeroptera")) #8

EFF15_ephem_genus = tax_glom(EFF15_ephem, "Genus") %>% 
  transform_sample_counts(function(x) {x * 100/sum(x)}) 
EFF15_ephem_genus<-prune_taxa(taxa_sums(EFF15_ephem_genus) > 0, EFF15_ephem_genus)

EFF15_ephem_genus_df <- psmelt(EFF15_ephem_genus) %>%  
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  dplyr::summarise(Mean = mean(Abundance), SD = sd(Abundance))

EFF15_ephem_eff<- EFF15_ephem_genus_df %>%
  mutate(Genus = fct_other(Genus, keep=c("AAP99","Achromobacter", "Acidovorax" ,"Acinetobacter","Aeromonas" ,"Alcaligenes" ,"Amaricoccus" ,"Anaerovorax","Arcticibacter","Aridibacter","Arthrobacter" ,"Asticcacaulis","Azoarcus" ,"Azospira","Bacillus" ,"Bacteroides","Bauldia" ,"Bdellovibrio","Blastocatella" ,"Blastomonas","Blautia" ,"Blvii28 wastewater-sludge group","Bordetella","Brachybacterium","Bryobacter","Candidatus Accumulibacter" , "Candidatus Amoebophilus" ,"Candidatus Anammoximicrobium","Candidatus Competibacter" ,"Candidatus Microthrix" ,"Candidatus Nitrotoga" ,"Cetobacterium" ,"Chryseobacterium" ,"Citrobacter" ,"Cloacibacterium" ,"Clostridium sensu stricto 1" ,"Clostridium sensu stricto 2" ,"Clostridium sensu stricto 3","Clostridium sensu stricto 5" ,"Clostridium sensu stricto 8","Clostridium sensu stricto 9","Clostridium sensu stricto 10","Clostridium sensu stricto 13","Clostridium sensu stricto 14" ,"Comamonas" ,"Corynebacterium" ,"Cytophaga" ,"Dechloromonas" ,"Defluviicoccus" ,"Desulfatiglans" ,"Desulfovibrio" ,"Devosia","Ensifer","Enterococcus" ,"Escherichia-Shigella" ,"Faecalibacterium" ,"Faecalitalea" ,"Ferruginibacter" ,"Flexibacter","Fluviicola","Gemmata","Gordonia","Haliscomenobacter" ,"Helicobacter" ,"Hydrogenophaga" ,"Hyphomicrobium" ,"Hyphomonas","Ignavibacterium","Isosphaera" ,"Klebsiella","Lachnospira" ,"Lachnobacillus" ,"Legionella" ,"Methylocystis" ,"Methylotenera","Micrococcus" ,"Moraxella" ,"Mycobacterium" ,"Neisseria" ,"Nitrobacter","Nitrosomonas" ,"Nitrospira" ,"Nocardioides" ,"Novosphingobium" ,"Ottawia" ,"Paludibacter" ,"Pantoea","Parabacteroides","Paracoccus" ,"Pseudomonas"  ,"Ralstonia" ,"Rhodobacter" ,"Rhodococcus","Romboutsia" ,"Roseburia","Ruminococcus" ,"Runella" ,"Serratia" ,"Shinella" ,"Smithella","Staphylococcus" ,"Stenotrophobacter" ,"Stenotrophomonas","Streptococcus","Sulfuritalea" ,"Syntrophobacter" ,"Syntrophomonas","Syntrophus","Terrimonas" ,"Thermomonas" ,"Trichococcus" ,"Xanthomonas","Zoogloea")))%>%
  ungroup()
View(EFF15_ephem_eff)

EFF15_ephem_eff$SD_squared ='^'(EFF15_ephem_eff$SD,2)
EFF15_ephem_eff$SD_squared_divided =(EFF15_ephem_eff$SD_squared/8) 
  
top_means_eff_EFF15_ephem<-aggregate(Mean ~ Genus, data = EFF15_ephem_eff, FUN = sum)
top_SD_eff_EFF15_ephem<-aggregate(SD_squared_divided ~ Genus, data = EFF15_ephem_eff, FUN = sum)

top_eff_EFF15_ephem<-merge(top_means_eff_EFF15_ephem, top_SD_eff_EFF15_ephem, by=c("Genus"))
top_eff_EFF15_ephem$SD_Final<-sqrt(top_eff_EFF15_ephem$SD_squared_divided)
View(top_eff_EFF15_ephem)
...
ggplot(sample.df.agg.effluent.genus.Bow.invert, aes(x=Site, y=Abundance, fill=Genus)) + 
  ...
  theme(legend.position="right", legend.key.size = unit(0.3, "cm"),
        legend.text=element_text(size=6, face="italic"), legend.title=element_text(size=10)) +
  guides (fill=guide_legend(nrow=30, ncol=4)) +
  theme(axis.title.y=element_text(size=16),
        axis.text.y=element_text(size=16, color="black"),
        axis.text.x=element_text(angle=45, hjust=1, size=16, color="black"),
                        axis.title=element_text(size=18))+
...
#106 total effluent associated bacteria
...
#87 total effluent associated bacteria 
...
## Trying to find bacteria unique to sites located downstream of outfalls
```{r}
#Hydro
hydro_grvbr <- subset_samples(sample_ps, Family=="Hydropsychidae" & Site == "Graves Bridge")
hydro_grvbr<- prune_taxa(taxa_sums(hydro_grvbr) > 0, hydro_grvbr) # prune OTUs that are not present in at least one sample

hydro_pmf <- subset_samples(sample_ps, Family=="Hydropsychidae" & Site == "Policeman Flats")
hydro_pmf<- prune_taxa(taxa_sums(hydro_pmf) > 0, hydro_pmf) # prune OTUs that are not present in at least one sample

intersect_GRVBR_PMF <- intersect(taxa_names(hydro_grvbr), taxa_names(hydro_pmf))

taxa_sub_grvbr_pmf <- as.data.frame(sample_ps@tax_table[rownames(sample_ps@tax_table) %in% intersect_GRVBR_PMF, ])


#Venn diagram

hydro_ps_Bow<-subset_samples(sample_ps, Family=="Hydropsychidae", Stream_Type=="Bow River")
hydro_ps_Bow<- prune_taxa(taxa_sums(hydro_ps_Bow) > 0, hydro_ps_Bow)

hydro_ps_rel_abun_Bow<-microbiome::transform(hydro_ps_Bow, "compositional")

hydro_exposed_unexposed_Bow <- unique(as.character(meta(hydro_ps_rel_abun_Bow)$Exposure))
print(hydro_exposed_unexposed_Bow)


list_core <- c() # an empty object to store information

for (n in hydro_exposed_unexposed_Bow){ # for each variable n in DiseaseState
    #print(paste0("Identifying Core Taxa for ", n))
    
    ps.sub <- subset_samples(hydro_ps_rel_abun_Bow, Exposure == n) # Choose sample from DiseaseState by n
...
mycols <- c(Unexposed="#cbf3f0", Exposed="#fcf5c7") 
plot(venn(list_core), fills = mycols)

taxa_names(hydro_ps_rel_abun_Bow)[1:5]

# format names
pseq.rel.f <- format_to_besthit(hydro_ps_rel_abun_Bow)
# check names
...
for (n in hydro_exposed_unexposed_Bow){ # for each variable n in DiseaseState
    ...
    ps.sub <- subset_samples(pseq.rel.f, Exposure == n) # Choose sample from DiseaseState by n
...
#Also need a .biom file with the otu/asv table (ASVs are labelled with a unique identifier that matches the fasta file)

#### Creating FASTA file with first column as the ASV identifier ("ASV1", "ASV2", "ASV3"...) and the second column as the genetic sequence of the amplicon. ####
...
OTUdf1<- rownames_to_column(OTUdf, "Sequences")

#Take the first column of the filtered OTU table to retrieve bacterial DNA sequences for the fasta file. 
seq<-OTUdf1[,1] #Makes a column of sequences

#Now we need to create a new column with an ASV identifer for those sequences that we pulled.
sample_ps_1<-sample_ps
taxa_names(sample_ps_1) <- paste0("ASV", seq(ntaxa(sample_ps_1))) #this will change the sequences to a numbered ASV ID


# Extract abundance matrix from the phyloseq object
OTU2 = as(otu_table(sample_ps_1), "matrix")
# Coerce to data.frame
...
ASVfasta<-data.frame(cbind(ASV, seq)) #Combines the ASV IDs and the sequences in a dataframe
#Remove headers
#View(ASVfasta)

#Converts data frame to fasta file
ASVfasta = dataframe2fas(ASVfasta, file="microbiome_analysis/PICRUSt2 Data/ASV_sequences.fna")
...
#View(OTUdf2) #Change ASVs to a row name

OTUdf3<-column_to_rownames(OTUdf2, "ASV")
#View(OTUdf3)

OTUbiom<-make_biom(OTUdf3) #Turns dataframe to biom format
write_biom(OTUbiom, biom_file="microbiome_analysis/PICRUSt2 Data/ASV_table.biom") #Saves biom format as file
#Can also be used as a .tsv (text) file. 
write.table(OTUdf3, file="microbiome_analysis/PICRUSt2 Data/ASV_table.tsv", sep="\t") #Saves dataframe to text file (tab separated values)

#The row and column names have quotations when you open the file since they are saved as strings from R. You have to delete those quotations before running the PICRUSt pipeline. I did this directly in my text editor.

```


## Loading and modifying data format of PICRUSt2 output (turning it into a phyloseq object)

'description' is a higher order classification (pathway class). Multiple individual pathways can be categorized into a pathway class.
...
metacyc_ont_1<-as.data.frame(metacyc_ont[,c(1,16)])

metacyc_ont_2<-dplyr::rename(metacyc_ont_1, "pathway_type" = "Final_pathway")
...
pathway_type<-metacyc_combined[,names(metacyc_combined) %in% c("Pathways", "pathway_type")]
pathway_type_1<-column_to_rownames(pathway_type, "Pathways")
pathway_type_mat<-as.matrix(pathway_type_1)
pathway_type_mat_1 = tax_table(pathway_type_mat) #Final table to use for taxonomy table
...
picrust_ps<-phyloseq(metacyc_rounded_mat_1, sample_data(metadata_picrust), pathway_type_mat_1)
```
...
top_pathways_bow <- sample.df.agg_bow %>%
    ...
    summarise(Mean = mean(Abundance)) %>%
    arrange(-Mean)
top_24_pathways_bow <- top_pathways_bow$pathway_type[1:109]
df0_bow <- sample.df.agg_bow %>%
...
length(unique(df0_bow$pathway_type)) #25
...
#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/Top 25 MetaCyc pathway classes across sites and taxa Bow.png", height=10, width=24)


#### ACWA ####

nsamples(ACWA_ps_picrust<-subset_samples(picrust_ps, Stream_Type!="Bow River")) #55

rel_abun_ACWA<- speedyseq::tax_glom(ACWA_ps_picrust, taxrank = "description")
comp_rel_abun_ACWA <- transform_sample_counts(rel_abun_ACWA, function(x) x/sum(x))

rel_abun_ACWA_df <- psmelt(comp_rel_abun_ACWA)

sample.df.agg_ACWA <- aggregate(Abundance ~ description + Site_code + Family + Stage, data = rel_abun_ACWA_df, FUN = mean) 
...
top_pathways_ACWA <- sample.df.agg_ACWA %>%
    ...
    summarise(Mean = mean(Abundance)) %>%
    arrange(-Mean)
top_24_pathways_ACWA <- top_pathways_ACWA$description[1:72]
df0_ACWA <- sample.df.agg_ACWA %>%
    mutate(description = fct_other(description, top_24_pathways_ACWA))
df0_ACWA

df0_ACWA<-aggregate(Abundance~description + Site_code + Family + Stage, data=df0_ACWA, FUN="sum") #Aggregate again to combine all the 'Other' categories into a single bar on the graph.

length(unique(df0_ACWA$description)) #26

df0_ACWA$Site_code<-factor(df0_ACWA$Site_code, levels=c("BRR2", "PCR1", "PCR3"), labels=c("REF", "EFF5", "EFF15"))
...
toppathwaysplot_ACWA <- ggplot(df0_ACWA, aes(x=Site_code, y=Abundance, fill=description)) + 
  geom_bar(stat="identity", color="black") + 
  facet_nested_wrap(~Stage + Family, scales="free_x", nrow=1)+
  ylab("Mean Relative Abundance") +
  theme_bw()+
...
#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/Top 25 MetaCyc pathway classes across sites and taxa ACWA.png", height=10, width=24)
```
...
picrust_hydro_ps_Bow<-subset_samples(picrust_ps, Family=="Hydropsychidae" & Stream_Type=="Bow River")

picrust_deseq_exposed_hydro_Bow <- phyloseq_to_deseq2(picrust_hydro_ps_Bow, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_hydro_Bow$Exposure <-relevel(picrust_deseq_exposed_hydro_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_hydro_Bow <- DESeq(picrust_deseq_exposed_hydro_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_hydro_Bow <- results(picrust_deseq_exposed_hydro_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_hydro_Bow = picrust_res_deseq_exposed_hydro_Bow[which(picrust_res_deseq_exposed_hydro_Bow$padj < alpha), ]
picrust_sigtab_exposed_hydro_Bow = cbind(as(picrust_sigtab_exposed_hydro_Bow, "data.frame"), as(tax_table(picrust_hydro_ps_Bow)[rownames(picrust_sigtab_exposed_hydro_Bow), ], "matrix"))

picrust_diff_abund_hydro_Bow_plot<-ggplot(picrust_sigtab_exposed_hydro_Bow, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_hydro_Bow_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/exposed-non exposed hydro diff abun pathways.png", height=24, width=22)


## Using Cushing Bridge as a reference site ##

picrust_hydro_ps_Bow<-subset_samples(picrust_ps, Family=="Hydropsychidae" & Stream_Type=="Bow River")

picrust_hydro_ps_Bow_subset<-subset_samples(picrust_hydro_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_hydro_Bow_subset <- phyloseq_to_deseq2(picrust_hydro_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_hydro_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_hydro_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_hydro_Bow_subset <- DESeq(picrust_deseq_exposed_hydro_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_hydro_Bow_subset <- results(picrust_deseq_exposed_hydro_Bow_subset, cooksCutoff = FALSE)
picrust_res_deseq_exposed_hydro_Bow_subset
alpha = 0.001

picrust_sigtab_exposed_hydro_Bow_subset = picrust_res_deseq_exposed_hydro_Bow_subset[which(picrust_res_deseq_exposed_hydro_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_hydro_Bow_subset = cbind(as(picrust_sigtab_exposed_hydro_Bow_subset, "data.frame"), as(tax_table(picrust_hydro_ps_Bow_subset)[rownames(picrust_sigtab_exposed_hydro_Bow_subset), ], "matrix"))

x_exposed_hydro_Bow_subset_picrust = tapply(picrust_sigtab_exposed_hydro_Bow_subset$log2FoldChange, picrust_sigtab_exposed_hydro_Bow_subset$description, function(x) max(x))
x_exposed_hydro_Bow_subset_picrust = sort(x_exposed_hydro_Bow_subset_picrust, TRUE)
picrust_sigtab_exposed_hydro_Bow_subset$description = factor(as.character(picrust_sigtab_exposed_hydro_Bow_subset$description), levels=names(x_exposed_hydro_Bow_subset_picrust))
#4 differentially expressed pathways

picrust_diff_abund_hydro_Bow_plot_subset<-ggplot(picrust_sigtab_exposed_hydro_Bow_subset, aes(x=log2FoldChange, y=description))+
  geom_point(pch=21, size=8, fill="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
          theme(legend.title = element_text(size=34, color="black"), legend.position="bottom")+
...
picrust_diff_abund_hydro_Bow_plot_subset



#COCH - SUN

picrust_hydro_sun_coch<-subset_samples(picrust_hydro_ps_Bow, Site=="Cochrane" | Site=="Sunalta")

picrust_deseq_sun_coch_hydro <- phyloseq_to_deseq2(picrust_hydro_sun_coch, ~ Site)

picrust_deseq_sun_coch_hydro <- DESeq(picrust_deseq_sun_coch_hydro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_sun_coch_hydro <- results(picrust_deseq_sun_coch_hydro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_sun_coch_hydro = picrust_res_deseq_sun_coch_hydro[which(picrust_res_deseq_sun_coch_hydro$padj < alpha), ]
picrust_sigtab_sun_coch_hydro = cbind(as(picrust_sigtab_sun_coch_hydro, "data.frame"), as(tax_table(picrust_hydro_sun_coch)[rownames(picrust_sigtab_sun_coch_hydro), ], "matrix"))
#0


#CUSHBR - COCH

picrust_hydro_cush_coch<-subset_samples(picrust_hydro_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_hydro <- phyloseq_to_deseq2(picrust_hydro_cush_coch, ~ Site)

picrust_deseq_cush_coch_hydro <- DESeq(picrust_deseq_cush_coch_hydro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_hydro <- results(picrust_deseq_cush_coch_hydro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_hydro = picrust_res_deseq_cush_coch_hydro[which(picrust_res_deseq_cush_coch_hydro$padj < alpha), ]
picrust_sigtab_cush_coch_hydro = cbind(as(picrust_sigtab_cush_coch_hydro, "data.frame"), as(tax_table(picrust_hydro_cush_coch)[rownames(picrust_sigtab_cush_coch_hydro), ], "matrix"))
#10 diff abun pathways

picrust_diff_abund_hydro_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_hydro, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hydro_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch hydro diff abun pathways.png", height=24, width=22)


#GRVBR - COCH

picrust_hydro_grvbr_coch<-subset_samples(picrust_hydro_ps_Bow, Site=="Graves Bridge" | Site=="Cochrane")

picrust_deseq_grvbr_coch_hydro <- phyloseq_to_deseq2(picrust_hydro_grvbr_coch, ~ Site)

picrust_deseq_grvbr_coch_hydro <- DESeq(picrust_deseq_grvbr_coch_hydro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_grvbr_coch_hydro <- results(picrust_deseq_grvbr_coch_hydro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_grvbr_coch_hydro = picrust_res_deseq_grvbr_coch_hydro[which(picrust_res_deseq_grvbr_coch_hydro$padj < alpha), ]
picrust_sigtab_grvbr_coch_hydro = cbind(as(picrust_sigtab_grvbr_coch_hydro, "data.frame"), as(tax_table(picrust_hydro_grvbr_coch)[rownames(picrust_sigtab_grvbr_coch_hydro), ], "matrix"))
#79 diff abun pathways

picrust_diff_abund_hydro_grvbr_coch_plot<-ggplot(picrust_sigtab_grvbr_coch_hydro, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y  = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hydro_grvbr_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/grvbr-coch hydro diff abun pathways.png", height=24, width=22)



#PMF - COCH

picrust_hydro_pmf_coch<-subset_samples(picrust_hydro_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_hydro <- phyloseq_to_deseq2(picrust_hydro_pmf_coch, ~ Site)


picrust_deseq_pmf_coch_hydro <- DESeq(picrust_deseq_pmf_coch_hydro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_hydro <- results(picrust_deseq_pmf_coch_hydro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_hydro = picrust_res_deseq_pmf_coch_hydro[which(picrust_res_deseq_pmf_coch_hydro$padj < alpha), ]
picrust_sigtab_pmf_coch_hydro = cbind(as(picrust_sigtab_pmf_coch_hydro, "data.frame"), as(tax_table(picrust_hydro_pmf_coch)[rownames(picrust_sigtab_pmf_coch_hydro), ], "matrix"))
#170 observations

picrust_diff_abund_hydro_pmf_coch_plot<-ggplot(picrust_sigtab_pmf_coch_hydro, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hydro_pmf_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/pmf-coch hydro diff abun pathways.png", height=24, width=22)
...
picrust_hydro_cush_sun<-subset_samples(picrust_hydro_ps_Bow, Site=="Cushing Bridge" | Site=="Sunalta")

picrust_deseq_cush_sun_hydro <- phyloseq_to_deseq2(picrust_hydro_cush_sun, ~ Site)

picrust_deseq_cush_sun_hydro <- DESeq(picrust_deseq_cush_sun_hydro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_sun_hydro <- results(picrust_deseq_cush_sun_hydro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_sun_hydro = picrust_res_deseq_cush_sun_hydro[which(picrust_res_deseq_cush_sun_hydro$padj < alpha), ]
picrust_sigtab_cush_sun_hydro = cbind(as(picrust_sigtab_cush_sun_hydro, "data.frame"), as(tax_table(picrust_hydro_cush_sun)[rownames(picrust_sigtab_cush_sun_hydro), ], "matrix"))
#7 diff abun pathways

picrust_diff_abund_hydro_cush_sun_plot<-ggplot(picrust_sigtab_cush_sun_hydro, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hydro_cush_sun_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-sun hydro diff abun pathways.png", height=24, width=22)


#CUSHBR - GRVBR

picrust_hydro_cush_grvbr<-subset_samples(picrust_hydro_ps_Bow, Site=="Cushing Bridge" | Site=="Graves Bridge")

picrust_deseq_cush_grvbr_hydro <- phyloseq_to_deseq2(picrust_hydro_cush_grvbr, ~ Site)

picrust_deseq_cush_grvbr_hydro <- DESeq(picrust_deseq_cush_grvbr_hydro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_grvbr_hydro <- results(picrust_deseq_cush_grvbr_hydro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_grvbr_hydro = picrust_res_deseq_cush_grvbr_hydro[which(picrust_res_deseq_cush_grvbr_hydro$padj < alpha), ]
picrust_sigtab_cush_grvbr_hydro = cbind(as(picrust_sigtab_cush_grvbr_hydro, "data.frame"), as(tax_table(picrust_hydro_cush_grvbr)[rownames(picrust_sigtab_cush_grvbr_hydro), ], "matrix"))
#1 diff abun pathways

picrust_diff_abund_hydro_cush_grvbr_plot<-ggplot(picrust_sigtab_cush_grvbr_hydro, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hydro_cush_grvbr_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-grvbr hydro diff abun pathways.png", height=24, width=22)


#CUSHBR - PMF

picrust_hydro_cush_pmf<-subset_samples(picrust_hydro_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_hydro <- phyloseq_to_deseq2(picrust_hydro_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_hydro <- DESeq(picrust_deseq_cush_pmf_hydro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_hydro <- results(picrust_deseq_cush_pmf_hydro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_hydro = picrust_res_deseq_cush_pmf_hydro[which(picrust_res_deseq_cush_pmf_hydro$padj < alpha), ]
picrust_sigtab_cush_pmf_hydro = cbind(as(picrust_sigtab_cush_pmf_hydro, "data.frame"), as(tax_table(picrust_hydro_cush_pmf)[rownames(picrust_sigtab_cush_pmf_hydro), ], "matrix"))
#52 diff abun pathways

picrust_diff_abund_hydro_cush_pmf_plot<-ggplot(picrust_sigtab_cush_pmf_hydro, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hydro_cush_pmf_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-pmf hydro diff abun pathways.png", height=24, width=22)

#### Heptageniidae ####

#Exposed vs non wastewater exposed sites

picrust_hep_ps_Bow<-subset_samples(picrust_ps, Family=="Heptageniidae" & Stream_Type=="Bow River")

picrust_deseq_exposed_hep_Bow <- phyloseq_to_deseq2(picrust_hep_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_hep_Bow$Exposure <-relevel(picrust_deseq_exposed_hep_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_hep_Bow <- DESeq(picrust_deseq_exposed_hep_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_hep_Bow <- results(picrust_deseq_exposed_hep_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_hep_Bow = picrust_res_deseq_exposed_hep_Bow[which(picrust_res_deseq_exposed_hep_Bow$padj < alpha), ]
picrust_sigtab_exposed_hep_Bow = cbind(as(picrust_sigtab_exposed_hep_Bow, "data.frame"), as(tax_table(picrust_hep_ps_Bow)[rownames(picrust_sigtab_exposed_hep_Bow), ], "matrix"))
#8 diff abun path

picrust_diff_abund_hep_Bow_plot<-ggplot(picrust_sigtab_exposed_hep_Bow, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_hep_Bow_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/exposed-non exposed hep diff abun pathways.png", height=24, width=22)
...
picrust_hep_ps_Bow_subset<-subset_samples(picrust_hep_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_hep_Bow_subset <- phyloseq_to_deseq2(picrust_hep_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_hep_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_hep_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_hep_Bow_subset <- DESeq(picrust_deseq_exposed_hep_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_hep_Bow_subset <- results(picrust_deseq_exposed_hep_Bow_subset, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_hep_Bow_subset = picrust_res_deseq_exposed_hep_Bow_subset[which(picrust_res_deseq_exposed_hep_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_hep_Bow_subset = cbind(as(picrust_sigtab_exposed_hep_Bow_subset, "data.frame"), as(tax_table(picrust_hep_ps_Bow_subset)[rownames(picrust_sigtab_exposed_hep_Bow_subset), ], "matrix"))

x_exposed_hep_Bow_subset_picrust = tapply(picrust_sigtab_exposed_hep_Bow_subset$log2FoldChange, picrust_sigtab_exposed_hep_Bow_subset$description, function(x) max(x))
x_exposed_hep_Bow_subset_picrust = sort(x_exposed_hep_Bow_subset_picrust, TRUE)
picrust_sigtab_exposed_hep_Bow_subset$description = factor(as.character(picrust_sigtab_exposed_hep_Bow_subset$description), levels=names(x_exposed_hep_Bow_subset_picrust))
#12 differentially expressed pathways

picrust_diff_abund_hep_Bow_plot_subset<-ggplot(picrust_sigtab_exposed_hep_Bow_subset, aes(x=log2FoldChange, y=description))+
  geom_point(pch=21, size=8, fill="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_hep_Bow_plot_subset


#COCH - SUN

picrust_hep_sun_coch<-subset_samples(picrust_hep_ps_Bow, Site=="Cochrane" | Site=="Sunalta")

picrust_deseq_sun_coch_hep <- phyloseq_to_deseq2(picrust_hep_sun_coch, ~ Site)

picrust_deseq_sun_coch_hep <- DESeq(picrust_deseq_sun_coch_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_sun_coch_hep <- results(picrust_deseq_sun_coch_hep, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_sun_coch_hep = picrust_res_deseq_sun_coch_hep[which(picrust_res_deseq_sun_coch_hep$padj < alpha), ]
picrust_sigtab_sun_coch_hep = cbind(as(picrust_sigtab_sun_coch_hep, "data.frame"), as(tax_table(picrust_hep_sun_coch)[rownames(picrust_sigtab_sun_coch_hep), ], "matrix"))
#0


#CUSHBR - COCH

picrust_hep_cush_coch<-subset_samples(picrust_hep_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_hep <- phyloseq_to_deseq2(picrust_hep_cush_coch, ~ Site)

picrust_deseq_cush_coch_hep <- DESeq(picrust_deseq_cush_coch_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_hep <- results(picrust_deseq_cush_coch_hep, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_hep = picrust_res_deseq_cush_coch_hep[which(picrust_res_deseq_cush_coch_hep$padj < alpha), ]
picrust_sigtab_cush_coch_hep = cbind(as(picrust_sigtab_cush_coch_hep, "data.frame"), as(tax_table(picrust_hep_cush_coch)[rownames(picrust_sigtab_cush_coch_hep), ], "matrix"))
#15 diff abun pathways

picrust_diff_abund_hep_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_hep, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hep_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch hep diff abun pathways.png", height=24, width=22)


#GRVBR - COCH

picrust_hep_grvbr_coch<-subset_samples(picrust_hep_ps_Bow, Site=="Graves Bridge" | Site=="Cochrane")

picrust_deseq_grvbr_coch_hep <- phyloseq_to_deseq2(picrust_hep_grvbr_coch, ~ Site)

picrust_deseq_grvbr_coch_hep <- DESeq(picrust_deseq_grvbr_coch_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_grvbr_coch_hep <- results(picrust_deseq_grvbr_coch_hep, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_grvbr_coch_hep = picrust_res_deseq_grvbr_coch_hep[which(picrust_res_deseq_grvbr_coch_hep$padj < alpha), ]
picrust_sigtab_grvbr_coch_hep = cbind(as(picrust_sigtab_grvbr_coch_hep, "data.frame"), as(tax_table(picrust_hep_grvbr_coch)[rownames(picrust_sigtab_grvbr_coch_hep), ], "matrix"))
#31 diff abun pathways

picrust_diff_abund_hep_grvbr_coch_plot<-ggplot(picrust_sigtab_grvbr_coch_hep, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y  = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hep_grvbr_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/grvbr-coch hep diff abun pathways.png", height=24, width=22)



#PMF - COCH

picrust_hep_pmf_coch<-subset_samples(picrust_hep_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_hep <- phyloseq_to_deseq2(picrust_hep_pmf_coch, ~ Site)

picrust_deseq_pmf_coch_hep <- DESeq(picrust_deseq_pmf_coch_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_hep <- results(picrust_deseq_pmf_coch_hep, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_hep = picrust_res_deseq_pmf_coch_hep[which(picrust_res_deseq_pmf_coch_hep$padj < alpha), ]
picrust_sigtab_pmf_coch_hep = cbind(as(picrust_sigtab_pmf_coch_hep, "data.frame"), as(tax_table(picrust_hep_pmf_coch)[rownames(picrust_sigtab_pmf_coch_hep), ], "matrix"))
#62 observations

picrust_diff_abund_hep_pmf_coch_plot<-ggplot(picrust_sigtab_pmf_coch_hep, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hep_pmf_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/pmf-coch hep diff abun pathways.png", height=24, width=22)
...
picrust_hep_cush_sun<-subset_samples(picrust_hep_ps_Bow, Site=="Cushing Bridge" | Site=="Sunalta")

picrust_deseq_cush_sun_hep <- phyloseq_to_deseq2(picrust_hep_cush_sun, ~ Site)

picrust_deseq_cush_sun_hep <- DESeq(picrust_deseq_cush_sun_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_sun_hep <- results(picrust_deseq_cush_sun_hep, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_sun_hep = picrust_res_deseq_cush_sun_hep[which(picrust_res_deseq_cush_sun_hep$padj < alpha), ]
picrust_sigtab_cush_sun_hep = cbind(as(picrust_sigtab_cush_sun_hep, "data.frame"), as(tax_table(picrust_hep_cush_sun)[rownames(picrust_sigtab_cush_sun_hep), ], "matrix"))
#0 diff abun pathways


#CUSHBR - GRVBR

picrust_hep_cush_grvbr<-subset_samples(picrust_hep_ps_Bow, Site=="Cushing Bridge" | Site=="Graves Bridge")

picrust_deseq_cush_grvbr_hep <- phyloseq_to_deseq2(picrust_hep_cush_grvbr, ~ Site)

picrust_deseq_cush_grvbr_hep <- DESeq(picrust_deseq_cush_grvbr_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_grvbr_hep <- results(picrust_deseq_cush_grvbr_hep, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_grvbr_hep = picrust_res_deseq_cush_grvbr_hep[which(picrust_res_deseq_cush_grvbr_hep$padj < alpha), ]
picrust_sigtab_cush_grvbr_hep = cbind(as(picrust_sigtab_cush_grvbr_hep, "data.frame"), as(tax_table(picrust_hep_cush_grvbr)[rownames(picrust_sigtab_cush_grvbr_hep), ], "matrix"))
#15 diff abun pathways

picrust_diff_abund_hep_cush_grvbr_plot<-ggplot(picrust_sigtab_cush_grvbr_hep, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hep_cush_grvbr_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-grvbr hep diff abun pathways.png", height=24, width=22)


#CUSHBR - PMF

picrust_hep_cush_pmf<-subset_samples(picrust_hep_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_hep <- phyloseq_to_deseq2(picrust_hep_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_hep <- DESeq(picrust_deseq_cush_pmf_hep, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_hep <- results(picrust_deseq_cush_pmf_hep, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_hep = picrust_res_deseq_cush_pmf_hep[which(picrust_res_deseq_cush_pmf_hep$padj < alpha), ]
picrust_sigtab_cush_pmf_hep = cbind(as(picrust_sigtab_cush_pmf_hep, "data.frame"), as(tax_table(picrust_hep_cush_pmf)[rownames(picrust_sigtab_cush_pmf_hep), ], "matrix"))
#10 diff abun pathways

picrust_diff_abund_hep_cush_pmf_plot<-ggplot(picrust_sigtab_cush_pmf_hep, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_hep_cush_pmf_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-pmf hep diff abun pathways.png", height=24, width=22)

#### Chironomidae ####

#Exposed vs non wastewater exposed sites

picrust_chiro_ps_Bow<-subset_samples(picrust_ps, Family=="Chironomidae" & Stream_Type=="Bow River")

picrust_deseq_exposed_chiro_Bow <- phyloseq_to_deseq2(picrust_chiro_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_chiro_Bow$Exposure <-relevel(picrust_deseq_exposed_chiro_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_chiro_Bow <- DESeq(picrust_deseq_exposed_chiro_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_chiro_Bow <- results(picrust_deseq_exposed_chiro_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_chiro_Bow = picrust_res_deseq_exposed_chiro_Bow[which(picrust_res_deseq_exposed_chiro_Bow$padj < alpha), ]
picrust_sigtab_exposed_chiro_Bow = cbind(as(picrust_sigtab_exposed_chiro_Bow, "data.frame"), as(tax_table(picrust_chiro_ps_Bow)[rownames(picrust_sigtab_exposed_chiro_Bow), ], "matrix"))
#8 diff abun path

picrust_diff_abund_chiro_Bow_plot<-ggplot(picrust_sigtab_exposed_chiro_Bow, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_chiro_Bow_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/exposed-non exposed chiro diff abun pathways.png", height=24, width=22)
...
picrust_chiro_ps_Bow_subset<-subset_samples(picrust_chiro_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_chiro_Bow_subset <- phyloseq_to_deseq2(picrust_chiro_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_chiro_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_chiro_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_chiro_Bow_subset <- DESeq(picrust_deseq_exposed_chiro_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_chiro_Bow_subset <- results(picrust_deseq_exposed_chiro_Bow_subset, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_chiro_Bow_subset = picrust_res_deseq_exposed_chiro_Bow_subset[which(picrust_res_deseq_exposed_chiro_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_chiro_Bow_subset = cbind(as(picrust_sigtab_exposed_chiro_Bow_subset, "data.frame"), as(tax_table(picrust_chiro_ps_Bow_subset)[rownames(picrust_sigtab_exposed_chiro_Bow_subset), ], "matrix"))

x_exposed_chiro_Bow_subset_picrust = tapply(picrust_sigtab_exposed_chiro_Bow_subset$log2FoldChange, picrust_sigtab_exposed_chiro_Bow_subset$description, function(x) max(x))

x_exposed_chiro_Bow_subset_picrust = sort(x_exposed_chiro_Bow_subset_picrust, TRUE)

picrust_sigtab_exposed_chiro_Bow_subset$description = factor(as.character(picrust_sigtab_exposed_chiro_Bow_subset$description), levels=names(x_exposed_chiro_Bow_subset_picrust))
#5 differentially expressed pathways

picrust_diff_abund_chiro_Bow_plot_subset<-ggplot(picrust_sigtab_exposed_chiro_Bow_subset, aes(x=log2FoldChange, y=description))+
  geom_point(pch=21, size=8, fill="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_chiro_Bow_plot_subset


#CUSHBR - COCH

picrust_chiro_cush_coch<-subset_samples(picrust_chiro_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_chiro <- phyloseq_to_deseq2(picrust_chiro_cush_coch, ~ Site)

picrust_deseq_cush_coch_chiro <- DESeq(picrust_deseq_cush_coch_chiro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_chiro <- results(picrust_deseq_cush_coch_chiro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_chiro = picrust_res_deseq_cush_coch_chiro[which(picrust_res_deseq_cush_coch_chiro$padj < alpha), ]
picrust_sigtab_cush_coch_chiro = cbind(as(picrust_sigtab_cush_coch_chiro, "data.frame"), as(tax_table(picrust_chiro_cush_coch)[rownames(picrust_sigtab_cush_coch_chiro), ], "matrix"))
#3 diff abun pathways

picrust_diff_abund_chiro_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_chiro, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_chiro_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch chiro diff abun pathways.png", height=24, width=22)


#PMF - COCH

picrust_chiro_pmf_coch<-subset_samples(picrust_chiro_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_chiro <- phyloseq_to_deseq2(picrust_chiro_pmf_coch, ~ Site)

picrust_deseq_pmf_coch_chiro <- DESeq(picrust_deseq_pmf_coch_chiro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_chiro <- results(picrust_deseq_pmf_coch_chiro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_chiro = picrust_res_deseq_pmf_coch_chiro[which(picrust_res_deseq_pmf_coch_chiro$padj < alpha), ]
picrust_sigtab_pmf_coch_chiro = cbind(as(picrust_sigtab_pmf_coch_chiro, "data.frame"), as(tax_table(picrust_chiro_pmf_coch)[rownames(picrust_sigtab_pmf_coch_chiro), ], "matrix"))
#10 observations

picrust_diff_abund_chiro_pmf_coch_plot<-ggplot(picrust_sigtab_pmf_coch_chiro, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_chiro_pmf_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/pmf-coch chiro diff abun pathways.png", height=24, width=22)
...
picrust_chiro_cush_pmf<-subset_samples(picrust_chiro_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_chiro <- phyloseq_to_deseq2(picrust_chiro_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_chiro <- DESeq(picrust_deseq_cush_pmf_chiro, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_chiro <- results(picrust_deseq_cush_pmf_chiro, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_chiro = picrust_res_deseq_cush_pmf_chiro[which(picrust_res_deseq_cush_pmf_chiro$padj < alpha), ]
picrust_sigtab_cush_pmf_chiro = cbind(as(picrust_sigtab_cush_pmf_chiro, "data.frame"), as(tax_table(picrust_chiro_cush_pmf)[rownames(picrust_sigtab_cush_pmf_chiro), ], "matrix"))
#5 diff abun pathways

picrust_diff_abund_chiro_cush_pmf_plot<-ggplot(picrust_sigtab_cush_pmf_chiro, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_chiro_cush_pmf_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-pmf chiro diff abun pathways.png", height=24, width=22)

#### Perlidae ####

#Exposed vs non wastewater exposed sites

picrust_perl_ps_Bow<-subset_samples(picrust_ps, Family=="Perlidae" & Stream_Type=="Bow River")

picrust_deseq_exposed_perl_Bow <- phyloseq_to_deseq2(picrust_perl_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_perl_Bow$Exposure <-relevel(picrust_deseq_exposed_perl_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_perl_Bow <- DESeq(picrust_deseq_exposed_perl_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_perl_Bow <- results(picrust_deseq_exposed_perl_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_perl_Bow = picrust_res_deseq_exposed_perl_Bow[which(picrust_res_deseq_exposed_perl_Bow$padj < alpha), ]
picrust_sigtab_exposed_perl_Bow = cbind(as(picrust_sigtab_exposed_perl_Bow, "data.frame"), as(tax_table(picrust_perl_ps_Bow)[rownames(picrust_sigtab_exposed_perl_Bow), ], "matrix"))
#0 diff abun path
...
picrust_perl_ps_Bow_subset<-subset_samples(picrust_perl_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_perl_Bow_subset <- phyloseq_to_deseq2(picrust_perl_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_perl_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_perl_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_perl_Bow_subset <- DESeq(picrust_deseq_exposed_perl_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_perl_Bow_subset <- results(picrust_deseq_exposed_perl_Bow_subset, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_perl_Bow_subset = picrust_res_deseq_exposed_perl_Bow_subset[which(picrust_res_deseq_exposed_perl_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_perl_Bow_subset = cbind(as(picrust_sigtab_exposed_perl_Bow_subset, "data.frame"), as(tax_table(picrust_perl_ps_Bow_subset)[rownames(picrust_sigtab_exposed_perl_Bow_subset), ], "matrix"))

x_exposed_perl_Bow_subset_picrust = tapply(picrust_sigtab_exposed_perl_Bow_subset$log2FoldChange, picrust_sigtab_exposed_perl_Bow_subset$description, function(x) max(x))

x_exposed_perl_Bow_subset_picrust = sort(x_exposed_perl_Bow_subset_picrust, TRUE)

picrust_sigtab_exposed_perl_Bow_subset$description = factor(as.character(picrust_sigtab_exposed_perl_Bow_subset$description), levels=names(x_exposed_perl_Bow_subset_picrust))
#31 differentially expressed pathways

picrust_diff_abund_perl_Bow_plot_subset<-ggplot(picrust_sigtab_exposed_perl_Bow_subset, aes(x=log2FoldChange, y=description))+
  geom_point(pch=21, size=8, fill="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_perl_Bow_plot_subset



#CUSHBR - COCH

picrust_perl_cush_coch<-subset_samples(picrust_perl_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_perl <- phyloseq_to_deseq2(picrust_perl_cush_coch, ~ Site)

picrust_deseq_cush_coch_perl <- DESeq(picrust_deseq_cush_coch_perl, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_perl <- results(picrust_deseq_cush_coch_perl, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_perl = picrust_res_deseq_cush_coch_perl[which(picrust_res_deseq_cush_coch_perl$padj < alpha), ]
picrust_sigtab_cush_coch_perl = cbind(as(picrust_sigtab_cush_coch_perl, "data.frame"), as(tax_table(picrust_perl_cush_coch)[rownames(picrust_sigtab_cush_coch_perl), ], "matrix"))
#4 diff abun pathways

picrust_diff_abund_perl_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_perl, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_perl_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch perl diff abun pathways.png", height=24, width=22)


#PMF - COCH

picrust_perl_pmf_coch<-subset_samples(picrust_perl_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_perl <- phyloseq_to_deseq2(picrust_perl_pmf_coch, ~ Site)

picrust_deseq_pmf_coch_perl <- DESeq(picrust_deseq_pmf_coch_perl, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_perl <- results(picrust_deseq_pmf_coch_perl, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_perl = picrust_res_deseq_pmf_coch_perl[which(picrust_res_deseq_pmf_coch_perl$padj < alpha), ]
picrust_sigtab_pmf_coch_perl = cbind(as(picrust_sigtab_pmf_coch_perl, "data.frame"), as(tax_table(picrust_perl_pmf_coch)[rownames(picrust_sigtab_pmf_coch_perl), ], "matrix"))
#5 observations

picrust_diff_abund_perl_pmf_coch_plot<-ggplot(picrust_sigtab_pmf_coch_perl, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_perl_pmf_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/pmf-coch perl diff abun pathways.png", height=24, width=22)
...
picrust_perl_cush_pmf<-subset_samples(picrust_perl_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_perl <- phyloseq_to_deseq2(picrust_perl_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_perl <- DESeq(picrust_deseq_cush_pmf_perl, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_perl <- results(picrust_deseq_cush_pmf_perl, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_perl = picrust_res_deseq_cush_pmf_perl[which(picrust_res_deseq_cush_pmf_perl$padj < alpha), ]
picrust_sigtab_cush_pmf_perl = cbind(as(picrust_sigtab_cush_pmf_perl, "data.frame"), as(tax_table(picrust_perl_cush_pmf)[rownames(picrust_sigtab_cush_pmf_perl), ], "matrix"))
#31 diff abun pathways

picrust_diff_abund_perl_cush_pmf_plot<-ggplot(picrust_sigtab_cush_pmf_perl, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_perl_cush_pmf_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-pmf perl diff abun pathways.png", height=24, width=22)

#### Trichoptera ####

#Exposed vs non wastewater exposed sites

picrust_trichop_ps_Bow<-subset_samples(picrust_ps, Family=="Trichoptera" & Stream_Type=="Bow River")

picrust_deseq_exposed_trichop_Bow <- phyloseq_to_deseq2(picrust_trichop_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_trichop_Bow$Exposure <-relevel(picrust_deseq_exposed_trichop_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_trichop_Bow <- DESeq(picrust_deseq_exposed_trichop_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_trichop_Bow <- results(picrust_deseq_exposed_trichop_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_trichop_Bow = picrust_res_deseq_exposed_trichop_Bow[which(picrust_res_deseq_exposed_trichop_Bow$padj < alpha), ]
picrust_sigtab_exposed_trichop_Bow = cbind(as(picrust_sigtab_exposed_trichop_Bow, "data.frame"), as(tax_table(picrust_trichop_ps_Bow)[rownames(picrust_sigtab_exposed_trichop_Bow), ], "matrix"))
#28 diff abun path

picrust_diff_abund_trichop_Bow_plot<-ggplot(picrust_sigtab_exposed_trichop_Bow, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_trichop_Bow_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/exposed-non exposed trichop diff abun pathways.png", height=24, width=22)
...
picrust_trichop_ps_Bow_subset<-subset_samples(picrust_trichop_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_trichop_Bow_subset <- phyloseq_to_deseq2(picrust_trichop_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_trichop_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_trichop_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_trichop_Bow_subset <- DESeq(picrust_deseq_exposed_trichop_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_trichop_Bow_subset <- results(picrust_deseq_exposed_trichop_Bow_subset, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_trichop_Bow_subset = picrust_res_deseq_exposed_trichop_Bow_subset[which(picrust_res_deseq_exposed_trichop_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_trichop_Bow_subset = cbind(as(picrust_sigtab_exposed_trichop_Bow_subset, "data.frame"), as(tax_table(picrust_trichop_ps_Bow_subset)[rownames(picrust_sigtab_exposed_trichop_Bow_subset), ], "matrix"))

x_exposed_trichop_Bow_subset_picrust = tapply(picrust_sigtab_exposed_trichop_Bow_subset$log2FoldChange, picrust_sigtab_exposed_trichop_Bow_subset$description, function(x) max(x))

x_exposed_trichop_Bow_subset_picrust = sort(x_exposed_trichop_Bow_subset_picrust, TRUE)

picrust_sigtab_exposed_trichop_Bow_subset$description = factor(as.character(picrust_sigtab_exposed_trichop_Bow_subset$description), levels=names(x_exposed_trichop_Bow_subset_picrust))
#2 differentially expressed pathways

picrust_diff_abund_trichop_Bow_plot_subset<-ggplot(picrust_sigtab_exposed_trichop_Bow_subset, aes(x=log2FoldChange, y=description))+
  geom_point(pch=21, size=8, fill="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_trichop_Bow_plot_subset



#COCH - SUN

picrust_trichop_sun_coch<-subset_samples(picrust_trichop_ps_Bow, Site=="Cochrane" | Site=="Sunalta")

picrust_deseq_sun_coch_trichop <- phyloseq_to_deseq2(picrust_trichop_sun_coch, ~ Site)

picrust_deseq_sun_coch_trichop <- DESeq(picrust_deseq_sun_coch_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_sun_coch_trichop <- results(picrust_deseq_sun_coch_trichop, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_sun_coch_trichop = picrust_res_deseq_sun_coch_trichop[which(picrust_res_deseq_sun_coch_trichop$padj < alpha), ]
picrust_sigtab_sun_coch_trichop = cbind(as(picrust_sigtab_sun_coch_trichop, "data.frame"), as(tax_table(picrust_trichop_sun_coch)[rownames(picrust_sigtab_sun_coch_trichop), ], "matrix"))
#1

picrust_diff_abund_trichop_sun_coch_plot<-ggplot(picrust_sigtab_sun_coch_trichop, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_trichop_sun_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/sun-coch trichop diff abun pathways.png", height=24, width=22)


#CUSHBR - COCH

picrust_trichop_cush_coch<-subset_samples(picrust_trichop_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_trichop <- phyloseq_to_deseq2(picrust_trichop_cush_coch, ~ Site)

picrust_deseq_cush_coch_trichop <- DESeq(picrust_deseq_cush_coch_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_trichop <- results(picrust_deseq_cush_coch_trichop, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_trichop = picrust_res_deseq_cush_coch_trichop[which(picrust_res_deseq_cush_coch_trichop$padj < alpha), ]
picrust_sigtab_cush_coch_trichop = cbind(as(picrust_sigtab_cush_coch_trichop, "data.frame"), as(tax_table(picrust_trichop_cush_coch)[rownames(picrust_sigtab_cush_coch_trichop), ], "matrix"))
#2 diff abun pathways

picrust_diff_abund_trichop_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_trichop, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_trichop_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch trichop diff abun pathways.png", height=24, width=22)


#GRVBR - COCH

picrust_trichop_grvbr_coch<-subset_samples(picrust_trichop_ps_Bow, Site=="Graves Bridge" | Site=="Cochrane")

picrust_deseq_grvbr_coch_trichop <- phyloseq_to_deseq2(picrust_trichop_grvbr_coch, ~ Site)

picrust_deseq_grvbr_coch_trichop <- DESeq(picrust_deseq_grvbr_coch_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_grvbr_coch_trichop <- results(picrust_deseq_grvbr_coch_trichop, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_grvbr_coch_trichop = picrust_res_deseq_grvbr_coch_trichop[which(picrust_res_deseq_grvbr_coch_trichop$padj < alpha), ]
picrust_sigtab_grvbr_coch_trichop = cbind(as(picrust_sigtab_grvbr_coch_trichop, "data.frame"), as(tax_table(picrust_trichop_grvbr_coch)[rownames(picrust_sigtab_grvbr_coch_trichop), ], "matrix"))
#1 diff abun pathways

picrust_diff_abund_trichop_grvbr_coch_plot<-ggplot(picrust_sigtab_grvbr_coch_trichop, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y  = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_trichop_grvbr_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/grvbr-coch trichop diff abun pathways.png", height=24, width=22)
...
picrust_trichop_cush_sun<-subset_samples(picrust_trichop_ps_Bow, Site=="Cushing Bridge" | Site=="Sunalta")

picrust_deseq_cush_sun_trichop <- phyloseq_to_deseq2(picrust_trichop_cush_sun, ~ Site)

picrust_deseq_cush_sun_trichop <- DESeq(picrust_deseq_cush_sun_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_sun_trichop <- results(picrust_deseq_cush_sun_trichop, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_sun_trichop = picrust_res_deseq_cush_sun_trichop[which(picrust_res_deseq_cush_sun_trichop$padj < alpha), ]
picrust_sigtab_cush_sun_trichop = cbind(as(picrust_sigtab_cush_sun_trichop, "data.frame"), as(tax_table(picrust_trichop_cush_sun)[rownames(picrust_sigtab_cush_sun_trichop), ], "matrix"))
#1 diff abun pathways

picrust_diff_abund_trichop_cush_sun_plot<-ggplot(picrust_sigtab_cush_sun_trichop, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_trichop_cush_sun_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-sun trichop diff abun pathways.png", height=24, width=22)


#CUSHBR - GRVBR

picrust_trichop_cush_grvbr<-subset_samples(picrust_trichop_ps_Bow, Site=="Cushing Bridge" | Site=="Graves Bridge")

picrust_deseq_cush_grvbr_trichop <- phyloseq_to_deseq2(picrust_trichop_cush_grvbr, ~ Site)

picrust_deseq_cush_grvbr_trichop <- DESeq(picrust_deseq_cush_grvbr_trichop, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_grvbr_trichop <- results(picrust_deseq_cush_grvbr_trichop, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_grvbr_trichop = picrust_res_deseq_cush_grvbr_trichop[which(picrust_res_deseq_cush_grvbr_trichop$padj < alpha), ]
picrust_sigtab_cush_grvbr_trichop = cbind(as(picrust_sigtab_cush_grvbr_trichop, "data.frame"), as(tax_table(picrust_trichop_cush_grvbr)[rownames(picrust_sigtab_cush_grvbr_trichop), ], "matrix"))
#2 diff abun pathways

picrust_diff_abund_trichop_cush_grvbr_plot<-ggplot(picrust_sigtab_cush_grvbr_trichop, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_trichop_cush_grvbr_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-grvbr trichop diff abun pathways.png", height=24, width=22)
...
picrust_ephem_ps_Bow<-subset_samples(picrust_ps, Family=="Ephemeroptera" & Stream_Type=="Bow River")

picrust_deseq_exposed_ephem_Bow <- phyloseq_to_deseq2(picrust_ephem_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_ephem_Bow$Exposure <-relevel(picrust_deseq_exposed_ephem_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_ephem_Bow <- DESeq(picrust_deseq_exposed_ephem_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_ephem_Bow <- results(picrust_deseq_exposed_ephem_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_ephem_Bow = picrust_res_deseq_exposed_ephem_Bow[which(picrust_res_deseq_exposed_ephem_Bow$padj < alpha), ]
picrust_sigtab_exposed_ephem_Bow = cbind(as(picrust_sigtab_exposed_ephem_Bow, "data.frame"), as(tax_table(picrust_ephem_ps_Bow)[rownames(picrust_sigtab_exposed_ephem_Bow), ], "matrix"))
#34 diff abun path

picrust_diff_abund_ephem_Bow_plot<-ggplot(picrust_sigtab_exposed_ephem_Bow, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_ephem_Bow_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/exposed-non exposed ephem diff abun pathways.png", height=24, width=22)
...
picrust_ephem_ps_Bow_subset<-subset_samples(picrust_ephem_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_ephem_Bow_subset <- phyloseq_to_deseq2(picrust_ephem_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_ephem_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_ephem_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_ephem_Bow_subset <- DESeq(picrust_deseq_exposed_ephem_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_ephem_Bow_subset <- results(picrust_deseq_exposed_ephem_Bow_subset, cooksCutoff = FALSE)
picrust_res_deseq_exposed_ephem_Bow_subset
alpha = 0.001
picrust_sigtab_exposed_ephem_Bow_subset = picrust_res_deseq_exposed_ephem_Bow_subset[which(picrust_res_deseq_exposed_ephem_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_ephem_Bow_subset = cbind(as(picrust_sigtab_exposed_ephem_Bow_subset, "data.frame"), as(tax_table(picrust_ephem_ps_Bow_subset)[rownames(picrust_sigtab_exposed_ephem_Bow_subset), ], "matrix"))
#0


#COCH - SUN

picrust_ephem_sun_coch<-subset_samples(picrust_ephem_ps_Bow, Site=="Cochrane" | Site=="Sunalta")

picrust_deseq_sun_coch_ephem <- phyloseq_to_deseq2(picrust_ephem_sun_coch, ~ Site)

picrust_deseq_sun_coch_ephem <- DESeq(picrust_deseq_sun_coch_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_sun_coch_ephem <- results(picrust_deseq_sun_coch_ephem, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_sun_coch_ephem = picrust_res_deseq_sun_coch_ephem[which(picrust_res_deseq_sun_coch_ephem$padj < alpha), ]
picrust_sigtab_sun_coch_ephem = cbind(as(picrust_sigtab_sun_coch_ephem, "data.frame"), as(tax_table(picrust_ephem_sun_coch)[rownames(picrust_sigtab_sun_coch_ephem), ], "matrix"))
#0


#CUSHBR - COCH

picrust_ephem_cush_coch<-subset_samples(picrust_ephem_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_ephem <- phyloseq_to_deseq2(picrust_ephem_cush_coch, ~ Site)

picrust_deseq_cush_coch_ephem <- DESeq(picrust_deseq_cush_coch_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_ephem <- results(picrust_deseq_cush_coch_ephem, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_ephem = picrust_res_deseq_cush_coch_ephem[which(picrust_res_deseq_cush_coch_ephem$padj < alpha), ]
picrust_sigtab_cush_coch_ephem = cbind(as(picrust_sigtab_cush_coch_ephem, "data.frame"), as(tax_table(picrust_ephem_cush_coch)[rownames(picrust_sigtab_cush_coch_ephem), ], "matrix"))
#34 diff abun pathways

picrust_diff_abund_ephem_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_ephem, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_ephem_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch ephem diff abun pathways.png", height=24, width=22)


#GRVBR - COCH

picrust_ephem_grvbr_coch<-subset_samples(picrust_ephem_ps_Bow, Site=="Graves Bridge" | Site=="Cochrane")

picrust_deseq_grvbr_coch_ephem <- phyloseq_to_deseq2(picrust_ephem_grvbr_coch, ~ Site)

picrust_deseq_grvbr_coch_ephem <- DESeq(picrust_deseq_grvbr_coch_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_grvbr_coch_ephem <- results(picrust_deseq_grvbr_coch_ephem, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_grvbr_coch_ephem = picrust_res_deseq_grvbr_coch_ephem[which(picrust_res_deseq_grvbr_coch_ephem$padj < alpha), ]
picrust_sigtab_grvbr_coch_ephem = cbind(as(picrust_sigtab_grvbr_coch_ephem, "data.frame"), as(tax_table(picrust_ephem_grvbr_coch)[rownames(picrust_sigtab_grvbr_coch_ephem), ], "matrix"))
#4 diff abun pathways

picrust_diff_abund_ephem_grvbr_coch_plot<-ggplot(picrust_sigtab_grvbr_coch_ephem, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y  = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_ephem_grvbr_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/grvbr-coch ephem diff abun pathways.png", height=24, width=22)



#PMF - COCH

picrust_ephem_pmf_coch<-subset_samples(picrust_ephem_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_ephem <- phyloseq_to_deseq2(picrust_ephem_pmf_coch, ~ Site)

picrust_deseq_pmf_coch_ephem <- DESeq(picrust_deseq_pmf_coch_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_ephem <- results(picrust_deseq_pmf_coch_ephem, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_ephem = picrust_res_deseq_pmf_coch_ephem[which(picrust_res_deseq_pmf_coch_ephem$padj < alpha), ]
picrust_sigtab_pmf_coch_ephem = cbind(as(picrust_sigtab_pmf_coch_ephem, "data.frame"), as(tax_table(picrust_ephem_pmf_coch)[rownames(picrust_sigtab_pmf_coch_ephem), ], "matrix"))
#3 observations

picrust_diff_abund_ephem_pmf_coch_plot<-ggplot(picrust_sigtab_pmf_coch_ephem, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_ephem_pmf_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/pmf-coch ephem diff abun pathways.png", height=24, width=22)
...
picrust_ephem_cush_sun<-subset_samples(picrust_ephem_ps_Bow, Site=="Cushing Bridge" | Site=="Sunalta")

picrust_deseq_cush_sun_ephem <- phyloseq_to_deseq2(picrust_ephem_cush_sun, ~ Site)

picrust_deseq_cush_sun_ephem <- DESeq(picrust_deseq_cush_sun_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_sun_ephem <- results(picrust_deseq_cush_sun_ephem, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_sun_ephem = picrust_res_deseq_cush_sun_ephem[which(picrust_res_deseq_cush_sun_ephem$padj < alpha), ]
picrust_sigtab_cush_sun_ephem = cbind(as(picrust_sigtab_cush_sun_ephem, "data.frame"), as(tax_table(picrust_ephem_cush_sun)[rownames(picrust_sigtab_cush_sun_ephem), ], "matrix"))
#34 diff abun pathways

picrust_diff_abund_ephem_cush_sun_plot<-ggplot(picrust_sigtab_cush_sun_ephem, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_ephem_cush_sun_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-sun ephem diff abun pathways.png", height=24, width=22)


#CUSHBR - GRVBR

picrust_ephem_cush_grvbr<-subset_samples(picrust_ephem_ps_Bow, Site=="Cushing Bridge" | Site=="Graves Bridge")

picrust_deseq_cush_grvbr_ephem <- phyloseq_to_deseq2(picrust_ephem_cush_grvbr, ~ Site)

picrust_deseq_cush_grvbr_ephem <- DESeq(picrust_deseq_cush_grvbr_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_grvbr_ephem <- results(picrust_deseq_cush_grvbr_ephem, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_grvbr_ephem = picrust_res_deseq_cush_grvbr_ephem[which(picrust_res_deseq_cush_grvbr_ephem$padj < alpha), ]
picrust_sigtab_cush_grvbr_ephem = cbind(as(picrust_sigtab_cush_grvbr_ephem, "data.frame"), as(tax_table(picrust_ephem_cush_grvbr)[rownames(picrust_sigtab_cush_grvbr_ephem), ], "matrix"))
#33 diff abun pathways

picrust_diff_abund_ephem_cush_grvbr_plot<-ggplot(picrust_sigtab_cush_grvbr_ephem, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_ephem_cush_grvbr_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-grvbr ephem diff abun pathways.png", height=24, width=22)


#CUSHBR - PMF

picrust_ephem_cush_pmf<-subset_samples(picrust_ephem_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_ephem <- phyloseq_to_deseq2(picrust_ephem_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_ephem <- DESeq(picrust_deseq_cush_pmf_ephem, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_ephem <- results(picrust_deseq_cush_pmf_ephem, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_ephem = picrust_res_deseq_cush_pmf_ephem[which(picrust_res_deseq_cush_pmf_ephem$padj < alpha), ]
picrust_sigtab_cush_pmf_ephem = cbind(as(picrust_sigtab_cush_pmf_ephem, "data.frame"), as(tax_table(picrust_ephem_cush_pmf)[rownames(picrust_sigtab_cush_pmf_ephem), ], "matrix"))
#0 diff abun pathways
...
picrust_dip_ps_Bow<-subset_samples(picrust_ps, Family=="Diptera" & Stream_Type=="Bow River")

picrust_deseq_exposed_dip_Bow <- phyloseq_to_deseq2(picrust_dip_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_dip_Bow$Exposure <-relevel(picrust_deseq_exposed_dip_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_dip_Bow <- DESeq(picrust_deseq_exposed_dip_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_dip_Bow <- results(picrust_deseq_exposed_dip_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_dip_Bow = picrust_res_deseq_exposed_dip_Bow[which(picrust_res_deseq_exposed_dip_Bow$padj < alpha), ]
picrust_sigtab_exposed_dip_Bow = cbind(as(picrust_sigtab_exposed_dip_Bow, "data.frame"), as(tax_table(picrust_dip_ps_Bow)[rownames(picrust_sigtab_exposed_dip_Bow), ], "matrix"))
#0 diff abun path

picrust_diff_abund_dip_Bow_plot<-ggplot(picrust_sigtab_exposed_dip_Bow, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_dip_Bow_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/exposed-non exposed dip diff abun pathways.png", height=24, width=22)
...
picrust_dip_ps_Bow_subset<-subset_samples(picrust_dip_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_dip_Bow_subset <- phyloseq_to_deseq2(picrust_dip_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_dip_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_dip_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_dip_Bow_subset <- DESeq(picrust_deseq_exposed_dip_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_dip_Bow_subset <- results(picrust_deseq_exposed_dip_Bow_subset, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_dip_Bow_subset = picrust_res_deseq_exposed_dip_Bow_subset[which(picrust_res_deseq_exposed_dip_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_dip_Bow_subset = cbind(as(picrust_sigtab_exposed_dip_Bow_subset, "data.frame"), as(tax_table(picrust_dip_ps_Bow_subset)[rownames(picrust_sigtab_exposed_dip_Bow_subset), ], "matrix"))
#0
...
picrust_dip_cush_coch<-subset_samples(picrust_dip_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_dip <- phyloseq_to_deseq2(picrust_dip_cush_coch, ~ Site)

picrust_deseq_cush_coch_dip <- DESeq(picrust_deseq_cush_coch_dip, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_dip <- results(picrust_deseq_cush_coch_dip, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_dip = picrust_res_deseq_cush_coch_dip[which(picrust_res_deseq_cush_coch_dip$padj < alpha), ]
picrust_sigtab_cush_coch_dip = cbind(as(picrust_sigtab_cush_coch_dip, "data.frame"), as(tax_table(picrust_dip_cush_coch)[rownames(picrust_sigtab_cush_coch_dip), ], "matrix"))
#50 diff abun pathways

picrust_diff_abund_dip_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_dip, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_dip_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch dip diff abun pathways.png", height=24, width=22)


#PMF - COCH

picrust_dip_pmf_coch<-subset_samples(picrust_dip_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_dip <- phyloseq_to_deseq2(picrust_dip_pmf_coch, ~ Site)

picrust_deseq_pmf_coch_dip <- DESeq(picrust_deseq_pmf_coch_dip, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_dip <- results(picrust_deseq_pmf_coch_dip, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_dip = picrust_res_deseq_pmf_coch_dip[which(picrust_res_deseq_pmf_coch_dip$padj < alpha), ]
picrust_sigtab_pmf_coch_dip = cbind(as(picrust_sigtab_pmf_coch_dip, "data.frame"), as(tax_table(picrust_dip_pmf_coch)[rownames(picrust_sigtab_pmf_coch_dip), ], "matrix"))
#32 observations

picrust_diff_abund_dip_pmf_coch_plot<-ggplot(picrust_sigtab_pmf_coch_dip, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_dip_pmf_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/pmf-coch dip diff abun pathways.png", height=24, width=22)


#CUSHBR - PMF

picrust_dip_cush_pmf<-subset_samples(picrust_dip_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_dip <- phyloseq_to_deseq2(picrust_dip_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_dip <- DESeq(picrust_deseq_cush_pmf_dip, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_dip <- results(picrust_deseq_cush_pmf_dip, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_dip = picrust_res_deseq_cush_pmf_dip[which(picrust_res_deseq_cush_pmf_dip$padj < alpha), ]
picrust_sigtab_cush_pmf_dip = cbind(as(picrust_sigtab_cush_pmf_dip, "data.frame"), as(tax_table(picrust_dip_cush_pmf)[rownames(picrust_sigtab_cush_pmf_dip), ], "matrix"))
#0 diff abun pathways
...
picrust_aran_ps_Bow<-subset_samples(picrust_ps, Family=="Araneidae" & Stream_Type=="Bow River")

picrust_deseq_exposed_aran_Bow <- phyloseq_to_deseq2(picrust_aran_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_aran_Bow$Exposure <-relevel(picrust_deseq_exposed_aran_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_aran_Bow <- DESeq(picrust_deseq_exposed_aran_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_aran_Bow <- results(picrust_deseq_exposed_aran_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_aran_Bow = picrust_res_deseq_exposed_aran_Bow[which(picrust_res_deseq_exposed_aran_Bow$padj < alpha), ]
picrust_sigtab_exposed_aran_Bow = cbind(as(picrust_sigtab_exposed_aran_Bow, "data.frame"), as(tax_table(picrust_aran_ps_Bow)[rownames(picrust_sigtab_exposed_aran_Bow), ], "matrix"))
#0 diff abun path
...
picrust_aran_ps_Bow_subset<-subset_samples(picrust_aran_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_aran_Bow_subset <- phyloseq_to_deseq2(picrust_aran_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_aran_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_aran_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_aran_Bow_subset <- DESeq(picrust_deseq_exposed_aran_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_aran_Bow_subset <- results(picrust_deseq_exposed_aran_Bow_subset, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_aran_Bow_subset = picrust_res_deseq_exposed_aran_Bow_subset[which(picrust_res_deseq_exposed_aran_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_aran_Bow_subset = cbind(as(picrust_sigtab_exposed_aran_Bow_subset, "data.frame"), as(tax_table(picrust_aran_ps_Bow_subset)[rownames(picrust_sigtab_exposed_aran_Bow_subset), ], "matrix"))

x_exposed_aran_Bow_subset_picrust = tapply(picrust_sigtab_exposed_aran_Bow_subset$log2FoldChange, picrust_sigtab_exposed_aran_Bow_subset$description, function(x) max(x))

x_exposed_aran_Bow_subset_picrust = sort(x_exposed_aran_Bow_subset_picrust, TRUE)

picrust_sigtab_exposed_aran_Bow_subset$description = factor(as.character(picrust_sigtab_exposed_aran_Bow_subset$description), levels=names(x_exposed_aran_Bow_subset_picrust))
#3 differentially expressed pathways

picrust_diff_abund_aran_Bow_plot_subset<-ggplot(picrust_sigtab_exposed_aran_Bow_subset, aes(x=log2FoldChange, y=description))+
  geom_point(pch=21, size=8, fill="black") + 
  theme_bw()+
  theme(axis.text.y = element_text(size=40, face="italic", color="black")) + theme(axis.text.x=element_text(size=34, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_aran_Bow_plot_subset


#COCH - SUN

picrust_aran_sun_coch<-subset_samples(picrust_aran_ps_Bow, Site=="Cochrane" | Site=="Sunalta")

picrust_deseq_sun_coch_aran <- phyloseq_to_deseq2(picrust_aran_sun_coch, ~ Site)

picrust_deseq_sun_coch_aran <- DESeq(picrust_deseq_sun_coch_aran, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_sun_coch_aran <- results(picrust_deseq_sun_coch_aran, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_sun_coch_aran = picrust_res_deseq_sun_coch_aran[which(picrust_res_deseq_sun_coch_aran$padj < alpha), ]
picrust_sigtab_sun_coch_aran = cbind(as(picrust_sigtab_sun_coch_aran, "data.frame"), as(tax_table(picrust_aran_sun_coch)[rownames(picrust_sigtab_sun_coch_aran), ], "matrix"))
#27

picrust_diff_abund_aran_sun_coch_plot<-ggplot(picrust_sigtab_sun_coch_aran, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_aran_sun_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/sun-coch aran diff abun pathways.png", height=24, width=22)


#CUSHBR - COCH

picrust_aran_cush_coch<-subset_samples(picrust_aran_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_aran <- phyloseq_to_deseq2(picrust_aran_cush_coch, ~ Site)

picrust_deseq_cush_coch_aran <- DESeq(picrust_deseq_cush_coch_aran, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_aran <- results(picrust_deseq_cush_coch_aran, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_aran = picrust_res_deseq_cush_coch_aran[which(picrust_res_deseq_cush_coch_aran$padj < alpha), ]
picrust_sigtab_cush_coch_aran = cbind(as(picrust_sigtab_cush_coch_aran, "data.frame"), as(tax_table(picrust_aran_cush_coch)[rownames(picrust_sigtab_cush_coch_aran), ], "matrix"))
#0 diff abun pathways


#GRVBR - COCH

picrust_aran_grvbr_coch<-subset_samples(picrust_aran_ps_Bow, Site=="Graves Bridge" | Site=="Cochrane")

picrust_deseq_grvbr_coch_aran <- phyloseq_to_deseq2(picrust_aran_grvbr_coch, ~ Site)

picrust_deseq_grvbr_coch_aran <- DESeq(picrust_deseq_grvbr_coch_aran, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_grvbr_coch_aran <- results(picrust_deseq_grvbr_coch_aran, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_grvbr_coch_aran = picrust_res_deseq_grvbr_coch_aran[which(picrust_res_deseq_grvbr_coch_aran$padj < alpha), ]
picrust_sigtab_grvbr_coch_aran = cbind(as(picrust_sigtab_grvbr_coch_aran, "data.frame"), as(tax_table(picrust_aran_grvbr_coch)[rownames(picrust_sigtab_grvbr_coch_aran), ], "matrix"))
#0 diff abun pathways


#PMF - COCH

picrust_aran_pmf_coch<-subset_samples(picrust_aran_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_aran <- phyloseq_to_deseq2(picrust_aran_pmf_coch, ~ Site)

picrust_deseq_pmf_coch_aran <- DESeq(picrust_deseq_pmf_coch_aran, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_aran <- results(picrust_deseq_pmf_coch_aran, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_aran = picrust_res_deseq_pmf_coch_aran[which(picrust_res_deseq_pmf_coch_aran$padj < alpha), ]
picrust_sigtab_pmf_coch_aran = cbind(as(picrust_sigtab_pmf_coch_aran, "data.frame"), as(tax_table(picrust_aran_pmf_coch)[rownames(picrust_sigtab_pmf_coch_aran), ], "matrix"))
#176 observations

picrust_diff_abund_aran_pmf_coch_plot<-ggplot(picrust_sigtab_pmf_coch_aran, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_aran_pmf_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/pmf-coch aran diff abun pathways.png", height=24, width=22)
...
picrust_aran_cush_sun<-subset_samples(picrust_aran_ps_Bow, Site=="Cushing Bridge" | Site=="Sunalta")

picrust_deseq_cush_sun_aran <- phyloseq_to_deseq2(picrust_aran_cush_sun, ~ Site)

picrust_deseq_cush_sun_aran <- DESeq(picrust_deseq_cush_sun_aran, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_sun_aran <- results(picrust_deseq_cush_sun_aran, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_sun_aran = picrust_res_deseq_cush_sun_aran[which(picrust_res_deseq_cush_sun_aran$padj < alpha), ]
picrust_sigtab_cush_sun_aran = cbind(as(picrust_sigtab_cush_sun_aran, "data.frame"), as(tax_table(picrust_aran_cush_sun)[rownames(picrust_sigtab_cush_sun_aran), ], "matrix"))
#2 diff abun pathways


#CUSHBR - GRVBR

picrust_aran_cush_grvbr<-subset_samples(picrust_aran_ps_Bow, Site=="Cushing Bridge" | Site=="Graves Bridge")

picrust_deseq_cush_grvbr_aran <- phyloseq_to_deseq2(picrust_aran_cush_grvbr, ~ Site)

picrust_deseq_cush_grvbr_aran <- DESeq(picrust_deseq_cush_grvbr_aran, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_grvbr_aran <- results(picrust_deseq_cush_grvbr_aran, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_grvbr_aran = picrust_res_deseq_cush_grvbr_aran[which(picrust_res_deseq_cush_grvbr_aran$padj < alpha), ]
picrust_sigtab_cush_grvbr_aran = cbind(as(picrust_sigtab_cush_grvbr_aran, "data.frame"), as(tax_table(picrust_aran_cush_grvbr)[rownames(picrust_sigtab_cush_grvbr_aran), ], "matrix"))
#0 diff abun pathways


#CUSHBR - pmf

picrust_aran_cush_pmf<-subset_samples(picrust_aran_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_aran <- phyloseq_to_deseq2(picrust_aran_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_aran <- DESeq(picrust_deseq_cush_pmf_aran, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_aran <- results(picrust_deseq_cush_pmf_aran, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_aran = picrust_res_deseq_cush_pmf_aran[which(picrust_res_deseq_cush_pmf_aran$padj < alpha), ]
picrust_sigtab_cush_pmf_aran = cbind(as(picrust_sigtab_cush_pmf_aran, "data.frame"), as(tax_table(picrust_aran_cush_pmf)[rownames(picrust_sigtab_cush_pmf_aran), ], "matrix"))
#133 diff abun pathways
...
picrust_tetra_ps_Bow<-subset_samples(picrust_ps, Family=="Tetragnathidae" & Stream_Type=="Bow River")

picrust_deseq_exposed_tetra_Bow <- phyloseq_to_deseq2(picrust_tetra_ps_Bow, ~ Exposure)
#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_tetra_Bow$Exposure <-relevel(picrust_deseq_exposed_tetra_Bow$Exposure, "Unexposed")

picrust_deseq_exposed_tetra_Bow <- DESeq(picrust_deseq_exposed_tetra_Bow, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_tetra_Bow <- results(picrust_deseq_exposed_tetra_Bow, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_tetra_Bow = picrust_res_deseq_exposed_tetra_Bow[which(picrust_res_deseq_exposed_tetra_Bow$padj < alpha), ]
picrust_sigtab_exposed_tetra_Bow = cbind(as(picrust_sigtab_exposed_tetra_Bow, "data.frame"), as(tax_table(picrust_tetra_ps_Bow)[rownames(picrust_sigtab_exposed_tetra_Bow), ], "matrix"))
#0 diff abun path
...
picrust_tetra_ps_Bow_subset<-subset_samples(picrust_tetra_ps_Bow, Site!="Cochrane" & Site!="Sunalta")

picrust_deseq_exposed_tetra_Bow_subset <- phyloseq_to_deseq2(picrust_tetra_ps_Bow_subset, ~ Exposure)

#Re level the reference variable so that the comparison is exposed vs unexposed
picrust_deseq_exposed_tetra_Bow_subset$Exposure <-relevel(picrust_deseq_exposed_tetra_Bow_subset$Exposure, "Unexposed")

picrust_deseq_exposed_tetra_Bow_subset <- DESeq(picrust_deseq_exposed_tetra_Bow_subset, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_exposed_tetra_Bow_subset <- results(picrust_deseq_exposed_tetra_Bow_subset, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_exposed_tetra_Bow_subset = picrust_res_deseq_exposed_tetra_Bow_subset[which(picrust_res_deseq_exposed_tetra_Bow_subset$padj < alpha), ]

picrust_sigtab_exposed_tetra_Bow_subset = cbind(as(picrust_sigtab_exposed_tetra_Bow_subset, "data.frame"), as(tax_table(picrust_tetra_ps_Bow_subset)[rownames(picrust_sigtab_exposed_tetra_Bow_subset), ], "matrix"))
#0


#COCH - SUN

picrust_tetra_sun_coch<-subset_samples(picrust_tetra_ps_Bow, Site=="Cochrane" | Site=="Sunalta")

picrust_deseq_sun_coch_tetra <- phyloseq_to_deseq2(picrust_tetra_sun_coch, ~ Site)

picrust_deseq_sun_coch_tetra <- DESeq(picrust_deseq_sun_coch_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_sun_coch_tetra <- results(picrust_deseq_sun_coch_tetra, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_sun_coch_tetra = picrust_res_deseq_sun_coch_tetra[which(picrust_res_deseq_sun_coch_tetra$padj < alpha), ]
picrust_sigtab_sun_coch_tetra = cbind(as(picrust_sigtab_sun_coch_tetra, "data.frame"), as(tax_table(picrust_tetra_sun_coch)[rownames(picrust_sigtab_sun_coch_tetra), ], "matrix"))
#2

picrust_diff_abund_tetra_sun_coch_plot<-ggplot(picrust_sigtab_sun_coch_tetra, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_tetra_sun_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/sun-coch tetra diff abun pathways.png", height=24, width=22)



#CUSHBR - COCH

picrust_tetra_cush_coch<-subset_samples(picrust_tetra_ps_Bow, Site=="Cushing Bridge" | Site=="Cochrane")

picrust_deseq_cush_coch_tetra <- phyloseq_to_deseq2(picrust_tetra_cush_coch, ~ Site)

picrust_deseq_cush_coch_tetra <- DESeq(picrust_deseq_cush_coch_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_coch_tetra <- results(picrust_deseq_cush_coch_tetra, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_coch_tetra = picrust_res_deseq_cush_coch_tetra[which(picrust_res_deseq_cush_coch_tetra$padj < alpha), ]
picrust_sigtab_cush_coch_tetra = cbind(as(picrust_sigtab_cush_coch_tetra, "data.frame"), as(tax_table(picrust_tetra_cush_coch)[rownames(picrust_sigtab_cush_coch_tetra), ], "matrix"))
#19 diff abun pathways

picrust_diff_abund_tetra_cush_coch_plot<-ggplot(picrust_sigtab_cush_coch_tetra, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_tetra_cush_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-coch tetra diff abun pathways.png", height=24, width=22)


#GRVBR - COCH

picrust_tetra_grvbr_coch<-subset_samples(picrust_tetra_ps_Bow, Site=="Graves Bridge" | Site=="Cochrane")

picrust_deseq_grvbr_coch_tetra <- phyloseq_to_deseq2(picrust_tetra_grvbr_coch, ~ Site)

picrust_deseq_grvbr_coch_tetra <- DESeq(picrust_deseq_grvbr_coch_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_grvbr_coch_tetra <- results(picrust_deseq_grvbr_coch_tetra, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_grvbr_coch_tetra = picrust_res_deseq_grvbr_coch_tetra[which(picrust_res_deseq_grvbr_coch_tetra$padj < alpha), ]
picrust_sigtab_grvbr_coch_tetra = cbind(as(picrust_sigtab_grvbr_coch_tetra, "data.frame"), as(tax_table(picrust_tetra_grvbr_coch)[rownames(picrust_sigtab_grvbr_coch_tetra), ], "matrix"))
#2 diff abun pathways

picrust_diff_abund_tetra_grvbr_coch_plot<-ggplot(picrust_sigtab_grvbr_coch_tetra, aes(y=description, x=log2FoldChange))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y  = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_tetra_grvbr_coch_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/grvbr-coch tetra diff abun pathways.png", height=24, width=22)


#PMF - COCH

picrust_tetra_pmf_coch<-subset_samples(picrust_tetra_ps_Bow, Site=="Policeman Flats" | Site=="Cochrane")

picrust_deseq_pmf_coch_tetra <- phyloseq_to_deseq2(picrust_tetra_pmf_coch, ~ Site)

picrust_deseq_pmf_coch_tetra <- DESeq(picrust_deseq_pmf_coch_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_pmf_coch_tetra <- results(picrust_deseq_pmf_coch_tetra, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_pmf_coch_tetra = picrust_res_deseq_pmf_coch_tetra[which(picrust_res_deseq_pmf_coch_tetra$padj < alpha), ]
picrust_sigtab_pmf_coch_tetra = cbind(as(picrust_sigtab_pmf_coch_tetra, "data.frame"), as(tax_table(picrust_tetra_pmf_coch)[rownames(picrust_sigtab_pmf_coch_tetra), ], "matrix"))
#0 observations
...
picrust_tetra_cush_sun<-subset_samples(picrust_tetra_ps_Bow, Site=="Cushing Bridge" | Site=="Sunalta")

picrust_deseq_cush_sun_tetra <- phyloseq_to_deseq2(picrust_tetra_cush_sun, ~ Site)

picrust_deseq_cush_sun_tetra <- DESeq(picrust_deseq_cush_sun_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_sun_tetra <- results(picrust_deseq_cush_sun_tetra, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_sun_tetra = picrust_res_deseq_cush_sun_tetra[which(picrust_res_deseq_cush_sun_tetra$padj < alpha), ]
picrust_sigtab_cush_sun_tetra = cbind(as(picrust_sigtab_cush_sun_tetra, "data.frame"), as(tax_table(picrust_tetra_cush_sun)[rownames(picrust_sigtab_cush_sun_tetra), ], "matrix"))
#1 diff abun pathways

picrust_diff_abund_tetra_cush_sun_plot<-ggplot(picrust_sigtab_cush_sun_tetra, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_tetra_cush_sun_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-sun tetra diff abun pathways.png", height=24, width=22)


#CUSHBR - GRVBR

picrust_tetra_cush_grvbr<-subset_samples(picrust_tetra_ps_Bow, Site=="Cushing Bridge" | Site=="Graves Bridge")

picrust_deseq_cush_grvbr_tetra <- phyloseq_to_deseq2(picrust_tetra_cush_grvbr, ~ Site)

picrust_deseq_cush_grvbr_tetra <- DESeq(picrust_deseq_cush_grvbr_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_grvbr_tetra <- results(picrust_deseq_cush_grvbr_tetra, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_grvbr_tetra = picrust_res_deseq_cush_grvbr_tetra[which(picrust_res_deseq_cush_grvbr_tetra$padj < alpha), ]
picrust_sigtab_cush_grvbr_tetra = cbind(as(picrust_sigtab_cush_grvbr_tetra, "data.frame"), as(tax_table(picrust_tetra_cush_grvbr)[rownames(picrust_sigtab_cush_grvbr_tetra), ], "matrix"))
#0 diff abun pathways


#CUSHBR - PMF

picrust_tetra_cush_pmf<-subset_samples(picrust_tetra_ps_Bow, Site=="Cushing Bridge" | Site=="Policeman Flats")

picrust_deseq_cush_pmf_tetra <- phyloseq_to_deseq2(picrust_tetra_cush_pmf, ~ Site)

picrust_deseq_cush_pmf_tetra <- DESeq(picrust_deseq_cush_pmf_tetra, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_cush_pmf_tetra <- results(picrust_deseq_cush_pmf_tetra, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_cush_pmf_tetra = picrust_res_deseq_cush_pmf_tetra[which(picrust_res_deseq_cush_pmf_tetra$padj < alpha), ]
picrust_sigtab_cush_pmf_tetra = cbind(as(picrust_sigtab_cush_pmf_tetra, "data.frame"), as(tax_table(picrust_tetra_cush_pmf)[rownames(picrust_sigtab_cush_pmf_tetra), ], "matrix"))
#8 diff abun pathways

picrust_diff_abund_tetra_cush_pmf_plot<-ggplot(picrust_sigtab_cush_pmf_tetra, aes(x=log2FoldChange, y=description))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=24, face="italic", color="black")) + theme(axis.text.x=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
picrust_diff_abund_tetra_cush_pmf_plot

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/cushbr-pmf tetra diff abun pathways.png", height=24, width=22)
...
picrust_hydro_ps_ACWA<-subset_samples(picrust_ps, Family=="Hydropsychidae" & Stream_Type=="ACWA Streams")

picrust_hydro_REF_EFF5<-subset_samples(picrust_hydro_ps_ACWA, Site=="BRR2" | Site=="PCR1")

picrust_deseq_REF_EFF5_hydro_ACWA <- phyloseq_to_deseq2(picrust_hydro_REF_EFF5, ~ Site)

picrust_deseq_REF_EFF5_hydro_ACWA <- DESeq(picrust_deseq_REF_EFF5_hydro_ACWA, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_REF_EFF5_hydro_ACWA <- results(picrust_deseq_REF_EFF5_hydro_ACWA, cooksCutoff = FALSE)
picrust_res_deseq_REF_EFF5_hydro_ACWA
alpha = 0.001
picrust_sigtab_REF_EFF5_hydro_ACWA = picrust_res_deseq_REF_EFF5_hydro_ACWA[which(picrust_res_deseq_REF_EFF5_hydro_ACWA$padj < alpha), ]
picrust_sigtab_REF_EFF5_hydro_ACWA = cbind(as(picrust_sigtab_REF_EFF5_hydro_ACWA, "data.frame"), as(tax_table(picrust_hydro_ps_ACWA)[rownames(picrust_sigtab_REF_EFF5_hydro_ACWA), ], "matrix"))
#4 diff abun pathways

ggplot(picrust_sigtab_REF_EFF5_hydro_ACWA, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=60, color="black"), axis.text.x=element_text(size=60, color="black"), axis.title = element_blank())+
  guides(color=guide_legend(nrow=3, byrow=TRUE))+
  rremove("grid")+
  scale_color_brewer(palette = "Set3")

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/EFF5-REF hydro diff abun pathways ACWA.png", height=14, width=34)


#REF - EFF15

picrust_hydro_REF_EFF15<-subset_samples(picrust_hydro_ps_ACWA, Site=="BRR2" | Site=="PCR3")

picrust_deseq_REF_EFF15_hydro_ACWA <- phyloseq_to_deseq2(picrust_hydro_REF_EFF15, ~ Site)

picrust_deseq_REF_EFF15_hydro_ACWA <- DESeq(picrust_deseq_REF_EFF15_hydro_ACWA, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_REF_EFF15_hydro_ACWA <- results(picrust_deseq_REF_EFF15_hydro_ACWA, cooksCutoff = FALSE)
picrust_res_deseq_REF_EFF15_hydro_ACWA
alpha = 0.001
picrust_sigtab_REF_EFF15_hydro_ACWA = picrust_res_deseq_REF_EFF15_hydro_ACWA[which(picrust_res_deseq_REF_EFF15_hydro_ACWA$padj < alpha), ]
picrust_sigtab_REF_EFF15_hydro_ACWA = cbind(as(picrust_sigtab_REF_EFF15_hydro_ACWA, "data.frame"), as(tax_table(picrust_hydro_ps_ACWA)[rownames(picrust_sigtab_REF_EFF15_hydro_ACWA), ], "matrix"))
#4 diff abun pathways

ggplot(picrust_sigtab_REF_EFF15_hydro_ACWA, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=60, color="black"), axis.text.x=element_text(size=60, color="black"), axis.title = element_blank())+
  guides(color=guide_legend(nrow=3, byrow=TRUE))+
...
#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/EFF15-REF hydro diff abun pathways ACWA.png", height=14, width=34)


#EFF5 - EFF15

picrust_hydro_EFF5_EFF15<-subset_samples(picrust_hydro_ps_ACWA, Site=="PCR1" | Site=="PCR3")

picrust_deseq_EFF5_EFF15_hydro_ACWA <- phyloseq_to_deseq2(picrust_hydro_EFF5_EFF15, ~ Site)

picrust_deseq_EFF5_EFF15_hydro_ACWA <- DESeq(picrust_deseq_EFF5_EFF15_hydro_ACWA, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_EFF5_EFF15_hydro_ACWA <- results(picrust_deseq_EFF5_EFF15_hydro_ACWA, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_EFF5_EFF15_hydro_ACWA = picrust_res_deseq_EFF5_EFF15_hydro_ACWA[which(picrust_res_deseq_EFF5_EFF15_hydro_ACWA$padj < alpha), ]
picrust_sigtab_EFF5_EFF15_hydro_ACWA = cbind(as(picrust_sigtab_EFF5_EFF15_hydro_ACWA, "data.frame"), as(tax_table(picrust_hydro_ps_ACWA)[rownames(picrust_sigtab_EFF5_EFF15_hydro_ACWA), ], "matrix"))
#0 diff abun pathways
...
picrust_baetid_ps_ACWA<-subset_samples(picrust_ps, Family=="Baetidae" & Stream_Type=="ACWA Streams")

picrust_baetid_EFF5_EFF15<-subset_samples(picrust_baetid_ps_ACWA, Site=="PCR1" | Site=="PCR3")

picrust_deseq_EFF5_EFF15_baetid_ACWA <- phyloseq_to_deseq2(picrust_baetid_EFF5_EFF15, ~ Site)

picrust_deseq_EFF5_EFF15_baetid_ACWA <- DESeq(picrust_deseq_EFF5_EFF15_baetid_ACWA, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_EFF5_EFF15_baetid_ACWA <- results(picrust_deseq_EFF5_EFF15_baetid_ACWA, cooksCutoff = FALSE)
alpha = 0.001
picrust_sigtab_EFF5_EFF15_baetid_ACWA = picrust_res_deseq_EFF5_EFF15_baetid_ACWA[which(picrust_res_deseq_EFF5_EFF15_baetid_ACWA$padj < alpha), ]
picrust_sigtab_EFF5_EFF15_baetid_ACWA = cbind(as(picrust_sigtab_EFF5_EFF15_baetid_ACWA, "data.frame"), as(tax_table(picrust_baetid_ps_ACWA)[rownames(picrust_sigtab_EFF5_EFF15_baetid_ACWA), ], "matrix"))
#0 diff abun pathways
...
picrust_ephem_ps_ACWA<-subset_samples(picrust_ps, Family=="Ephemeroptera" & Stream_Type=="ACWA Streams")

picrust_ephem_EFF5_EFF15<-subset_samples(picrust_ephem_ps_ACWA, Site=="PCR1" | Site=="PCR3")

picrust_deseq_EFF5_EFF15_ephem_ACWA <- phyloseq_to_deseq2(picrust_ephem_EFF5_EFF15, ~ Site)

picrust_deseq_EFF5_EFF15_ephem_ACWA <- DESeq(picrust_deseq_EFF5_EFF15_ephem_ACWA, test = "Wald", fitType = "parametric", sfType = "poscounts")

picrust_res_deseq_EFF5_EFF15_ephem_ACWA <- results(picrust_deseq_EFF5_EFF15_ephem_ACWA, cooksCutoff = FALSE)
picrust_res_deseq_EFF5_EFF15_ephem_ACWA
alpha = 0.001
picrust_sigtab_EFF5_EFF15_ephem_ACWA = picrust_res_deseq_EFF5_EFF15_ephem_ACWA[which(picrust_res_deseq_EFF5_EFF15_ephem_ACWA$padj < alpha), ]
picrust_sigtab_EFF5_EFF15_ephem_ACWA = cbind(as(picrust_sigtab_EFF5_EFF15_ephem_ACWA, "data.frame"), as(tax_table(picrust_ephem_ps_ACWA)[rownames(picrust_sigtab_EFF5_EFF15_ephem_ACWA), ], "matrix"))
#2 diff abun pathways

ggplot(picrust_sigtab_EFF5_EFF15_ephem_ACWA, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=60, color="black"), axis.text.x=element_text(size=60, color="black"), axis.title = element_blank())+
  guides(color=guide_legend(nrow=3, byrow=TRUE))+
...
#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/EFF5-EFF15 ephem diff abun pathways ACWA.png", height=10, width=30)


#### All taxa plotted together Bow River ####

# Hydropsychidae #
picrust_diff_abund_hydro_Bow_plot_1<- picrust_diff_abund_hydro_Bow_plot_subset + rremove("axis.title") + rremove("legend")
picrust_diff_abund_hydro_Bow_plot_1

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/picrust DA hydro lar cushbr as reference.png", height=10, width=20, bg="white")

# Heptageniidae #
picrust_diff_abund_hep_plot_1<- picrust_diff_abund_hep_Bow_plot_subset + rremove("axis.title")+ rremove("legend")
picrust_diff_abund_hep_plot_1

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/picrust DA hep lar cushbr as reference.png", height=10, width=26, bg="white")

# Chironomidae #
picrust_diff_abund_chiro_plot_1<- picrust_diff_abund_chiro_Bow_plot_subset + rremove("axis.title")+ rremove("legend")
picrust_diff_abund_chiro_plot_1

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/picrust DA chiro lar cushbr as reference.png", height=10, width=24, bg="white")

# Perlidae #
picrust_diff_abund_perl_plot_1<- picrust_diff_abund_perl_Bow_plot_subset + rremove("axis.title")+ rremove("legend")
picrust_diff_abund_perl_plot_1

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/picrust DA perl lar cushbr as reference.png", height=16, width=34, bg="white")

# Trichoptera #
picrust_diff_abund_trichop_plot_1<- picrust_diff_abund_trichop_Bow_plot_subset + rremove("axis.title")+ rremove("legend")
picrust_diff_abund_trichop_plot_1

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/picrust DA trichop ad cushbr as reference.png", height=10, width=24, bg="white")

# Araneidae #

picrust_diff_abund_aran_Bow_plot_1<-picrust_diff_abund_aran_Bow_plot_subset + rremove("axis.title")+ rremove("legend")
picrust_diff_abund_aran_Bow_plot_1

#ggsave(filename="microbiome_analysis/PICRUSt2 Data/Figures/picrust DA aran cushbr as reference.png", height=10, width=30, bg="white")


#### Taxa plotted all together ACWA ####

# Hydropsychidae ACWA #

picrust_diff_abund_hydro_ACWA_plot_REF_EFF5<-ggplot(picrust_sigtab_REF_EFF5_hydro_ACWA, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=18, face="italic", color="black")) + theme(axis.text.x=element_text(size=18, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
          theme(legend.title = element_text(size=34, color="black"), legend.position="bottom")+
  guides(color=guide_legend(nrow=3, byrow=TRUE))+
  rremove("grid")+
...
picrust_diff_abund_hydro_ACWA_plot_REF_EFF5


picrust_diff_abund_hydro_ACWA_plot_REF_EFF15<-ggplot(picrust_sigtab_REF_EFF15_hydro_ACWA, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=18, face="italic", color="black")) + theme(axis.text.x=element_text(size=18, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
...
picrust_diff_abund_hydro_ACWA_plot_REF_EFF15

# Ephemeroptera ACWA #

picrust_diff_abund_ephem_ACWA_plot<-ggplot(picrust_sigtab_EFF5_EFF15_ephem_ACWA, aes(x=log2FoldChange, y=description))+
  geom_point(size=8) + 
  theme_bw()+
  theme(axis.text.y = element_text(size=28, face="italic", color="black")) + theme(axis.text.x=element_text(size=28, color="black"), axis.title=element_text(size=36)) +
          theme(legend.text=element_text(size=34, color="black")) + 
          theme(legend.title = element_text(size=34, color="black"), legend.position="bottom")+
  guides(color=guide_legend(nrow=3, byrow=TRUE))+
  rremove("grid")+
...
picrust_diff_abund_ephem_ACWA_plot

# Hydropsychidae #
picrust_diff_abund_hydro_ACWA_REF_EFF5_1<- picrust_diff_abund_hydro_ACWA_plot_REF_EFF5 + rremove("axis.title") + rremove("legend")
picrust_diff_abund_hydro_ACWA_REF_EFF5_1

picrust_diff_abund_hydro_ACWA_REF_EFF15_2<- picrust_diff_abund_hydro_ACWA_plot_REF_EFF15 + rremove("axis.title") + rremove("legend")
picrust_diff_abund_hydro_ACWA_REF_EFF15_2

# Ephemeroptera #
picrust_diff_abund_ephem_plot_1<- picrust_diff_abund_ephem_Bow_plot + rremove("axis.title")+ rremove("legend")
picrust_diff_abund_ephem_plot_1

ggarrange(picrust_diff_abund_ephem_plot_1, picrust_diff_abund_hydro_ACWA_REF_EFF5_1, picrust_diff_abund_hydro_ACWA_REF_EFF15_2, nrow=2, ncol=2)

#ggsave(file="microbiome_analysis/PICRUSt2 Data/Figures/picrust DA across taxa ACWA faceted.png", height=22, width=40)
```
...
## Testing significant changes in top 25 MetaCyc pathways across sites and inverts
...
hydro_Bow_picrust<-subset_samples(picrust_ps, Family=="Hydropsychidae")
hydro_Bow_picrust<-prune_taxa(taxa_sums(hydro_Bow_picrust) > 0, hydro_Bow_picrust) 

hydro_glom_Bow_picrust = tax_glom(hydro_Bow_picrust, "description") %>% transform_sample_counts(function(x) {x * 100/sum(x)})

hydro_Bow_df_picrust<-psmelt(hydro_glom_Bow_picrust)

#Listing the top abundant phyla
hydro_Bow_df_picrust_mean<-hydro_Bow_df_picrust %>%
    group_by(description) %>%
  dplyr::select(c("Abundance", "description")) %>%
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>%
  arrange(desc(Mean)) %>%
    ungroup 

hydro_Bow_df_picrust_top_25<-hydro_Bow_df_picrust[1:25,]

shapiro.test(hydro_Bow_df_picrust$Abundance) #p<0.001 not normal

top25pathways<-list("Amino-Acid-Biosynthesis", "Energy-Metabolism", "Super-Pathways // Nucleotide-Biosynthesis", "Cofactor-Biosynthesis", "Super-Pathways // Amino-Acid-Biosynthesis", "Nucleotide-Biosynthesis", "Lipid-Biosynthesis", "Super-Pathways // Cofactor-Biosynthesis", "Carbohydrates-Biosynthesis", "Super-Pathways // Lipid-Biosynthesis", "Cell-Structure-Biosynthesis", "Energy-Metabolism // Super-Pathways", "Super-Pathways // Cell-Structure-Biosynthesis", "NUCLEO-DEG", "SECONDARY-METABOLITE-BIOSYNTHESIS", "Metabolic-Clusters // Nucleotide Biosynthesis" )


#Amino-Acid_Biosynthesis

amino_acid_syn_hydro<-subset()
