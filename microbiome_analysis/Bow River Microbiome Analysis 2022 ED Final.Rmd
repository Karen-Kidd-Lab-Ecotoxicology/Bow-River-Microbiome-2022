...
sort((table(tax_table(effluent_Bow)[,6]))/sum(ntaxa(effluent_Bow))*100) #107 genera
#Hyphomicrobium (5.39%), Pseudorhodobacter (5.31%), Ferruginibacter (5.23%)
sum(sample_sums(effluent_Bow))/sum(sample_sums(Bow_ps))*100
#Effluent bacteria comprises 9.06% of the total microbiome in the Bow River inverts (across all invertebrate types). 
...
#Hyphomicrobium (5.39%), Pseudorhodobacter (5.31%), Ferruginibacter (5.23%)
sum(sample_sums(effluent_Bow))/sum(sample_sums(Bow_ps))*100
#Effluent bacteria comprises 9.06% of the total microbiome in the Bow River (across all invertebrate types). Decently small amount.
...
Bow_genus <- tax_glom(Bow_ps, taxrank = "Genus")
Bow_genus<-prune_taxa(taxa_sums(Bow_genus) > 0, Bow_genus) 
Bow_genus #715 genera, 276 samples
comp_Bow_genus <- transform_sample_counts(Bow_genus, function(x) x/sum(x))

Bow_genus_df <- psmelt(comp_Bow_genus)

Bow_agg_genus <- aggregate(Abundance ~ Genus + Site_code + sample_Family + Stage, data = Bow_genus_df, FUN = mean) #aggregate to Genus level, take the mean

#Sort abundance of each genus by mean at each site
sort_genus_Bow <- Bow_agg_genus %>%
    group_by(Genus, Site_code) %>%
    summarize(Mean = mean(Abundance)) %>%
    arrange()
View(sort_genus_Bow)

effluents_Bow <- sort_genus_Bow$Genus[c(11:15, 41:45, 61:70, 101:105, 131:135, 176:180, 226:230, 281:285, 291:305, 331:340, 356:360, 366:370, 376:385, 396:400, 406:410, 416:430, 436:440, 471:475, 541:545, 551:560, 591:595, 636:640, 646:650, 746:750, 781:785, 811:815, 821:865, 891:895, 911:915, 956:960, 976:980, 986:990, 1051:1055, 1061:1065, 1161:1170, 1191:1195, 1211:1215, 1221:1225, 1246:1250, 1296:1300, 1301:1305, 1381:1385, 1446:1450, 1466:1470, 1481:1485, 1521:1525, 1531:1540, 1571:1575, 1646:1650, 1671:1675, 1756:1760, 1976:1985, 2016:2020, 2036:2040, 2071:2075, 2106:2125, 2146:2150, 2216:2220, 2266:2270, 2276:2280, 2286:2290, 2301:2305, 2601:2605, 2611:2615, 2656:2660, 2701:2710, 2761:2770, 2851:2855, 2861:2865, 2956:2965, 3081:3085, 3091:3100, 3106:3110, 3141:3145, 3171:3180, 3186:3190, 3226:3230, 3251:3255, 3276:3280, 3436:3440, 3476:3480)] 
  
eff_df_Bow_genus <- Bow_agg_genus %>%
    mutate(Genus = fct_other(Genus, effluents_Bow))

eff_agg_df_Bow<-aggregate(Abundance ~ Genus + Site_code + sample_Family + Stage, data = eff_df_Bow_genus, FUN = sum)
...
eff_plot_Bow <- ggplot(eff_agg_df_Bow, aes(x=Site_code, y=Abundance, fill=Genus)) +
  ...
  theme(strip.text = element_text(size = 22, color="black"), 
        ...
        axis.text.y=element_text(size=24, color="black"),
        axis.text.x=element_text(angle=45, hjust=1, size=24, color="black", face=colorado(eff_agg_df_Bow$Site_code, c("GRVBR", "PMF"))),
        axis.title.x=element_text(size=30))+
...
#ggsave(eff_plot_Bow, filename="microbiome_analysis/Final analysis/Relative abundance/Effluent bacteria/Effluent associated bacteria combined Bow river.png", height=14, width=22)
...
sort((table(tax_table(effluent_ACWA)[,6]))/sum(ntaxa(effluent_ACWA))*100) 
#Pseudorhodobacter (8.76%), Rhodobacter (6.37%), Hyphomicrobium (5.98%)
sum(sample_sums(effluent_ACWA))/sum(sample_sums(ACWA_ps))*100
#Effluent bacteria comprises 10.4% of the total microbiome in the ACWA streams (across all invertebrate types). 
...
effluents_ACWA <- sort_genus_ACWA$Genus[c(4:6, 13:15, 22:27, 46:48, 55:57, 76:78, 88:90, 109:114, 124:126, 133:135, 139:141, 145:147, 151:153, 160:162, 175:177, 199:201, 205:210, 220:222, 232:234, 238:240, 274:276, 292:294, 307:327, 331:333, 337:339, 364:366, 373:375, 379:381, 400:402, 424:426, 430:432, 466:468, 475:480, 496:501, 538:540, 553:555, 565:567, 580:582, 586:588, 601:603, 619:621, 637:639, 670:672, 745:747, 751:756, 766:768, 775:777, 784:786, 796:804, 811:813, 853:855, 859:861, 868:870, 961:966, 985:990, 1012:1017, 1045:1050, 1087:1089, 1099:1101, 1132:1134, 1138:1143, 1153:1155, 1171:1176, 1180:1182, 1201:1203, 1207:1209, 1213:1215)]
...
eff_agg_df_ACWA$Site_code <- factor(eff_agg_df_ACWA$Site_code, levels=c("BRR2", "PCR1", "PCR3"), labels=c("REF", "EFF5", "EFF15"))

eff_agg_df_ACWA$sample_Family<-factor(eff_agg_df_ACWA$sample_Family, levels=c("Hydropsychidae", "Baetidae", "Ephemeroptera"))

pal_ACWA <- gg_color_hue(88)
pal_ACWA[1:87]<-"red3"
pal_ACWA[88] <- "#43464B"
...
eff_plot_ACWA <- ggplot(eff_agg_df_ACWA, aes(x=Site_code, y=Abundance, fill=Genus)) +
  ...
  theme(strip.text = element_text(size = 22, color="black")) + 
  theme(legend.key.size = unit(0.8, "cm"),
        legend.text=element_text(size=12, face="italic"), legend.title=element_text(size=16), legend.position = "none") +
...
#ggsave(eff_plot_ACWA, filename="microbiome_analysis/Final analysis/Relative abundance/Effluent bacteria/Effluent associated bacteria combined ACWA.png", height=14, width=16)
```
