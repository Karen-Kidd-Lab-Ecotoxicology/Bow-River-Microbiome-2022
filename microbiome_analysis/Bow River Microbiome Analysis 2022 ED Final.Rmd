...
#Importing ASV table
ASVseq<-read.csv("C:\\Users\\Emilie\\Documents\\McMaster University 2022-2024\\Bow River Thesis\\BowRiverRProject\\Microbiome_Analysis\\Raw data\\ASV_data_2022_ED_fulldataset_samplesordered.csv")
...
taxonomy <-read.csv ("C:\\Users\\Emilie\\Documents\\McMaster University 2022-2024\\Bow River Thesis\\BowRiverRProject\\Microbiome_Analysis\\Raw data\\Taxonomy_ED_2022_fulldataset.csv")
...
metadata<-read.csv("C:\\Users\\Emilie\\Documents\\McMaster University 2022-2024\\Bow River Thesis\\BowRiverRProject\\Microbiome_Analysis\\Raw data\\metadata_noKK2078_samples_ordered.csv")
...
### Re-frame data to work as one data frame
```{r}
...
top3_pmf_ephem_glom <- pmf_ephem_glom_df$Phylum[1:3]
...
dunnTest(data=proteo_ephem_Bow, rel_abund ~ Site) #CUSHBR - GRVBR p = 0.0042
...
tetra_phylum_Bow$taxon<-factor(tetra_phylum_Bow$taxon, levels=c("Other", "Firmicutes", "Proteobacteria", "Cyanobacteria", "Bacteroidota"))
...
sample.df.agg_genus_ACWA$sample_Family<-factor(sample.df.agg_genus_ACWA$sample_Family, levels=c("Hydropsychidae", "Baetidae", "Ephemeroptera"), labels=c("Hydropsychidae", "Baetidae", "Baetidae "))
...
#ggsave(filename="Microbiome_ED/Final analysis/Relative abundance/All samples/Top 22 genera Hydropsychidae, Baetidae ACWA.png", height=10, width=22)
...
hist(div.df.Bow$Shannon) #looks uniform...what type of distribution do you use for that?
...
#Adult Trichoptera
kruskal.test(Shannon~Site, data=div.df.Bow.Tricho) #p= 0.588
dunnTest(Shannon~Site, data=div.df.Bow.Tricho) #no significant diff. across sites.

#Larval Heptageniidae (order Ephemeroptera)
kruskal.test(Shannon~Site, data=div.df.Bow.Hep) #p=0.186
dunnTest(Shannon~Site, data=div.df.Bow.Hep) #no significant diff. across sites.

#Adult Ephemeroptera
kruskal.test(Shannon~Site, data=div.df.Bow.Ephem) #p=0.0837
...
#Adult Chironomidae (Order Diptera)
kruskal.test(Shannon~Site, data=div.df.Bow.dip) #p=0.902
dunnTest(Shannon~Site, data=div.df.Bow.dip) #No differences across sites

#Larval Perlidae (Order Plecoptera)
kruskal.test(Shannon~Site, data=div.df.Bow.perl) #p=0.241
dunnTest(Shannon~Site, data=div.df.Bow.perl) #No sig diff across sites

#Araneidae
kruskal.test(Shannon~Site, data=div.df.Bow.aran) #p=0.088
dunnTest(Shannon~Site, data=div.df.Bow.aran) #No differences across sites

#Tetragnathidae
kruskal.test(Shannon~Site, data=div.df.Bow.tetra) #p=0.267
...
kruskal.test(data=ephem_ACWA, Shannon~Site) #0.17
...
#Bow River
adiv_hydro_Bow<-subset(adiv_hydro, Stream_Type=="Bow River")

shapiro.test(adiv_hydro_Bow$Shannon) #0.069, normal
hist(adiv_hydro_Bow$Shannon)

summary(lm1<-aov(data=adiv_hydro_Bow, Shannon~Site))
TukeyHSD(lm1)
#Graves Bridge and Policeman Flats sig. higher than upstream sites

#ACWA
adiv_hydro_ACWA<-subset(adiv_hydro, Stream_Type!="Bow River")

shapiro.test(adiv_hydro_ACWA$Shannon) #0.33, normal
hist(adiv_hydro_ACWA$Shannon)

summary(lm1<-aov(data=adiv_hydro_ACWA, Shannon~Site)) #p=0.313
TukeyHSD(lm1) #No diff. 
...
ephem_ad<-subset_samples(sample_ps, Family=="Ephemeroptera")

adiv_ephem <- data.frame(
  "Shannon" = phyloseq::estimate_richness(ephem_ad, measures = "Shannon"),
  "Site" = phyloseq::sample_data(ephem_ad)$Site)
...
test.adonis.Bow.trichop #p = 0.017. 
...
env_Bow_no_grvbr_2<-env_Bow_no_grvbr[ ,c("Water_Temp", "Dissolved_oxygen", "Conductivity", "Total_Phosphorus", "Total_Dissolved_Solids", "Total_Suspended_Solids", "Total_Organic_Carbon", "pH", "Nitrate")] #Selects only the numerical variables (water temp, conductivity, dissolved oxygen, etc)

#All environmental data isn't normally distributed. Use Spearman correlations. 
shapiro.test(env_Bow_no_grvbr_2$Water_Temp) #Non normal
shapiro.test(env_Bow_no_grvbr_2$Dissolved_oxygen) #non normal
shapiro.test(env_Bow_no_grvbr_2$Conductivity) #non normal
shapiro.test(env_Bow_no_grvbr_2$Total_Phosphorus) #non normal
shapiro.test(env_Bow_no_grvbr_2$Total_Dissolved_Solids) #Non normal
shapiro.test(env_Bow_no_grvbr_2$Total_Suspended_Solids) #Non normal
shapiro.test(env_Bow_no_grvbr_2$Total_Organic_Carbon) #Non normal
shapiro.test(env_Bow_no_grvbr_2$pH) #non normal
shapiro.test(env_Bow_no_grvbr_2$Nitrate) #non normal


correlation_matrix_env<-as.data.frame(cor(env_Bow_no_grvbr_2), method="spearman") #Some variables are highly correlated. Test with all of them to see if any are significant then get rid of the correlated variable that is less significant based on p value. 
...
ord_Bow_mds_no_grvbr #stress=0.196

gg_envfit(ord = ord_Bow_mds_no_grvbr, env = env_Bow_no_grvbr_2, perm = 9999, pt.size = 2, alpha = 0.2)
envfit(ord_Bow_mds_no_grvbr, env = env_Bow_no_grvbr_2, permutations = 9999)
cor(env_Bow_no_grvbr_2$Conductivity, env_Bow_no_grvbr_2$Total_Dissolved_Solids) #0.998 Highly correlated
cor(env_Bow_no_grvbr_2$Conductivity, env_Bow_no_grvbr_2$Total_Suspended_Solids) #0.733 Correlated
cor(env_Bow_no_grvbr_2$Conductivity, env_Bow_no_grvbr_2$Total_Organic_Carbon) #0.0944
cor(env_Bow_no_grvbr_2$Conductivity, env_Bow_no_grvbr_2$pH) #-0.692 #Correlated
cor(env_Bow_no_grvbr_2$pH, env_Bow_no_grvbr_2$Total_Organic_Carbon) #0.276

env_Bow_no_grvbr_3<-env_Bow_no_grvbr_2[ , c("Conductivity", "pH", "Total_Organic_Carbon")]

envfit_2<-envfit(ord_Bow_mds_no_grvbr, env = env_Bow_no_grvbr_3, permutations = 9999)

vif(envfit_2)

#Plotting ordination with sites as colours
gg_envfit(ord = ord_Bow_mds_no_grvbr, env = env_Bow_no_grvbr_3, perm = 9999, groups = env_Bow_no_grvbr$Site, pt.size = 2, alpha = 0.05, len=0.05, unit="cm")

#ggsave(file="Microbiome_ED/Final analysis/Beta Diversity Graphs/Envfit water quality Bow River sites.png", height=8, width=12)

#Plotting ordination with invertebrate taxa as colours
gg_envfit(ord = ord_Bow_mds_no_grvbr, env = env_Bow_no_grvbr_3, perm = 9999, groups = env_Bow_no_grvbr$Family, pt.size = 2, alpha = 0.2)
...
fit_all <- envfit(ord_Bow_mds_no_grvbr ~ Water_Temp + Dissolved_oxygen + Conductivity + Total_Phosphorus + Total_Dissolved_Solids + Total_Suspended_Solids + Total_Organic_Carbon + pH + Nitrate, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
fit_all

#Remove non-significant variables


bow1 <- envfit(ord_Bow_mds_no_grvbr ~ Dissolved_oxygen + Conductivity + Total_Phosphorus + Total_Dissolved_Solids + Total_Suspended_Solids + Total_Organic_Carbon + pH + Nitrate, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
bow1

bow2 <- envfit(ord_Bow_mds_no_grvbr ~ Conductivity + Total_Phosphorus + Total_Dissolved_Solids + Total_Suspended_Solids + Total_Organic_Carbon + pH + Nitrate, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
bow2

bow3 <- envfit(ord_Bow_mds_no_grvbr ~ Conductivity + Total_Dissolved_Solids + Total_Suspended_Solids + Total_Organic_Carbon + pH + Nitrate, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
bow3

bow4 <- envfit(ord_Bow_mds_no_grvbr ~ Conductivity + Total_Dissolved_Solids + Total_Suspended_Solids + Total_Organic_Carbon + pH, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
bow4

bow5 <- envfit(ord_Bow_mds_no_grvbr ~ Conductivity + Total_Dissolved_Solids + Total_Organic_Carbon + pH, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
bow5

bow6 <- envfit(ord_Bow_mds_no_grvbr ~ Total_Dissolved_Solids + Total_Organic_Carbon + pH, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
bow6

bow7 <- envfit(ord_Bow_mds_no_grvbr ~ Conductivity + Total_Organic_Carbon + pH, data = env_Bow_no_grvbr_2, perm = 999, na.rm=T)
bow7

cor(env_Bow_no_grvbr_2$pH, env_Bow_no_grvbr_2$Conductivity) #-0.692
cor(env_Bow_no_grvbr_2$pH, env_Bow_no_grvbr_2$Total_Dissolved_Solids) #-0.731

#pH is more corelated with total dissolved solids thus I will remove that variable and keep conductivity.

cor(env_Bow_no_grvbr_2$Total_Organic_Carbon, env_Bow_no_grvbr_2$Conductivity) #0.0944
cor(env_Bow_no_grvbr_2$Total_Organic_Carbon, env_Bow_no_grvbr_2$Total_Dissolved_Solids) #0.0777

#Pretty similar. Doesn't matter which variable we use because it is not correlated with total organic carbon

#Can't use AIC for envfit models. Need to use p-value method and ensure no multicollinearity


#Total organic carbon, pH, and conductivity are significantly correlated to microbiome composition
...
plot_ACWA_taxa<-plot_ordination(ACWA_rel_abun, ord_NMDS_ACWA, shape = "Family", color="Site") +
  theme_bw() +
  rremove("grid")+
...
sample_ps2 #1801 taxa, 335 samples. 
```
...
## Comparing un-exposed sites to exposed sites - Bow River
```{r}
...
hydro_ps<-subset_samples(sample_ps, Family=="Hydropsychidae")
...
#140 differentially abundant taxa

ggplot(sigtab_exposed_hydro, aes(x=Genus, y=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1, size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
#ggsave(filename="Microbiome_ED/Final analysis/Differential Abundance Results/Bow River exposed vs unexposed sites hydropsychidae larvae only.png", height=15, width=22)
...
#28 differentially abundant taxa

ggplot(sigtab_exposed_trichop, aes(x=Genus, y=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1, size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
#ggsave(filename="Microbiome_ED/Final analysis/Differential Abundance Results/Bow River exposed vs unexposed sites caddisfly adults only.png", height=15, width=22)
...
ggplot(sigtab_exposed_hep, aes(x=Genus, y=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1, size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
#ggsave(filename="Microbiome_ED/Final analysis/Differential Abundance Results/Bow River exposed vs unexposed sites larval heptageniidae only.png", height=15, width=22)


#### Adult mayfly ####

ephem_ps<-subset_samples(sample_ps, Family=="Ephemeroptera")
...
#9 differentially abundant taxa

ggplot(sigtab_exposed_ephem, aes(x=Genus, y=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1, size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
#ggsave(filename="Microbiome_ED/Final analysis/Differential Abundance Results/Bow River exposed vs unexposed sites adult mayfly only.png", height=15, width=22)
...
ggplot(sigtab_exposed_chiro, aes(x=Genus, y=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1, size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
#ggsave(filename="Microbiome_ED/Final analysis/Differential Abundance Results/Bow River exposed vs unexposed sites chironomid larvae only.png", height=15, width=22)
...
ggplot(sigtab_exposed_dip, aes(x=Genus, y=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1, size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
#ggsave(filename="Microbiome_ED/Final analysis/Differential Abundance Results/Bow River exposed vs unexposed sites chironomidae adults only.png", height=15, width=22)
...
ggplot(sigtab_exposed_perl, aes(x=Genus, y=log2FoldChange, color=Phylum))+
  geom_point(size=6) + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1, size=24, face="italic")) + theme(axis.text=element_text(size=24, color="black"), axis.title=element_text(size=28)) +
          theme(legend.text=element_text(size=24, color="black")) + 
...
#ggsave(filename="Microbiome_ED/Final analysis/Differential Abundance Results/Bow River exposed vs unexposed sites larval perlidae only.png", height=15, width=22)
...
#No differentially abundant taxa when separate from Araneids
```
...
tax_select(sample_ps, tax_list="Candidatus") #75 taxa.
...
tax_select(sample_ps, tax_list="Candidatus Hamiltonella") #none matched
...
endo1<-subset_taxa(sample_ps, Genus %in% c("Buchnera", "Candidatus Cardinium", "Hamiltonella", "Rickettsia", "Rickettsiella", "Wolbachia", "Spiroplasma", "Arsenophonus", "Diplorickettsia", "Candidatus Hamiltonella"))
endo1 #31. Same as above. 
...
endo_coch #23 taxa, 65 samples
#43 samples have endosymbionts/65 = 66.2 %
