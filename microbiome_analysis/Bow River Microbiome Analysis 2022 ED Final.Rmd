...
This data comes from a study done in 2022 across 5 sites on the Bow River, Calgary AB and 3 experimental streams from Advancing Candian Water Assets (ACWA) at the Pine Creek wastewater treatment plant with varying municipal wastewater effluent exposures (stream additions: 0, 5, or 15% effluent). The objective of this study was to determine whether there are changes to the microbiome of freshwater macroinvertebrates and riparian spiders with exposure to wastewater effluents as well as any differences across taxa and across metamorphosis/life stage. 
...
#ps_df<-psmelt(ps)
#write.csv(ps_df, "microbiome_analysis/ps_df.csv")
```
...
nsamples(spider_Bow) #78

#Total number of reads
print(sum(sample_sums(Bow_ps))) #10,857,965 
#Average # of reads per sample
mean(sample_sums(Bow_ps)) #38,232 average reads/sample
sort(sample_sums(Bow_ps)) #reads/sample ranges from 2283 - 222,956 counts
ntaxa(Bow_ps) #4194

#All phyla combined
sort((table(tax_table(Bow_ps)[,2]))/sum(ntaxa(Bow_ps))*100)
#Top 5 abundant phyla are Proteobacteria (37.6 %), Bacteroidota (30.7 %), Firmicutes (11.2 %), Cyanobacteria (5.91 %), Actinobacteria (5.65 %)
sort((table(tax_table(Bow_ps)[,5])/sum(ntaxa(Bow_ps))*100))
#Top 5 families are Comamonadaceae (7.82 %), Saprospiraceae (6.27 %), Flavobacteriaceae (4.58 %), Rhodobacteraceae (4.29 %), and Sphingomonadaceae (3.79 %)
...
get_taxa_unique(Bow_ps, "Class") #69 Classes total
get_taxa_unique(Bow_ps, "Order") #167 orders total
get_taxa_unique(Bow_ps, "Family") #238 families total
get_taxa_unique(Bow_ps, "Genus") #481 genera total
...
#Total number of reads
print(sum(sample_sums(ACWA_ps))) #2,220,340
#Average # of reads per sample
mean(sample_sums(ACWA_ps)) #40,369 average reads/sample
sort(sample_sums(ACWA_ps)) #reads/sample ranges from 9154 - 91,209 counts
ntaxa(ACWA_ps) #2605
...
get_taxa_unique(ACWA_ps, "Class") #61 Classes total
get_taxa_unique(ACWA_ps, "Order") #145 orders total
get_taxa_unique(ACWA_ps, "Family") #197 families total
get_taxa_unique(ACWA_ps, "Genus") #339 genera total
```
...
get_taxa_unique(coch, "Class") #51 Classes total
get_taxa_unique(coch, "Order") #127 orders total
get_taxa_unique(coch, "Family") #187 families total
...
get_taxa_unique(hep_coch, "Class") #41 Classes total
get_taxa_unique(hep_coch, "Order") #86 orders total
get_taxa_unique(hep_coch, "Family") #126 families total
get_taxa_unique(hep_coch, "Genus") #199 genera total

#Chironomidae
chiro_coch<-subset_samples(coch, Family=="Chironomidae")
chiro_coch<-prune_taxa(taxa_sums(chiro_coch)>0, chiro_coch)
ntaxa(chiro_coch) #380
nsamples(chiro_coch) #5

get_taxa_unique(chiro_coch, "Phylum") #11 phyla total
get_taxa_unique(chiro_coch, "Class") #26 Classes total
get_taxa_unique(chiro_coch, "Order") #70 orders total
get_taxa_unique(chiro_coch, "Family") #94 families total
get_taxa_unique(chiro_coch, "Genus") #124 genera total
...
get_taxa_unique(dip_coch, "Family") #55 families total
get_taxa_unique(dip_coch, "Genus") #89 genera total
...
get_taxa_unique(chiro_pmf, "Order") #79 orders total
get_taxa_unique(chiro_pmf, "Family") #113 families total
get_taxa_unique(chiro_pmf, "Genus") #156 genera total
...
top3_phy_coch_chiro <- chiro_coch_phy$Phylum[1:3]
  
coch_phy_sum_chiro<- chiro_coch_phy %>%
  mutate(Phylum = fct_other(Phylum, top3_phy_coch_chiro))
...
top3_phy_pmf_chiro <- chiro_pmf_phy$Phylum[1:3]
  
pmf_phy_sum_chiro<- chiro_pmf_phy %>%
  mutate(Phylum = fct_other(Phylum, top3_phy_pmf_chiro))
...
endo_rel_abun<-transform_sample_counts(endo, function(x) {x * 100/sum(x)})
...
endo_lar_hydro_Bow_rel_abun<-transform_sample_counts(endo_lar_hydro_Bow, function(x) {x * 100/sum(x)})

endo_lar_hydro_Bow_df<-psmelt(endo_lar_hydro_Bow_rel_abun)
...
trichop_Bow<-subset_samples(sample_ps, Family=="Trichoptera" & Stream_Type=="Bow River")

trichop_endo_rel_abun = transform_sample_counts(trichop_Bow, function(x) {x * 100/sum(x)})

endo_trichop_Bow<-subset_samples(trichop_endo_rel_abun, Family=="Trichoptera" & Stream_Type=="Bow River")
nsamples(endo_trichop_Bow) #31

trichop_endo_glom<-prune_taxa(taxa_sums(trichop_endo_glom) > 0, trichop_endo_glom) 

endo_trichop_df<-psmelt(trichop_endo_glom) #%>%
  as_tibble %>%
    group_by(Genus) %>%
  dplyr::select(c("Abundance", "Genus")) %>%
  summarize(Mean = mean(Abundance), SD = sd(Abundance)) %>% ungroup

sort((table(tax_table(endo_trichop_Bow)[,6]))/sum(ntaxa(endo_trichop_Bow))*100)
#Buchnera (16.7%) Spiroplasma (16.7) Rickettsia (66.7%)
sum(sample_sums(endo_trichop_Bow))/sum(sample_sums(trichop_Bow))*100
#36.7% 
...
nsamples(endo_lar_trichop_Bow_coch) #8
...
ephem_Bow<-subset_samples(sample_ps, Family=="Ephemeroptera" & Stream_Type=="Bow River")
ephem_Bow_df<-psmelt(ephem_Bow)

endo_ephem_Bow<-subset_taxa(ephem_Bow, Genus=="Rickettsia" | Genus=="Buchnera" | Genus=="Candidatus cardinium" | Genus=="Spiroplasma" | Genus=="Wolbachia")

endo_ephem_Bow<-prune_taxa(taxa_sums(endo_ephem_Bow) > 0, endo_ephem_Bow) 
sort((table(tax_table(endo_ephem_Bow)[,6]))/sum(ntaxa(endo_ephem_Bow))*100)
#Buchnera (14.3%) Spiroplasma (14.3) Wolbachia (14.3) Rickettsia (57.1%)
sum(sample_sums(endo_ephem_Bow))/sum(sample_sums(ephem_Bow))*100
#5.39% 


#Cochrane 
ephem_coch<-subset_samples(ephem_Bow, Site=="Cochrane")
nsamples(ephem_coch) #8

endo_ephem_Bow_coch<-subset_samples(endo_ephem_Bow, Site=="Cochrane")
endo_ephem_Bow_coch<-prune_taxa(taxa_sums(endo_ephem_Bow_coch) > 0, endo_ephem_Bow_coch) 
sort((table(tax_table(endo_ephem_Bow_coch)[,6]))/sum(ntaxa(endo_ephem_Bow_coch))*100)
#Rickettsia: 100%
sum(sample_sums(endo_ephem_Bow_coch))/sum(sample_sums(ephem_coch))*100
#26.4 % of total
...
#Cushing Bridge
ephem_cush<-subset_samples(ephem_Bow, Site=="Cushing Bridge")
ephem_cush_df<-psmelt(ephem_cush)
sum(ephem_cush_df$Abundance)

endo_ephem_Bow_cush<-subset_taxa(ephem_cush, Genus=="Rickettsia" | Genus=="Buchnera" | Genus=="Candidatus cardinium" | Genus=="Spiroplasma" | Genus=="Wolbachia")
endo_ephem_Bow_cush<-prune_taxa(taxa_sums(endo_ephem_Bow_cush) > 0, endo_ephem_Bow_cush) 
sort((table(tax_table(endo_ephem_Bow_cush)[,6]))/sum(ntaxa(endo_ephem_Bow_cush))*100)
#Buchnera: 33.3%, Spiroplasma: 33.3%, Rickettsia: 33.3%
sum(sample_sums(endo_ephem_Bow_cush))/sum(sample_sums(ephem_cush))*100
#0.085 % of total
