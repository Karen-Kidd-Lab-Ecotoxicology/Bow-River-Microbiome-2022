...
table(tax_table(ps1)[,"Phylum"],exclude=NULL) #Removed NAs
...
ntaxa(ps1) - ntaxa(ps2) #Got rid of 914 ASVs
table(tax_table(ps2)[,"Order"],exclude=NULL) #Removed Chloroplasts. No Mitochondria present.

ps2 #28,862 taxa, 349 samples
sort(sample_sums(ps2)) #Reads/sample range from 46 - 222,777
```
...
prevalenceThreshold #We are keeping phyla that occur at least 6.98 (7) samples (2% of samples) with a prevalence of 2 or greater 
...
prevdf2 = data.frame(Prevalence = prev1,
                      TotalAbundance = taxa_sums(ps2),
                      tax_table(ps2))
keepPhyla1 = table(prevdf2$Phylum)[(table(prevdf2$Phylum) > 0.05)]
...
psprune1 #8698 taxa. Still a very large percentage of taxa removed but we need to look at the amount of total reads removed to understand the implications to the data.

reads_pruned1 <- taxa_sums(psprune1)
print(sum(reads_pruned1)) #10,601,592 total reads left after pruning
reads_per_OTU <- taxa_sums(psexp) 
(1-(sum(reads_pruned1))/(sum(reads_per_OTU)))*100 #19.2% of the total data was removed. Still a decently large percentage 
...
#I will use a prevalence threshold of 0.5 % meaning that the bacteria has to be present in at least 2 samples 
```
