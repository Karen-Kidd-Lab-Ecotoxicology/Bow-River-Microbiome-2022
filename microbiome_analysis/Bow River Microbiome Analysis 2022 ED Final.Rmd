...
kruskal.test(data=firm_hydro_Bow, rel_abund ~ Site) #p = 0.022
dunnTest(data=firm_hydro_Bow, rel_abund ~ Site) #COCH - PMF p=0.017, z = 3.13
...
kruskal.test(data=cyano_hydro_Bow, rel_abund ~ Site) #p = 0.000142
...
top5genusplotBow <- ggplot(top5genusdf_Bow, aes(x=factor(Site_code, labels = c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF")), y=Abundance, fill=Genus)) +
  ...
  scale_fill_manual(name="Genus", values=c(as.vector(stepped(21)),"#C8BC78", "#D5CE9D", "#F7F2CE", "#43464B"))+
  scale_y_continuous(label = scales::percent)+
  xlab("Site")+
  rremove("grid")
top5genusplotBow 
...
glom_lar_Bow <- tax_glom(lar_Bow, taxrank = "Genus")
rel_abun_lar_Bow <- transform_sample_counts(glom_lar_Bow, function(x) x/sum(x))

rel_abun_lar_Bow_df <- psmelt(rel_abun_lar_Bow)

agg_lar_Bow<- aggregate(Abundance ~ Genus + Site_code + sample_Family + Stage, data = rel_abun_lar_Bow_df, FUN = mean) 


#Top 10 genus, all inverts included
top_genus_lar_Bow <- agg_lar_Bow %>%
    group_by(Site_code, Genus) %>%
    summarize(Mean = mean(Abundance)) %>%
    arrange(-Mean)
top10genus_lar_Bow <- top_genus_lar_Bow$Genus[1:20]
top10genuslardfBow <- agg_lar_Bow %>%
    mutate(Genus = fct_other(Genus, top10genus_lar_Bow))

top10genuslardf_Bow <- aggregate(Abundance ~ Genus + Site_code + sample_Family + Stage, data = top10genuslardfBow, FUN = sum)

#Top 10 across all larvae:         
...
top10genuslardf_Bow$sample_Family<-factor(top10genuslardf_Bow$sample_Family, levels=c("Hydropsychidae", "Heptageniidae", "Chironomidae", "Perlidae"))


top10genuslarplotBow <- ggplot(top10genuslardf_Bow, aes(x=factor(Site_code, labels = c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF")), y=Abundance, fill=Genus)) +
  geom_bar(stat="identity", color="black") + 
  ylab("Mean Relative Abundance") +
  facet_nested_wrap(~Stage+sample_Family, scale="free_x", nrow=1) + 
  theme_bw()+
  ...
  xlab("Site")+
  rremove("grid")
top10genuslarplotBow 

#ggsave(filename="microbiome_analysis/Final analysis/Relative abundance/All samples/Top 10 genera larvae.png", height=10, width=14)
...
endo_plot_Bow <- ggplot(endo_agg_df_Bow, aes(x=Site_code, y=Abundance, fill=Genus)) +
  ...
  theme(axis.title.y=element_text(size=30),
        ...
        axis.text.x=element_text(angle=45, hjust=1, size=24, color="black", face = ifelse(levels(endo_agg_df_Bow$Site_code)=="GRVBR","PMF"), "bold", "plain"),
                        axis.title.x=element_text(size=30))+
  scale_fill_manual(name="Genus", values=c("#808080", "tan1","seagreen1", "skyblue1", "lightgoldenrod1","lightpink1"))+
  scale_y_continuous(label = scales::percent)+
  xlab("Site")+
  rremove("grid")
endo_plot_Bow
...
## Hydropsychidae ##
hydro_endo_Bow<-subset(Genus_endo_Bow_graphing_df, sample_Family=="Hydropsychidae")

hydro_endo_Bow$Site<-factor(hydro_endo_Bow$Site, levels=c("Cochrane", "Sunalta", "Cushing Bridge", "Graves Bridge", "Policeman Flats"))

ggplot(hydro_endo_Bow, aes(y=taxon, x=rel_abund, color=Site)) +
  stat_summary(fun.data=median_hilow, geom = "pointrange",
               fun.args=list(conf.int=0.5),
               position = position_dodge(width=1)) +
  coord_trans(x="log10") +
...
nsamples(hydro_REF<-subset_samples(sample_ps, Site=="BRR2" & Family=="Hydropsychidae")) #8

#Hydropsychidae 
effluent_BRR2<-subset_samples(effluent_bac, Site=="BRR2")
effluent_BRR2<-prune_taxa(taxa_sums(effluent_BRR2) > 0, effluent_BRR2) 
effluent_BRR2 #212 taxa, 8 samples/8=100 prevalence%

sort((table(tax_table(effluent_BRR2)[,6]))/sum(ntaxa(effluent_BRR2))*100)
#Rhodoferax (10.8), Pseudorhodobacter (9.43), Rhodobacter (6.13)

## EXP5 ACWA Stream ##

#Hydropsychidae
nsamples(hydro_EFF5<-subset_samples(effluent_bac, Site=="PCR1" & Family=="Hydropsychidae")) #8
hydro_EFF5<-prune_taxa(taxa_sums(hydro_EFF5) > 0, hydro_EFF5) 
hydro_EFF5 #266 taxa, 8 samples=100%

sort((table(tax_table(hydro_EFF5)[,6]))/sum(ntaxa(hydro_EFF5))*100)
#Rhodoferax (9.40), Rhodobacter (6.77), Pseudorhodobacter (6.39)

#Baetidae
nsamples(baetid_EFF5<-subset_samples(effluent_bac, Site=="PCR1" & Family=="Baetidae")) #8
baetid_EFF5<-prune_taxa(taxa_sums(baetid_EFF5) > 0, baetid_EFF5) 
baetid_EFF5 #262 taxa, 8 samples=100% prevalence

sort((table(tax_table(baetid_EFF5)[,6]))/sum(ntaxa(baetid_EFF5))*100)
#Pseudorhodobacter (13.4), Rhodobacter (8.02), Rhodoferax (6.49)

#Ephemeroptera
nsamples(ephem_EFF5<-subset_samples(effluent_bac, Site=="PCR1" & Family=="Ephemeroptera")) #8
ephem_EFF5<-prune_taxa(taxa_sums(ephem_EFF5) > 0, ephem_EFF5) 
ephem_EFF5 #46 taxa, 8 samples=100% prevalence

sort((table(tax_table(ephem_EFF5)[,6]))/sum(ntaxa(ephem_EFF5))*100)
#Pseudorhodobacter (10.9), Staphylococcus (8.70), Rhodoferax (8.70)


## EXP15 ACWA Stream ##

#Hydropsychidae
nsamples(hydro_EFF15<-subset_samples(effluent_bac, Site=="PCR3" & Family=="Hydropsychidae")) #8
hydro_EFF15<-prune_taxa(taxa_sums(hydro_EFF15) > 0, hydro_EFF15) 
hydro_EFF15 #211 taxa, 8 samples=100%

sort((table(tax_table(hydro_EFF15)[,6]))/sum(ntaxa(hydro_EFF15))*100)
#Pseudorhodobacter (8.53), Rhodoferax (7.11), Rhodobacter (5.21)

#Baetidae
nsamples(baetid_EFF15<-subset_samples(effluent_bac, Site=="PCR3" & Family=="Baetidae")) #7
baetid_EFF15<-prune_taxa(taxa_sums(baetid_EFF15) > 0, baetid_EFF15) 
baetid_EFF15 #245 taxa, 7 samples=100% prevalence

sort((table(tax_table(baetid_EFF15)[,6]))/sum(ntaxa(baetid_EFF15))*100)
#Pseudorhodobacter (15.5), Rhodobacter (10.2), Leptothrix (6.94)

#Ephemeroptera
nsamples(ephem_EFF15<-subset_samples(effluent_bac, Site=="PCR3" & Family=="Ephemeroptera")) #8
ephem_EFF15<-prune_taxa(taxa_sums(ephem_EFF15) > 0, ephem_EFF15) 
ephem_EFF15 #65 taxa, 8 samples=100% prevalence

sort((table(tax_table(ephem_EFF15)[,6]))/sum(ntaxa(ephem_EFF15))*100)
#Staphylococcus (7.69), Faecalibacterium (7.69), Corynebacterium (6.15)
