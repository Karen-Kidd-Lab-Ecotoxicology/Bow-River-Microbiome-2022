...
taxon_pool_Phylum_Bow <- taxon_rel_abund_Phylum_Bow %>%
  ...
  dplyr::summarize(pool = max(median) < 4,
...
hydro_otu_rel_abund_Bow<-subset(otu_rel_abund_Bow, sample_Family=="Hydropsychidae")

taxon_rel_abund_Phylum_hydro_Bow <- hydro_otu_rel_abund_Bow %>%
  dplyr::filter(level=="Phylum") %>%
  dplyr::group_by(Site, Study_ID, taxon) %>%
  dplyr::summarize(rel_abund = 100*sum(rel_abund), .groups="drop") %>%
  drop_na(Site, taxon)
View(taxon_rel_abund_Phylum_hydro_Bow)

taxon_pool_Phylum_hydro_Bow <- taxon_rel_abund_Phylum_hydro_Bow %>%
  dplyr::group_by(Site, Study_ID, taxon) %>%
  dplyr::summarize(median=median(rel_abund), .groups="drop") %>%
  dplyr::group_by(taxon) %>%
  dplyr::summarize(pool = max(median) < 4,
            median = median(median),
            .groups="drop")
View(taxon_pool_Phylum_hydro_Bow)

#Graphing pointrange plot with median and 50% quartiles
hydro_phylum_Bow<- dplyr::inner_join(taxon_rel_abund_Phylum_hydro_Bow, taxon_pool_Phylum_hydro_Bow, by="taxon") %>%
 dplyr::mutate(taxon = if_else(pool, "Other", taxon)) %>%
  ...
  dplyr::mutate(rel_abund = if_else(rel_abund == 0,
                             2/3 * lod,
                             rel_abund))
View(hydro_phylum_Bow)

hydro_phylum_Bow$taxon<-factor(hydro_phylum_Bow$taxon, levels=c("Other", "Actinobacteriota", "Deferribacterota", "Firmicutes", "Proteobacteria", "Bacteroidota"))
...
shapiro.test(hydro_phylum_Bow$rel_abund) #not normal.
...
#Actinobacteriota
actino_hydro_Bow<-subset(hydro_phylum_Bow, taxon=="Actinobacteriota")
kruskal.test(data=actino_hydro_Bow, rel_abund ~ Site) #p = 0.000394
dunnTest(data=actino_hydro_Bow, rel_abund ~ Site) 
#COCH - PMF z = -3.33, P = 0.0077
#GRVBR - SUN z =2.82, p=0.038
#PMF - SUN z = 3.67, P = 0.0024

#Deferribacterota
def_hydro_Bow<-subset(hydro_phylum_Bow, taxon=="Deferribacterota")
kruskal.test(data=def_hydro_Bow, rel_abund ~ Site) #p = 0.2066
...
hep_phylum_Bow$taxon<-factor(hep_phylum_Bow$taxon, levels=c("Other", "Deferribacterota", "Actinobacteriota", "Bacteroidota", "Firmicutes", "Proteobacteria"))
...
kruskal.test(data=proteo_hep_Bow, rel_abund ~ Site) #p = 0.069. No diff 
...
#Actinobacteriota
actino_hep_Bow<-subset(hep_phylum_Bow, taxon=="Actinobacteriota")
kruskal.test(data=actino_hep_Bow, rel_abund ~ Site) #p = 0.3214

#Deferribacterota
def_hep_Bow<-subset(hep_phylum_Bow, taxon=="Deferribacterota")
kruskal.test(data=def_hep_Bow, rel_abund ~ Site) #p = 0.406

#No differences in Heps


## Chironomidae ##
chiro_phylum_Bow<-subset(Phylum_Bow_graphing_df, sample_Family=="Chironomidae")

chiro_phylum_Bow$taxon<-factor(chiro_phylum_Bow$taxon, levels=c("Other", "Deferribacterota", "Actinobacteriota", "Firmicutes", "Bacteroidota", "Proteobacteria"))
...
kruskal.test(data=proteo_chiro_Bow, rel_abund ~ Site) #p = 0.094 possible difference
...
#Actinobacteriota
actino_chiro_Bow<-subset(chiro_phylum_Bow, taxon=="Actinobacteriota")
kruskal.test(data=actino_chiro_Bow, rel_abund ~ Site) #p = 0.558

#Deferribacterota
def_chiro_Bow<-subset(chiro_phylum_Bow, taxon=="Deferribacterota")
kruskal.test(data=def_chiro_Bow, rel_abund ~ Site) #p = 0.449
...
perl_phylum_Bow$taxon<-factor(perl_phylum_Bow$taxon, levels=c("Other", "Deferribacterota", "Actinobacteriota", "Firmicutes", "Bacteroidota", "Proteobacteria"))
...
#Actinobacteriota
actino_perl_Bow<-subset(perl_phylum_Bow, taxon=="Actinobacteriota")
kruskal.test(data=actino_perl_Bow, rel_abund ~ Site) #p = 0.018
dunnTest(data=actino_perl_Bow, rel_abund ~ Site)
#COCH - PMF z = -2.82, P = 0.0144

#Deferribacterota
def_perl_Bow<-subset(perl_phylum_Bow, taxon=="Deferribacterota")
kruskal.test(data=def_perl_Bow, rel_abund ~ Site)
...
kruskal.test(data=bacter_trichop_Bow, rel_abund ~ Site) #p = 0.5497
...
top30genusplotBow <- ggplot(top30genusdf_Bow, aes(x=factor(Site_code, labels = c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF")), y=Abundance, fill=Genus)) +
  ...
  scale_fill_manual(name="Genus", values=c("#fcff5d", "#7dfc00", "#0ec434", "#228c68", "#8ad8e8", "#235b54", "#29bdab", "#3998f5", "#37294f", "#277da7", "#3750db", "#f22020", "#991919", "#ffcba5", "#e68f66", "#c56133", "#96341c", "#632819", "#ffc413", "#f47a22", "#2f2aa0", "#b732cc", "#772b9d", "#f07cab", "#d30b94", "#edeff3", "#c3a5b4", "#946aa2", "#5d4c86","#FEA1B3","#43464B"))+
  scale_y_continuous(label = scales::percent)+
  xlab("Site")+
  rremove("grid")
