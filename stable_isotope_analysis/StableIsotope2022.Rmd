...
#Removed 29 total: COCH_Ad_Hydro_SI_1, COCH_Ad_Hydro_SI_2, COCH_Lar_Hydro_SI_1, COCH_Lar_Hydro_SI_2, CUSHBR_Ad_Mayfly_1_SI_1, CUSHBR_Ad_Rhya_SI_1, CUSHBR_Lar_Chiro_SI_1, CUSHBR_Lar_Mayfly_1_SI_1, CUSHBR_Lar_Mayfly_1_SI_2, GRVBR_Ad_Chiro_SI_1, GRVBR_Ad_Mayfly_1_SI_1, GRVBR_Coleop_SI_1, GRVBR_Coleop_SI_2, GRVBR_Coleop_SI_3, GRVBR_Lar_Hydro_SI_1, GRVBR_Lar_Hydro_SI_2, GRVBR_Lar_Hydro_SI_3, POLFLT_Ad_Mayfly_1_SI_1, POLFLT_Ad_Rhya_SI_1, POLFLT_Biofilm_SI_1, POLFLT_Biofilm_SI_2, POLFLT_Biofilm_SI_3, POLFLT_Coleop_SI_1, POLFLT_Lar_Hydro_SI_1, POLFLT_Lar_Hydro_SI_2, POLFLT_Lar_Hydro_SI_3, SUNAL_Ad_Mayfly_1_SI_1, SUNAL_Lar_Hydro_SI_1, SUNAL_Lar_Hydro_SI_2 
```
...
#Some taxa have very low sample sizes. Combine taxa by larvae, adults, spiders, biofilm, leaves, and by functional feeding group 
...
#Need to remove the groups with less than 3 replicates before statistical analysis

summary_FFG_subset<-isotope_subset %>% 
  dplyr::select("FFG", "Stage", "d13C", "d15N", "Site")%>%
  dplyr::group_by(FFG, Stage, Site) %>%
  dplyr::summarise(MeanC = mean(d13C), sdC = sd(d13C), MeanN = mean(d15N), sdN = sd(d15N), n = n())

View(summary_FFG_subset)
```
...
ggplot(isotope_subset, aes(x=X.C, y=d13C))+
  geom_point()+
...
biofilm<-filter(isotope_subset, Taxa=="Biofilm")
...
inverts<-subset(isotope_subset, Consumer!="Basal food source")
...
ggplot(biofilm_raw_no_HCl, aes(x=Site, y=d15N, label=SampleName))+ 
 ...
 geom_text(hjust=-0.1)+
  theme_bw()
                                      ...
                                      ## Non acid treated data outliers ##

nontreated<-subset(biofilm_raw, Stage=="Non acid treated")

#Cochrane
nontreated_coch<-subset(nontreated, Site=="Cochrane")

plot(nontreated_coch$d15N, main="Visual Inspection for Outliers", xlab="Site", ylab="d15N", pch=19, col="blue")
abline(h=mean(nontreated_coch$d15N), col="red")  # Add a horizontal line at the mean
text(nontreated_coch$d15N, nontreated_coch$SampleName, cex=0.6, pos=4, col="red")

#Sunalta
nontreated_sun<-subset(nontreated, Site=="Sunalta")

plot(nontreated_sun$d15N, main="Visual Inspection for Outliers", xlab="Site", ylab="d15N", pch=19, col="blue")
abline(h=mean(nontreated_sun$d15N), col="red")  # Add a horizontal line at the mean
text(nontreated_sun$d15N, nontreated_sun$SampleName, cex=0.6, pos=4, col="red")

#Cushing Bridge
nontreated_cush<-subset(nontreated, Site=="Cushing Bridge")

plot(nontreated_cush$d15N, main="Visual Inspection for Outliers", xlab="Site", ylab="d15N", pch=19, col="blue")
abline(h=mean(nontreated_cush$d15N), col="red")  # Add a horizontal line at the mean
text(nontreated_cush$d15N, nontreated_cush$SampleName, cex=0.6, pos=4, col="red")

#Graves Bridge
nontreated_grv<-subset(nontreated, Site=="Graves Bridge")

plot(nontreated_grv$d15N, main="Visual Inspection for Outliers", xlab="Site", ylab="d15N", pch=19, col="blue")
abline(h=mean(nontreated_grv$d15N), col="red")  # Add a horizontal line at the mean
text(nontreated_grv$d15N, nontreated_grv$SampleName, cex=0.6, pos=4, col="red")

#Policeman Flats
nontreated_pmf<-subset(nontreated, Site=="Policeman Flats")

plot(nontreated_pmf$d15N, main="Visual Inspection for Outliers", xlab="Site", ylab="d15N", pch=19, col="blue")
abline(h=mean(nontreated_pmf$d15N), col="red")  # Add a horizontal line at the mean
text(nontreated_pmf$d15N, nontreated_pmf$SampleName, cex=0.6, pos=4, col="red")
...
#Biofilm outlier removed

all_biofilm_subset<-subset(all_biofilm, SampleName!="POLFLT_Biofilm_SI_1")

ggplot(all_biofilm_subset, aes(x=d13C, y=d15N))+
  geom_point(aes(color=Stage)) +
  theme_bw()+
  ylab(expression(paste(delta^15, "N (\u2030)", sep="")))+
  xlab(expression(paste(delta^13, "C (\u2030)", sep="")))+
  theme(legend.title = element_blank(), axis.title=element_text(size=16, color="black"), axis.text=element_text(size=14, color="black"), strip.text.x = element_text(size=14, color="black"), legend.text = element_text(size=14, color="black"))+
  facet_grid(~Site)+
  rremove("grid")
...
shapiro.test(all_biofilm_subset$d13C) #not normal p=0.0042
leveneTest(data=all_biofilm_subset, d13C ~ Site*Stage) #Homogeneity p=0.327
hist(all_biofilm$d13C) #not normal looking

#Use Kruskal Wallis tests for each site individually

#Cochrane
coch_biofilm<-subset(all_biofilm_subset, Site=="Cochrane")
shapiro.test(coch_biofilm$d13C) #p=0.26, normal
t.test(d13C~Stage, data=coch_biofilm, paired=T) #0.199, not sig diff
kruskal.test(d13C ~ Stage, data=coch_biofilm) #p=0.49, not sig different

#Sunalta
sun_biofilm<-subset(all_biofilm_subset, Site=="Sunalta")
...
#Cushing Bridge
cushbr_biofilm<-subset(all_biofilm_subset, Site=="Cushing Bridge")
...
#Graves Bridge
grvbr_biofilm<-subset(all_biofilm_subset, Site=="Graves Bridge")
...
#Policeman Flats
pmf_biofilm<-subset(all_biofilm_subset, Site=="Policeman Flats")
...
shapiro.test(all_biofilm_subset$d15N) #normal p=0.197
leveneTest(data=all_biofilm_subset, d15N ~ Site*Stage) #Homogeneity p=0.429
hist(all_biofilm_subset$d15N) #not very normal looking but passes the normality test

summary(N_aov<-aov(data=all_biofilm_subset, d15N ~ Site*Stage))
...
ggplot(isotope_subset, aes(x=d13C, y=d15N, color=Taxa, shape=Stage))+
  geom_point()+
  facet_grid(.~Site)

# Sites plotted individually
Cochrane<-filter(isotope_subset, Site=="Cochrane")
...
data.sum<-ddply(isotope_subset, c("Site_Code", "Taxa", "FFG"), summarise,
...
```{r}
#### N plot across sites and FFG/life stage ####
...
isotope_subset$FFG<-factor(isotope_subset$FFG, levels=c("Terrestrial Leaves", "Biofilm", "Scraper", "Filterer-Collector", "Gatherer-Collector", "Piercer", "Predator"))

isotope_subset$Site_Code<-factor(isotope_subset$Site_Code, levels=c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF"))

isotope_subset$Stage_plot<-factor(isotope_subset$Stage_plot, levels=c("Basal food source", "Aquatic larvae", "Aquatic adult", "Terrestrial adult", "Spider"))
...
N_plot_taxa_subset<-ggplot(isotope_subset, aes(x=Site_Code, y=d15N))+
  ...
  rremove("grid")+
  xlab("Site")+
  facet_nested_wrap(~Stage_plot + FFG, scales="free_x", nrow=1)
N_plot_taxa_subset
...
biofilm_subset<-subset(isotope_subset, Taxa=="Biofilm")

shapiro.test(biofilm_subset$d15N) #0.929 #normal distribution
leveneTest(data=biofilm_subset, d15N~Site) #0.610, homogeneity of variance
hist(biofilm_subset$d15N) 
plot(biofilm_subset$d15N) 

summary(N_biofilm<-aov(data=biofilm_subset, d15N~Site, na.action = na.exclude)) #p=0.131
Anova(N_biofilm, type=3) #p=0.1308 no sig differences across sites

plot(N_biofilm) #Doesn't fit too well with assumptions of ANOVA but there is a small number of data points which could affect it. Test using Kruskal-Wallis test too. 

kruskal.test(data=biofilm_subset, d15N~Site) #p=0.113 no differences across sites


# Terrestrial Leaves 

leaves_subset<-subset(isotope_subset, Taxa=="Terrestrial Leaves") 

shapiro.test(leaves_subset$d15N) #0.740 #normal distribution
leveneTest(data=leaves_subset, d15N~Site) #0.752, homogeneity of variance
hist(leaves_subset$d15N) 
plot(leaves_subset$d15N) 

summary(N_leaves<-aov(data=leaves_subset, d15N~Site, na.action = na.exclude)) #p=0.00217
Anova(N_leaves, type=3) #p=0.00217 
TukeyHSD(N_leaves)
#Differences between:
#COCH - GRVBR
#COCH - PMF

plot(N_leaves) #adheres to assumptions
...
isotope_lar_scrapers<-subset(isotope_subset, Stage=="Aquatic larvae" & FFG=="Scraper")
...
#Graves Bridge higher than all sites
#Policeman Flats lower at all sites
...
isotope_lar_gath<-subset(isotope_subset, Stage=="Aquatic larvae" & FFG=="Gatherer-Collector")
...
isotope_lar_pred<-subset(isotope_subset, Stage=="Aquatic larvae" & FFG=="Predator")

shapiro.test(isotope_lar_pred$d15N) #0.50 normal distribution
leveneTest(data=isotope_lar_pred, d15N~Site) #0.139, homogeneity of variance
hist(isotope_lar_pred$d15N) #uniform ish
plot(isotope_lar_pred$d15N) #evenly spread

summary(N_lar_pred<-aov(data=isotope_lar_pred, d15N~Site, na.action = na.exclude)) #p = 0.0031
Anova(N_lar_pred, type=3) #p=0.0031
...
plot(N_ad_gath) #Scale location plot not great but rest is good
...
plot(N_ad_filt) #Scale location plot not great but rest is good
...
isotope_terr_ad<-subset(isotope_subset, Stage=="Terrestrial adult")
...
summary(N_terr_ad<-aov(data=isotope_terr_ad, d15N~Site, na.action = na.exclude)) #p=0.011
...
isotope_spiders<-subset(isotope_subset, Stage=="Spider")
...
```{r}
#### C plot across sites and FFG/life stage ####

isotope_subset$Stage_plot<-factor(isotope_subset$Stage_plot, levels=c("Basal food source", "Aquatic larvae", "Aquatic adult","Terrestrial adult","Spider"))

isotope_subset$Site_Code<-factor(isotope_subset$Site_Code, levels=c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF"))

isotope_subset$FFG<-factor(isotope_subset$FFG, levels=c("Terrestrial Leaves", "Biofilm", "Scraper", "Gatherer-Collector", "Filterer-Collector", "Predator", "Piercer"))
...
shapiro.test(isotope_subset_no_food_sources$d13C) #0.230
leveneTest(data=isotope_subset_no_food_sources, d13C~Site*FFG*Stage) #0.203
...
biofilm_subset<-subset(isotope_subset, Taxa=="Biofilm" & Site !="Policeman Flats") #Subset biofilm from all sites except Policeman Flats since there was an outlier removed and there are not enough data points for statistics (2).

shapiro.test(biofilm_subset$d13C) #0.272 #normal distribution
leveneTest(data=biofilm_subset, d13C~Site) #0.411, homogeneity of variance

summary(C_biofilm<-aov(data=biofilm_subset, d13C~Site, na.action = na.exclude)) #p=0.172
Anova(C_biofilm, type=3) #p=0.172 no sig differences across sites

#### Riparian Leaves ####

leaves_subset<-subset(isotope_subset, Taxa=="Terrestrial Leaves") 

shapiro.test(leaves_subset$d13C) #0.108 #normal distribution
leveneTest(data=leaves_subset, d13C~Site) #0.886, homogeneity of variance

summary(C_leaves<-aov(data=leaves_subset, d13C~Site, na.action = na.exclude)) #p=0.323
Anova(C_leaves, type=3) #p=0.3226 no sig differences across sites

#### Aquatic Larvae ####

isotope_aq_lar<-subset(isotope_subset, Stage=="Aquatic larvae")
shapiro.test(isotope_aq_lar$d13C) #0.11 #normal distribution
leveneTest(data=isotope_aq_lar, d13C~Site*FFG) #0.429, homogeneity of variance
...
summary(C_lar_scraper<-aov(data=isotope_lar_scrapers, d13C~Site, na.action = na.exclude)) #p=0.00515
Anova(C_lar_scraper, type=3) #p=0.005

emmeans(C_lar_scraper, pairwise ~ Site)
#Higher C values in downstream sites compared to Cochrane
...
#PMF had higher C than GRVBR
#PMF had higher C than Sunalta
...
shapiro.test(isotope_lar_pred$d13C) #p<0.001 not normal distribution
leveneTest(data=isotope_lar_pred, d13C~Site) #0.177, homogeneity of variance
hist(isotope_lar_pred$d13C) #not very normal
plot(isotope_lar_pred$d13C) #not evenly spread

kruskal.test(data=isotope_lar_pred, d13C~Site) #p=0.007
dunnTest(data=isotope_lar_pred, d13C~Site) #CUSHBR - PMF

#### Aquatic Adults ####

isotope_aq_ad<-subset(isotope_subset, Stage=="Aquatic adult")
...
emmeans(C_ad_filt, pairwise ~ Site) 

plot(C_ad_filt) #QQ plot and Scale-Location plot are questionable

kruskal.test(data=isotope_ad_filt, d13C~Site) #p=0.015
dunnTest(data=isotope_ad_filt, d13C~Site)
#Sig comparisons don't match up with the plotted data
...
summary(C_terr_ad<-aov(data=isotope_terr_ad, d13C~Site, na.action = na.exclude)) #p=0.104
...
summary(C_spiders<-aov(data=isotope_spiders, d13C~Site, na.action = na.exclude))
Anova(C_spiders, type=3)

plot(C_spiders) #Fits assumptions of ANOVA/lm pretty well

emmeans(C_spiders, pairwise ~ Site)


#### Comparing spider C values to terrestrial and aquatic adults to see what they are eating ####

# Comparing spiders to terrestrial insects across sites

spi_terr<- subset(isotope_subset, Stage %in% c("Spider", "Terrestrial adult"))

shapiro.test(spi_terr$d13C) #0.269
leveneTest(data=spi_terr, d13C ~ Site*Stage) #0.518
leveneTest(data=spi_terr, d13C ~ Site*Taxa) #0.906
hist(spi_terr$d13C)
plot(spi_terr$d13C)

summary(spi_terr_aov<-aov(data=spi_terr, d13C ~ Site*Stage))
Anova(spi_terr_aov)

emmeans(spi_terr_aov, pairwise ~ Stage | Site)
#C values of spiders were significantly different from those of terrestrial adult insects at each site

summary(spi_terr_aov_taxa<-aov(data=spi_terr, d13C ~ Site*Taxa))
Anova(spi_terr_aov_taxa)

emmeans(spi_terr_aov_taxa, pairwise ~ Taxa | Site)

plot(spi_terr_aov_taxa) #looks good


# Comparing spiders to aquatic adult insects across sites

spi_aq_ad<- subset(isotope_subset, Stage %in% c("Spider", "Aquatic adult"))

shapiro.test(spi_aq_ad$d13C) #0.1804
leveneTest(data=spi_aq_ad, d13C ~ Site*FFG*Stage) #0.1554
leveneTest(data=spi_aq_ad, d13C ~ Site*Taxa) #0.011
hist(spi_aq_ad$d13C)
plot(spi_aq_ad$d13C)

summary(spi_aq_ad_aov<-aov(data=spi_aq_ad, d13C ~ Site*Taxa))
Anova(spi_aq_ad_aov)

emmeans(spi_aq_ad_aov, pairwise ~ Taxa | Site)
#overlapping with all taxa

plot(spi_aq_ad_aov) #not very normal but the rest are okay
...
```{r}
C_plot<-C_plot_taxa_subset + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())

N_plot<-N_plot_taxa_subset + theme(axis.title.x = element_blank(), strip.text.x = element_blank())

C_N_combined<-ggarrange(C_plot, N_plot, nrow=2, ncol=1, align="v")

annotate_figure(C_N_combined, bottom = textGrob("Site", gp = gpar(cex = 1.8)))

#ggsave(filename="stable_isotope_analysis/Figures/C and N across FFG and sites.png", height=14, width=28, bg="white")
```
...
isotope_no_subset_no_food_sources<-subset(isotope, Stage!="Biofilm" & Stage!="Terrestrial leaves")

isotope_no_food_sources_subset_taxa<-subset(isotope_no_subset_no_food_sources, Taxa %in% c("Hydropsychidae", "Chironomidae", "Araneidae", "Tetragnathidae"))

isotope_no_food_sources_subset_taxa_C<-isotope_no_food_sources_subset_taxa%>%
  dplyr::select("Site", "FFG","Taxa_Specific", "d13C")%>%
  group_by(Site, FFG, Taxa_Specific)%>%
  summarise(MeanC = mean(d13C), sdC = sd(d13C))

Ylims_C_subset_taxa<-aes(ymax=MeanC + sdC, ymin=MeanC-sdC)

isotope_no_food_sources_subset_taxa_C$Taxa_Specific<-factor(isotope_no_food_sources_subset_taxa_C$Taxa_Specific, levels=c("Chironomidae Larvae", "Hydropsychidae Larvae", "Chironomidae Adult", "Hydropsychidae Adult", "Araneidae", "Tetragnathidae"))

C_reliance_graph_subset_taxa<-ggplot(isotope_no_food_sources_subset_taxa_C, aes(x=FFG, y=MeanC, shape=FFG, color=Taxa_Specific))+
  geom_point(size=6)+
  facet_grid(~Site)+
  theme_bw() +
  rremove("grid")+
  geom_errorbar(Ylims_C_subset_taxa)+
   labs(shape="Functional Feeding Group", colour="Invertebrate Taxa")+
  geom_hline(data=isotope_biofilm_C, aes(yintercept=MedianC), linetype='dashed', color='red', linewidth=1)+ #Biofilm
  geom_hline(data=isotope_leaves_C, aes(yintercept=MedianC), linetype='dashed', color='blue', linewidth=1)+ #leaves
  ylab(expression(paste(delta^13, "C (\u2030)", sep="")))+
...
isotope_no_food_sources_subset_taxa_N<-isotope_no_food_sources_subset_taxa%>%
  dplyr::select("Site", "FFG","Taxa_Specific", "d15N")%>%
  group_by(Site, FFG, Taxa_Specific)%>%
  summarise(MeanN = mean(d15N), sdN = sd(d15N))

Ylims_N_subset_taxa<-aes(ymax=MeanN + sdN, ymin=MeanN-sdN)

isotope_no_food_sources_subset_taxa_C$Taxa_Specific<-factor(isotope_no_food_sources_subset_taxa_C$Taxa_Specific, levels=c("Chironomidae Larvae", "Hydropsychidae Larvae", "Chironomidae Adult", "Hydropsychidae Adult", "Araneidae", "Tetragnathidae"))

N_reliance_graph_subset_taxa<-ggplot(isotope_no_food_sources_subset_taxa_N, aes(x=FFG, y=MeanN, shape=FFG, color=Taxa_Specific))+
  geom_point(size=6)+
  facet_grid(~Site)+
  theme_bw() +
  rremove("grid")+
  geom_errorbar(Ylims_N_subset_taxa)+
   labs(shape="Functional Feeding Group", colour="Invertebrate Taxa")+
  geom_hline(data=isotope_biofilm_N, aes(yintercept=MedianN), linetype='dashed', color='red', linewidth=1)+ #Biofilm
  geom_hline(data=isotope_leaves_N, aes(yintercept=MedianN), linetype='dashed', color='blue', linewidth=1)+ #leaves
  ylab(expression(paste(delta^15, "N (\u2030)", sep="")))+
  theme(axis.text.y = element_text(size=20, color="black"), axis.text.x=element_blank(), axis.ticks.x = element_blank(), axis.title.x = element_blank(), axis.title.y=element_text(size=22, color="black"), strip.text.x = element_text(size=20, color="black"), legend.text = element_text(size=20), legend.title = element_text(size=22, color="black"))+
  scale_color_manual(values=c( '#800000',  '#000075',  '#f58231','#dcbeff','#42d4f4', '#3cb44b' ))+
  scale_shape_manual(values=c(16, 17, 15, 18, 7))
N_reliance_graph_subset_taxa


isotope_no_food_sources<-subset(isotope_subset, Stage!="Biofilm" & Stage!="Terrestrial leaves" & Stage!="Terrestrial adult")

isotope_no_food_sources_subset_taxa_N<-isotope_no_food_sources%>%
  dplyr::select("Site", "FFG","Stage", "d15N")%>%
  group_by(Site, FFG, Stage)%>%
  summarise(MeanN = mean(d15N), sdN = sd(d15N))

Ylims_N_subset_taxa<-aes(ymax=MeanN + sdN, ymin=MeanN-sdN)

isotope_no_food_sources_subset_taxa_N$Stage<-factor(isotope_no_food_sources_subset_taxa_N$Stage, levels=c("Aquatic larvae", "Aquatic adult", "Spider"))

N_reliance_graph_stage<-ggplot(isotope_no_food_sources_subset_taxa_N, aes(x=FFG, y=MeanN, shape=FFG, color=Stage))+
  geom_point(size=6)+
  facet_grid(~Site)+
  theme_bw() +
  rremove("grid")+
  geom_errorbar(Ylims_N_subset_taxa)+
   labs(shape="Functional Feeding Group", colour="Sample Type")+
  geom_hline(data=isotope_biofilm_N, aes(yintercept=MedianN), linetype='dashed', color='red', linewidth=1)+ #Biofilm
  geom_hline(data=isotope_leaves_N, aes(yintercept=MedianN), linetype='dashed', color='blue', linewidth=1)+ #leaves
  ylab(expression(paste(delta^15, "N (\u2030)", sep="")))+
  theme(axis.text.y = element_text(size=20, color="black"), axis.text.x=element_blank(), axis.ticks.x = element_blank(), axis.title.x = element_blank(), axis.title.y=element_text(size=22, color="black"), strip.text.x = element_text(size=20, color="black"), legend.text = element_text(size=20), legend.title = element_text(size=22, color="black"))+
  scale_color_manual(values=c('#000075', '#3cb44b','#f58231'))+
  scale_shape_manual(values=c(16, 17, 15, 18, 7))
N_reliance_graph_stage

#ggsave(filename="C:\\Users\\Emilie\\Documents\\GitHub\\Bow-River-Microbiome-2022\\stable_isotope_analysis\\Figures\\LSETAC_N_graph.png", width=16, height=10)


#With basal food sources before removing data

isotope_subset_taxa<-subset(isotope, Taxa %in% c("Hydropsychidae", "Chironomidae", "Araneidae", "Tetragnathidae", "Biofilm", "Terrestrial Leaves"))

isotope_subset_taxa_N<-isotope_subset_taxa%>%
  dplyr::select("Site", "FFG","Taxa_Specific", "d15N")%>%
  group_by(Site, FFG, Taxa_Specific)%>%
  summarise(MeanN = mean(d15N), sdN = sd(d15N))
...
isotope_subset_taxa_N$Taxa_Specific<-factor(isotope_subset_taxa_N$Taxa_Specific, levels=c("Biofilm", "Terrestrial Leaves", "Chironomidae Larvae", "Hydropsychidae Larvae", "Chironomidae Adult", "Hydropsychidae Adult", "Araneidae", "Tetragnathidae"))

N_reliance_graph_subset_taxa<-ggplot(isotope_subset_taxa_N, aes(x=FFG, y=MeanN, shape=FFG, color=Taxa_Specific))+
  geom_point(size=6)+
  facet_grid(~Site)+
  theme_bw() +
  rremove("grid")+
...
# Mean +/- SD C vs N of inverts across sites with basal food source mean values graph
...
isotope_biofilm_mean_C<-isotope_biofilm %>%
  dplyr::select("Site", "d13C")%>%
  group_by(Site)%>%
  summarise(MeanC = mean(d13C))

isotope_leaves<-subset(isotope_subset, Taxa=="Terrestrial Leaves")

isotope_leaves_mean_C<-isotope_leaves %>%
  dplyr::select("Site", "d13C")%>%
  group_by(Site)%>%
  summarise(MeanC = mean(d13C))
...
C_reliance_graph_mean<-ggplot(isotope_subset_no_food_sources_C, aes(x=FFG, y=MeanC, shape=FFG, color=Taxa_Specific))+
  geom_point(size=6)+
  facet_grid(~Site)+
  theme_bw() +
  rremove("grid")+
  geom_errorbar(Ylims_C)+
   labs(shape="Functional Feeding Group", colour="Invertebrate Taxa")+
  geom_hline(data=isotope_biofilm_mean_C, aes(yintercept=MeanC), linetype='dashed', color='red', linewidth=1)+ #Biofilm
  geom_hline(data=isotope_leaves_mean_C, aes(yintercept=MeanC), linetype='dashed', color='blue', linewidth=1)+ #leaves
  ylab(expression(paste(delta^13, "C (\u2030)", sep="")))+
  theme(axis.text.y = element_text(size=20, color="black"), axis.text.x=element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size=22, color="black"), strip.text.x = element_text(size=20, color="black"), legend.text = element_text(size=20), legend.title = element_text(size=22, color="black"))+
  scale_color_manual(values=c( '#800000', '#e6194B', '#f58231','#ffe119', '#aaffc3', '#3cb44b', '#469990', '#42d4f4', '#4363d8', '#000075', '#dcbeff', '#f032e6', '#fabed4', '#9A6324'))+
  scale_shape_manual(values=c(16, 17, 15, 18, 7))
C_reliance_graph_mean
...
isotope_biofilm_mean_N<-isotope_biofilm %>%
  dplyr::select("Site", "d15N")%>%
  group_by(Site)%>%
  summarise(MeanN = mean(d15N))

isotope_leaves<-subset(isotope_subset, Taxa=="Terrestrial Leaves")

isotope_leaves_mean_N<-isotope_leaves %>%
  dplyr::select("Site", "d15N")%>%
  group_by(Site)%>%
  summarise(MeanN = mean(d15N))
...
N_reliance_graph_mean<-ggplot(isotope_subset_no_food_sources_N, aes(x=FFG, y=MeanN, shape=FFG, color=Taxa_Specific))+
  geom_point(size=6)+
  facet_grid(~Site)+
  theme_bw() +
  rremove("grid")+
  geom_errorbar(Ylims_N)+
   labs(shape="Functional Feeding Group", colour="Invertebrate Taxa")+
  geom_hline(data=isotope_biofilm_mean_N, aes(yintercept=MeanN), linetype='dashed', color='red', linewidth=1)+ #Biofilm
  geom_hline(data=isotope_leaves_mean_N, aes(yintercept=MeanN), linetype='dashed', color='blue', linewidth=1)+ #leaves
  ylab(expression(paste(delta^15, "N (\u2030)", sep="")))+
  theme(axis.text.y = element_text(size=20, color="black"), axis.text.x=element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size=22, color="black"), strip.text.x = element_text(size=20, color="black"), legend.text = element_text(size=20), legend.title = element_text(size=22, color="black"))+
  scale_color_manual(values=c( '#800000', '#e6194B', '#f58231','#ffe119', '#aaffc3', '#3cb44b', '#469990', '#42d4f4', '#4363d8', '#000075', '#dcbeff', '#f032e6', '#fabed4', '#9A6324'))+
scale_shape_manual(values=c(16, 17, 15, 18, 7))
 
N_reliance_graph_mean


#### Combining C and N graphs ####

C_reliance_mean<-C_reliance_graph_mean + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) + theme(legend.position="none")
N_reliance_mean<-N_reliance_graph_mean + theme(axis.title.x = element_blank(), legend.position="right")

C_N_combined_reliance_mean<-ggarrange(C_reliance_mean, N_reliance_mean, nrow=2, ncol=1, align="v", common.legend = T, legend = "right")

C_N_combined_reliance_mean

#ggsave(filename="stable_isotope_analysis/Figures/C and N reliance graph mean.png", height=12, width=20, bg="white")
```
...
food_sources<-subset(isotope_subset, Stage=="Biofilm" | Stage=="Terrestrial leaves")

summary_food_sources<-food_sources%>% 
  dplyr::select("Stage", "d13C", "d15N", "Site")%>%
  dplyr::group_by(Stage, Site) %>%
  dplyr::summarise(MeanC = mean(d13C), sdC = sd(d13C), MeanN = mean(d15N), sdN = sd(d15N), n = n())

View(summary_food_sources)
