...
isotope<-read.csv("C:\\Users\\Emilie\\Documents\\McMaster University 2022-2024\\Bow River Thesis\\BowRiverRProject\\Stable_isotope_ED\\StableIsotope2022.csv")
...
ggplot(isotope, aes(x=X.C, y=d13C))+ #Looks like there's a negative relationship between %C and d13C indicating no carbonates (inorganic carbon) in the samples since Brian mentioned the correlation should be positive if there was an effect
  geom_point()+
...
ggplot(biofilm, aes(x=X.C, y=d13C))+ 
  ...
  geom_smooth(method="lm")+
  facet_grid(.~Site)
#No effect of carbonates. Seems like there is one datapoint driving the others. 


### Lipid effect ###

ggplot(isotope, aes(x=C.N, y=d13C))+ #Only looks slightly depleted at Policeman Flats so probably not an issue for this data
  geom_point()+
  geom_smooth(method="lm")+
  facet_grid(.~Site)
...
data.median<-ddply(isotope, c("Site", "Taxa", "FFG"), summarise,
                ...
                #d13Cse=IQR(d13C)/sqrt(length(d13C)), #standard error
                d15Nmed=median(d15N), #mean
                d15NIQR=IQR(d15N)) #standard deviation
                #d15Nse=IQR(d15N)/sqrt(length(d15N))) #standard error
...
ggplot(data.median1, aes(x=d13Cmed, y=d15Nmed, colour=Taxa, shape=FFG))+
  geom_point(size=3)+
  geom_errorbar(Ylims, width=0.2)+
  geom_errorbarh(Xlims, height=0.2)+
  ylab(expression(paste(delta^15, "N (\u2030)", sep="")))+
...
ggplot(data.median, aes(x=d13Cmed, y=d15Nmed, colour=Taxa, shape=FFG))+
  ...
  facet_grid(~Site, scales="free_x")+
  theme_bw()+
  labs(fill="Sample Type", shape="Functional Feeding Group")+
  rremove("grid")


#ggsave(filename="Stable_isotope_ED/Figures/Median and IQR biplot d13C d15N faceted by site.png", height=6, width=12)

```

Checking for nitrogen enrichment across sites

```{r}

isotope_sub<-subset(isotope, Aqu_Terr!= "Terrestrial")

isotope_sub$Aqu_Terr<-factor(isotope_sub$Aqu_Terr, levels=c("Riparian Leaves", "Biofilm", "Aquatic Larvae", "Aquatic Adults", "Spiders"))

ggplot(isotope_sub, aes(x=factor(Site, labels=c("COCH", "SUN", "CUSHBR", "GRVBR", "PMF")), y=d15N))+
  geom_boxplot()+
  theme_bw()+
  ylab(expression(paste(delta^15, "N (\u2030)", sep="")))+
...
PERMANOVA of C and N across sites and taxa
...
#All samples, including basal food sources

dist_N<-dist(isotope$d15N, method="euclidean")

#Testing difference in delta 15 N across sites and tissue type (whole invertebrates/biofilm/shrubs 

SI.adonis.N <- adonis(dist_N ~ Taxa*Site, data = isotope)
SI.adonis.N <- as.data.frame(SI.adonis.N$aov.tab)
SI.adonis.N
#significant differences across taxa types, sites, and the interaction between the two

pairwise.adonis2(dist_N ~ Site, data = isotope)

#With all taxa types combined there were sig differences btw:
#COCH - GRVBR
#SUNAL - GRVBR
#GRVBR - CUSHBR
#GRVBR - PMF

disp_N_sites<-betadisper(dist_N, isotope$Site)
anova(disp_N_sites) #No significant beta dispersion. 

pairwise.adonis2(dist_N ~ Taxa, data = isotope)

#With all sites combined there were sig differences btw:
#Hydropscyhdiae - Coleoptera
#Hydropsychidae - Heptageniidae
#Hydropsychidae - Lepidoptera
#Hydropsychidae - Terrestrial leaves
#Hydropsychidae - Biofilm
#Chironomidae - Chloroperlidae
#Chironomidae - Coleoptera
#Chironomidae - Ephemerellidae
#Chironomidae - Heptageniidae
#Chironomidae - Lepidoptera
#Chironomidae - Terrestrial leaves
#Chironomidae - Biofilm
#Chloroperlidae - Tetragnathidae
#Chloroperlidae - Lepidoptera
#Chloroperlidae - Terrestrial Leaves
#Chloroperlidae - Biofilm
#Araneidae - Coleoptera
#Araneidae - Heptageniidae
#Araneidae - Lepidoptera
#Araneidae - Terrestrial leaves
#Araneidae - Biofilm
#Coleoptera - Tetragnathidae
#Coleoptera - Perlidae
#Coleoptera - Leptoceridae
#Ephemerellidae - Tetragnathidae
#Ephemerellidae - Lepidoptera
#Ephemerellidae - Terrestrial leaves
#Ephemerellidae - Biofilm
#Heptageniidae - Tetragnathidae
#Heptageniidae - Terrestrial leaves
#Heptageniidae - Biofilm
#Rhyacophilidae - Lepidoptera
#Rhyacophilidae - Terrestrial leaves
#Rhyacophilidae - Biofilm
#Tetragnathidae - Lepidoptera
#Tetragnathidae - Terrestrial leaves
#Tetragnathidae - Biofilm
#Lepidoptera - Perlidae
#Lepidoptera - Leptoceridae
#Perlidae - Terrestrial leaves
#Perlidae - Biofilm
#Leptoceridae - Terrestrial leaves
#Leptoceridae - biofilm

disp_N_taxa<-betadisper(dist_N, isotope$Taxa)
anova(disp_N_taxa) #p = 0.048, possible beta dispersion

ggplot(isotope, aes(x=Taxa, y=d15N))+
  geom_boxplot()+
  theme_bw()

#Samples excluding basal food sources
dist_N_invert<-dist(inverts$d15N, method="euclidean")

SI.adonis.N.invert <- adonis(dist_N_invert ~ Aqu_Terr, data = inverts)
SI.adonis.N.invert <- as.data.frame(SI.adonis.N.invert$aov.tab)
SI.adonis.N.invert #p=0.001

pairwise.adonis2(dist_N_invert ~ Site, data = inverts)
#CUSHBR - PMF
#CUSHBR - GRVBR
#PMF - GRVBR
#GRVBR - COCH
...
disp_N_invert_sites<-betadisper(dist_N_invert, inverts$Site)
anova(disp_N_invert_sites) #No significant beta dispersion. p=0.076

pairwise.adonis2(dist_N_invert ~ Aqu_Terr, data = inverts)

#Spiders vs aquatic larvae are significantly different in d15N. F (1,82) = 17.29, p=0.002, R^2=0.174

#Spiders vs aquatic adults are NOT sig. different in d15N. F(1,83)=0.2298, p=0.631, R^2=0.00276

#Spiders vs terrestrial insects are sig. diff in d15N. F(1,44)=40.18, p<0.001, R^2=0.477

#Aquatic larvae vs aquatic adults are sig. diff in d15N. F(1,107)=17.27, p<0.001, R^2=0.139

#Aquatic larvae vs terrestrial insects are sig. diff in d15N. F(1,68)=12.02, p<0.001, R^2=0.150

#Aquatic adults vs terrestrial insects are sig. diff in d15N. F(1,69)=36.99, p<0.001, R^2=0.349

disp_N_invert_taxa<-betadisper(dist_N_invert, inverts$Taxa)
anova(disp_N_invert_taxa) #No significant beta dispersion. p=0.486


# Biofilm across sites

hist(biofilm$d15N)

dist_N_biofilm<-dist(biofilm$d15N, method="euclidean")
SI.adonis.N.biofilm <- adonis(dist_N_biofilm ~ Site, data = biofilm)
SI.adonis.N.biofilm <- as.data.frame(SI.adonis.N.biofilm$aov.tab)
SI.adonis.N.biofilm #p=0.363

pairwise.adonis2(dist_N_biofilm ~ Site, data = biofilm)

disp_N_biofilm<-betadisper(dist_N_biofilm, biofilm$Site)
anova(disp_N_biofilm) #No beta dispersion. p=0.563


#Terrestrial Leaves across sites

hist(shrub$d15N)

dist_N_leaves<-dist(shrub$d15N, method="euclidean")
SI.adonis.N.leaves <- adonis(dist_N_leaves ~ Site, data = shrub)
SI.adonis.N.leaves <- as.data.frame(SI.adonis.N.leaves$aov.tab)
SI.adonis.N.leaves #p=0.003

pairwise.adonis2(dist_N_leaves ~ Site, data = shrub)

disp_N_leaves<-betadisper(dist_N_leaves, shrub$Site)
anova(disp_N_leaves) #No beta dispersion. p=0.752

#Pairwise adonis test says there are no differences in d15N in biofilm or terrestrial leaves across sites, however, kruskal-wallis test or ANOVA indicates there are. 
...
dist_C<-dist(isotope$d13C, method="euclidean")

SI.adonis.C <- adonis(dist_C ~ Taxa*Site, data = isotope)
SI.adonis.C <- as.data.frame(SI.adonis.C$aov.tab)
SI.adonis.C
#significant differences across taxa types, sites, and the interaction between the two

pairwise.adonis2(dist_C ~ Site, data = isotope)

#With all taxa types combined there are sig differences btw:
#COCH - PMF
#SUN - PMF

pairwise.adonis2(dist_C ~ Taxa, data = isotope)
#Several differences across taxa

pairwise.adonis2(dist_C ~ Stage, data = isotope)
...
disp_C_stage<-betadisper(dist_C, isotope$Stage)
anova(disp_C_stage) #Significant beta dispersion p < 0.001

ggplot(isotope, aes(x=Taxa, y=d13C))+
  geom_boxplot()+
  theme_bw()+
  scale_y_reverse()

pairwise.adonis2(dist_C ~ Aqu_Terr, data = isotope)
#Sig differences between:
#Spiders vs shrub
#Spider vs aquatic larvae
#Spider vs biofilm
#Shrub vs biofilm
#Shrub vs terrestrial prey
#Larvae aquatic vs biofilm
#Larvae aquatic vs adult aquatic
#Larvae aquatic vs terrestrial prey
#Biofilm vs adult aquatic
#Biofilm vs terrestrial prey

disp_C_aquatic_terr<-betadisper(dist_C, isotope$Aqu_Terr)
anova(disp_C_aquatic_terr) #Significant beta dispersion p < 0.001
...
dist_C_invert<-dist(inverts$d13C, method="euclidean")

SI.adonis.C.invert <- adonis(dist_C_invert ~ Aqu_Terr, data = inverts)
SI.adonis.C.invert <- as.data.frame(SI.adonis.C.invert$aov.tab)
SI.adonis.C.invert #p=0.001

pairwise.adonis2(dist_C_invert ~ Site, data = inverts)
#Only site comparisons that are not different are:
#CUSHBR - GRVBR
#COCH - SUN


pairwise.adonis2(dist_C_invert ~ Aqu_Terr, data = inverts)

#Spiders vs aquatic larvae are significantly different in d13C. F (1,82) = 22.14, p=0.001, R^2=0.213

#Spiders vs aquatic adults are NOT sig. different in d13C. F(1,83)=3.50, p=0.051, R^2=0.0404

#Spiders vs terrestrial insects are NOT sig. diff in d13C. F(1,44)= 1.17, p=0.306, R^2=0.026

#Aquatic larvae vs aquatic adults are sig. diff in d13C. F(1,107)=7.33, p=0.008, R^2=0.064

#Aquatic larvae vs terrestrial insects are sig. diff in d13C. F(1,68)=7.43, p=0.007, R^2=0.099

#Aquatic adults vs terrestrial insects are NOT sig. diff in d13C. F(1,69)=0.3511, p=0.587, R^2=0.00506

disp_C_aquatic_terr_invert<-betadisper(dist_C_invert, inverts$Aqu_Terr)
anova(disp_C_aquatic_terr_invert) #no significant beta dispersion.

```

Calculating average d13C and d15N values across sites and sample type (Table 10)
