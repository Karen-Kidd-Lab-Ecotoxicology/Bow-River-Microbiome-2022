...
data.sum1<-ddply(isotope, c("Site", "Taxa", "FFG"), summarise,
                d13Cmn=mean(d13C), #mean
                d13Csd=sd(d13C), #standard deviation
                #d13Cse=sd(d13C)/sqrt(length(d13C)), #standard error
                d15Nmn=mean(d15N), #mean
...
C slope correlations (consumers to biofilm)
```{r}
pri_consumer<-subset(isotope, Consumer=="Primary Consumer")
consumers_no_shrub<-subset(pri_basal_consumer, Tissue.Type!="Shrub")
biofilm<-subset(isotope, Taxa=="Biofilm")

Primary_consumer<-subset(isotope, Consumer=="Primary Consumer")
Primary_consumer<-Primary_consumer[,c(5,6,21)]%>%
  dplyr::rename(d13C_pri = d13C)

Biofilm<-subset(isotope, Taxa=="Biofilm")
Biofilm<-Biofilm[,c(5,6,21)]%>%
  dplyr::rename(d13C_biofilm = d13C)

pri_bio_con<-cbind(Primary_consumer, Biofilm)

ggplot(aes(x=biofilm$d13C, y=pri_consumer$d13C))+
  geom_point()+
  geom_abline(slope=1, intercept=0, size=1) +theme_bw()+ theme(axis.line=element_line(colour="black", size=1))
